...
CREATE TABLE `acc_bank_reconciliations` (
  ...
  `beginning_balance` decimal(15,2) DEFAULT NULL COMMENT 'S-SOP-KNOWN-ISSUES I11: previous completed reconciliation''s statement ending balance, else the account''s opening balance',
  `cleared_balance` decimal(15,2) DEFAULT NULL COMMENT 'I11: beginning + cleared deposits − cleared withdrawals; Difference = statement − cleared',
...
CREATE TABLE `acc_bill_lines` (
  ...
  `equipment_unit_id` int unsigned DEFAULT NULL COMMENT 'S-SOP-KNOWN-ISSUES I15: trailer/truck this cost belongs to; stamped on the bill JE line for the Per-Unit P&L (NULL = the bill header unit)',
  ...
  KEY `idx_equipment_unit` (`equipment_unit_id`),
  ...
  CONSTRAINT `fk_bill_lines_equipment_unit` FOREIGN KEY (`equipment_unit_id`) REFERENCES `equipment_units` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `acc_customer_deposits` (
  ...
  `bank_account_id` int unsigned DEFAULT NULL COMMENT 'S-SOP-KNOWN-ISSUES I10: bank account the deposit went into; NULL = accounting.default_cash_account_id',
  ...
  CONSTRAINT `fk_customer_deposits_bank_account` FOREIGN KEY (`bank_account_id`) REFERENCES `acc_bank_accounts` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `acc_fixed_assets` (
  ...
  `is_opening_balance` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'S-FA-IMPORT: 1 = asset predates the system; acquisition_date is a book-entry date, not a cash event. Excluded from cash-flow asset acquisitions.',
  `vendor_id` int unsigned DEFAULT NULL,
  ...
  KEY `idx_fa_opening_balance` (`is_opening_balance`,`acquisition_date`),
...
CREATE TABLE `acc_qbo_bank_account_map` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `acc_qbo_bank_transaction_map` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `acc_qbo_bill_map` (
  ...
  `origin` enum('ff_push','cutover_link') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'ff_push' COMMENT 'S-QBO-GOLIVE-AUDIT: ff_push = FF created the QBO bill; cutover_link = linked to one QBO already had',
  `link_method` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'S-QBO-GOLIVE-AUDIT: exact | doc_number | amount_unit | amount_date | manual | push_new',
  `linked_at` datetime DEFAULT NULL COMMENT 'S-QBO-GOLIVE-AUDIT: UTC time of the cutover link',
...
CREATE TABLE `acc_qbo_credit_memo_map` (
  ...
  `origin` enum('ff_push','cutover_link') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'ff_push' COMMENT 'S-QBO-GOLIVE-AUDIT: ff_push = FF created the QBO credit memo; cutover_link = linked to one QBO already had',
  `link_method` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'S-QBO-GOLIVE-AUDIT: exact | doc_number | amount_unit | amount_date | manual (a link) or push_new (released to push)',
  `linked_at` datetime DEFAULT NULL COMMENT 'S-QBO-GOLIVE-AUDIT: UTC time of the cutover link',
...
CREATE TABLE `acc_qbo_invoice_map` (
  ...
  `origin` enum('ff_push','cutover_link') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'ff_push' COMMENT 'S-QBO-GOLIVE-AUDIT: ff_push = FF created the QBO invoice; cutover_link = linked to an invoice QBO already had (never updated/voided by FF)',
  `link_method` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'S-QBO-GOLIVE-AUDIT: exact | doc_number | amount_unit | amount_date | manual (a link) or push_new (pre-go-live doc released to push)',
  `linked_at` datetime DEFAULT NULL COMMENT 'S-QBO-GOLIVE-AUDIT: UTC time of the cutover link',
...
CREATE TABLE `acc_tax_remittances` (
  ...
  `amount` decimal(15,2) NOT NULL,
  `direction` enum('payment','refund') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'payment' COMMENT 'S-SOP-KNOWN-ISSUES I4: refund = CRA paid FF (ITCs exceeded GST collected); amount stays positive',
  `tax_collected_cleared` decimal(15,2) DEFAULT NULL COMMENT 'I4: amount debited to the tax payable account (2030/2040) by this remittance',
  `itc_cleared` decimal(15,2) DEFAULT NULL COMMENT 'I4: GST/HST input tax credits credited out of 1050 by this remittance (0 for PST)',
...
CREATE TABLE `ai_summaries` (
  ...
  `entity_id` int unsigned NOT NULL,
  `summary_type` enum('lease_summary','customer_insights','fleet_health','unit_analysis','payment_risk','forecast','anomaly','accounting_overview','pl_narrative','bs_narrative','cashflow_narrative','budget_variance','invoice_analysis','reservation_summary','vendor_summary','payment_summary') COLLATE utf8mb4_unicode_ci NOT NULL,
  `with_money` tinyint(1) NOT NULL DEFAULT '1' COMMENT 'S-AI-SUMMARY-MONEY-KEY: 1 = generated for a can_view_financials() viewer (may contain amounts); 0 = money-redacted context. Part of the cache key - never serve a 1 row to a 0 viewer.',
  `content` longtext COLLATE utf8mb4_unicode_ci NOT NULL,
  `tokens_used` int unsigned DEFAULT NULL,
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_entity_type` (`entity_type`,`entity_id`,`summary_type`,`with_money`),
  KEY `idx_entity` (`entity_type`,`entity_id`),
...
CREATE TABLE `attention_item_events` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `billing_charges` (
  ...
  `customer_id` int unsigned NOT NULL,
  `description` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
  `item_type` enum('other','damage','fuel','wash','sweep','manual_adjustment') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'other' COMMENT 'invoice_line_items.item_type the line is billed as',
  `quantity` decimal(10,4) NOT NULL DEFAULT '1.0000',
  `unit_price` decimal(12,2) NOT NULL,
  `amount` decimal(12,2) NOT NULL COMMENT 'quantity x unit_price, in the lease currency',
  `taxable` tinyint(1) NOT NULL DEFAULT '1',
  `recurrence` enum('once','monthly') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'once',
  ...
  KEY `fk_billing_charge_created_by` (`created_by`),
  KEY `fk_billing_charge_cancelled_by` (`cancelled_by`),
...
CREATE TABLE `billing_cycle_readings` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `cycle_id` int unsigned NOT NULL,
  `lease_id` int unsigned NOT NULL,
  `odometer_km` decimal(10,2) DEFAULT NULL COMMENT 'Period-end odometer, canonical km',
  `entered_unit` enum('km','miles') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'km' COMMENT 'Unit the operator typed the odometer in',
  ...
  KEY `fk_cycle_reading_user` (`entered_by`),
...
CREATE TABLE `billing_cycle_reviews` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `cycle_id` int unsigned NOT NULL,
  `invoice_id` int unsigned NOT NULL,
  `status` enum('reviewed','query') COLLATE utf8mb4_unicode_ci NOT NULL,
  `note` varchar(500) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  ...
  KEY `fk_cycle_review_user` (`reviewed_by`),
...
CREATE TABLE `billing_cycles` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `reference` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'BC-YYYY-MM',
  ...
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `step_signoffs` json DEFAULT NULL COMMENT 'Per-step sign-off {step:{by,by_id,at,note}}',
  PRIMARY KEY (`id`),
  ...
  KEY `fk_billing_cycle_owner` (`owner_user_id`),
  KEY `fk_billing_cycle_opened_by` (`opened_by`),
  KEY `fk_billing_cycle_closed_by` (`closed_by`),
  KEY `fk_billing_cycle_reopened_by` (`reopened_by`),
...
CREATE TABLE `billing_holds` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `scope` enum('lease','customer') COLLATE utf8mb4_unicode_ci NOT NULL,
  ...
  `release_note` varchar(500) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `created_by` int unsigned DEFAULT NULL,
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  ...
  KEY `fk_billing_hold_created_by` (`created_by`),
  KEY `fk_billing_hold_released_by` (`released_by`),
...
CREATE TABLE `conversation_reads` (
  ...
  `last_read_at` datetime DEFAULT NULL COMMENT 'UTC time last_read_message_id last advanced (drives "Seen <time>")',
...
CREATE TABLE `credit_note_refunds` (
  ...
  KEY `idx_bank_account` (`bank_account_id`),
  KEY `idx_journal_entry` (`journal_entry_id`),
  KEY `fk_cn_refunds_created_by` (`created_by`),
...
CREATE TABLE `credit_notes` (
  ...
  `customer_id` int unsigned NOT NULL,
  `lease_id` int unsigned DEFAULT NULL,
  `source` enum('mileage_overpayment','invoice_adjustment','damage_resolution','goodwill','payment_returned','overpayment','other','precharge_refund','base_rental_reconciliation_overflow','hours_overpayment') COLLATE utf8mb4_unicode_ci NOT NULL,
  `source_invoice_id` int unsigned DEFAULT NULL,
  `source_payment_id` int unsigned DEFAULT NULL,
  `amount` decimal(12,2) NOT NULL,
  `currency` enum('CAD','USD') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'CAD',
  `exchange_rate_to_cad` decimal(10,6) DEFAULT NULL COMMENT 'S-AUDIT-BILLING-ENGINE-1: frozen CAD conversion rate for USD credit notes (from the source invoice/payment at mint). NULL for CAD.',
...
CREATE TABLE `customer_equipment_rates` (
  ...
  `hourly_rate` decimal(10,4) DEFAULT NULL COMMENT 'S-RATECARD-HOURLY: per-item hourly rate override for this customer.',
  `currency` enum('CAD','USD') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'CAD',
...
CREATE TABLE `customer_rate_history` (
  ...
  `mileage_rate` decimal(8,4) DEFAULT NULL,
  `mileage_unit` enum('km','miles') COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `hourly_rate` decimal(10,4) DEFAULT NULL COMMENT 'S-RATECARD-HOURLY: hourly rate snapshot at the time this history row was written.',
  `currency` enum('CAD','USD') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'CAD',
...
CREATE TABLE `equipment_distance_logs` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  ...
  `source` enum('obd','gps','manual') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'gps' COMMENT 'obd=ECU odometer, gps=gateway GPS, manual=user-edited before save',
  ...
  CONSTRAINT `fk_edl_unit` FOREIGN KEY (`equipment_unit_id`) REFERENCES `equipment_units` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Saved Samsara distance-period query results per unit (S-UNIT-DISTANCE-SECTION). Hard delete + audit_log for traceability.';
...
CREATE TABLE `equipment_templates` (
  ...
  `default_hourly_rate` decimal(10,4) DEFAULT NULL COMMENT 'S-RATECARD-HOURLY: default hourly rate for new leases on this template. NULL = not applicable.',
  ...
  UNIQUE KEY `slug` (`slug`),
  KEY `created_by` (`created_by`),
  KEY `idx_et_category_id` (`category_id`),
  KEY `idx_et_subcategory_id` (`subcategory_id`),
  CONSTRAINT `equipment_templates_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`) ON DELETE SET NULL,
  CONSTRAINT `et_ibfk_cat` FOREIGN KEY (`category_id`) REFERENCES `equipment_categories` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `et_ibfk_subcat` FOREIGN KEY (`subcategory_id`) REFERENCES `equipment_subcategories` (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `invoice_billing_exceptions` (
  ...
  `period_end` date NOT NULL,
  `reason` text COLLATE utf8mb4_unicode_ci NOT NULL,
  `source` enum('batch_generate','batch_run','manual','cron') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'batch_generate',
...
CREATE TABLE `invoices` (
  ...
  `engine_hours_at_period_start` decimal(10,2) DEFAULT NULL COMMENT 'S-LEASE-HOURLY-BILLING: engine hours at start of this invoice period (manual)',
  `engine_hours_at_period_end` decimal(10,2) DEFAULT NULL COMMENT 'S-LEASE-HOURLY-BILLING: engine hours at end of this invoice period (manual)',
  `period_engine_hours` decimal(10,2) DEFAULT NULL COMMENT 'S-LEASE-HOURLY-BILLING: engine hours billed in this period (end - start, clamped >=0)',
...
CREATE TABLE `leases` (
  ...
  `start_date` date NOT NULL,
  `start_time` time DEFAULT NULL COMMENT 'Pickup time of day (S-LEASE-RENTAL-DAY-TIME). NULL = legacy lease, no time captured.',
  `end_date` date DEFAULT NULL,
  `end_time` time DEFAULT NULL COMMENT 'Expected return time of day (S-LEASE-RENTAL-DAY-TIME). Optional; informs the operator when the unit should be back.',
  `actual_return_date` date DEFAULT NULL,
  `actual_return_time` time DEFAULT NULL COMMENT 'Actual return time of day at lease close (S-LEASE-RENTAL-DAY-TIME). Compared with start_time + return_grace_minutes to compute effective_billable_end_date.',
  ...
  `estimated_mileage_per_day` decimal(10,2) NOT NULL DEFAULT '0.00' COMMENT 'S-MILEAGE-EST-DAILY: estimated distance driven per day in the lease mileage_unit. Billed as an estimate each period (days x per-day x mileage_rate) then trued-up against actual. 0 = no estimated-mileage billing.',
  `estimated_mileage_per_day_km` decimal(12,4) DEFAULT NULL COMMENT 'S-MILEAGE-EST-DAILY: per-day estimate in km (canonical, drives the estimate math). Matches estimated_mileage_per_day when mileage_unit=km.',
  `estimated_mileage_per_day_miles` decimal(12,4) DEFAULT NULL COMMENT 'S-MILEAGE-EST-DAILY: per-day estimate in miles (informational when mileage_unit=km). Matches estimated_mileage_per_day when mileage_unit=miles.',
  ...
  `estimated_engine_hours_per_day` decimal(10,2) NOT NULL DEFAULT '0.00' COMMENT 'S-HOURS-EST-DAILY: estimated engine/reefer hours run per day. Billed as an estimate each period (days x per-day x hourly_rate) then trued-up against actual (engine_hours_at_end - engine_hours_at_start) at close. 0 = no estimated-hours billing.',
  `cartage_amount` decimal(10,2) DEFAULT NULL COMMENT 'S-LEASE-SERVICE-CHARGES: one-time delivery (cartage) charge entered at lease creation. Bills once on the first invoice. NULL/0 = none.',
  `cartage_billed_at` datetime DEFAULT NULL COMMENT 'S-LEASE-SERVICE-CHARGES: stamped when the cartage line is emitted, so cartage bills exactly once (on the first invoice).',
...
CREATE TABLE `notifications` (
  ...
  `entity_type` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `entity_id` int unsigned DEFAULT NULL,
  `group_key` varchar(191) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'S-ATTENTION-INBOX: same-key unread updates collapse into one row',
  `group_count` int unsigned NOT NULL DEFAULT '1' COMMENT 'How many updates this row stands for',
  ...
  KEY `idx_deleted` (`deleted_at`),
  KEY `idx_notif_user_group` (`user_id`,`group_key`,`is_read`),
...
CREATE TABLE `payments` (
  ...
  KEY `idx_deposit_bank_account` (`deposit_bank_account_id`),
  CONSTRAINT `fk_payments_deposit_bank_account` FOREIGN KEY (`deposit_bank_account_id`) REFERENCES `acc_bank_accounts` (`id`) ON DELETE SET NULL,
...
CREATE TABLE `rate_card_items` (
  ...
  `equipment_type` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
  `equipment_template_id` int unsigned DEFAULT NULL COMMENT 'S-RATE-CARD-TEMPLATE-ITEM: NULL=category-level row; non-NULL=specific template override',
  `daily_rate` decimal(10,2) DEFAULT NULL,
  `weekly_rate` decimal(10,2) DEFAULT NULL,
  `monthly_rate` decimal(10,2) DEFAULT NULL,
  `mileage_rate` decimal(8,4) DEFAULT NULL,
  `mileage_unit` enum('km','miles') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'km',
  `hourly_rate` decimal(10,4) DEFAULT NULL COMMENT 'S-RATECARD-HOURLY: per-item hourly rate (e.g. reefer engine-hours $/hr). NULL = not applicable.',
  `gps_price` decimal(10,2) DEFAULT NULL COMMENT 'S-GPS-RATE-ITEM: per-item GPS daily rate ($/day). NULL = no GPS charge for this equipment type.',
  `minimum_days` tinyint unsigned DEFAULT NULL COMMENT 'S-LEASE-MIN-DAYS: short-lease floor for this equipment row. When a lease bills fewer than this many days, charge a flat minimum_days x daily_rate. NULL = no minimum. Config layer 1 (overrides lease + global default).',
  `currency` enum('CAD','USD') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'CAD',
  `notes` text COLLATE utf8mb4_unicode_ci,
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `users` (
  ...
  `login_attempts` tinyint unsigned NOT NULL DEFAULT '0',
  `locked_until` datetime DEFAULT NULL,
  `locked_at` datetime DEFAULT NULL COMMENT 'S-USER-LOCKOUT: when a super_admin set status=locked. NULL unless currently locked.',
  `locked_by` int unsigned DEFAULT NULL COMMENT 'S-USER-LOCKOUT: users.id of the super_admin who locked this account.',
  `lock_reason` varchar(500) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'S-USER-LOCKOUT: mandatory reason captured when the account was locked.',
  `invite_token` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `invite_token_expiry` datetime DEFAULT NULL,
  `invite_sent_at` datetime DEFAULT NULL,
  `password_reset_token` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  ...
  `whatsapp_mode` enum('off','summary','urgent') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'off' COMMENT 'S-ATTENTION-WHATSAPP: off | summary | urgent (urgent items + morning summary)',
  `whatsapp_summary_hour` tinyint unsigned DEFAULT NULL COMMENT 'Local hour 0-23 for the morning summary; NULL = 7',
  `whatsapp_quiet_start` tinyint unsigned DEFAULT NULL COMMENT 'Local hour quiet time starts; NULL = 21',
  `whatsapp_quiet_end` tinyint unsigned DEFAULT NULL COMMENT 'Local hour quiet time ends; NULL = 7',
  `whatsapp_updates` json DEFAULT NULL COMMENT 'Update types also sent instantly on WhatsApp',
  `whatsapp_opted_in_at` datetime DEFAULT NULL COMMENT 'UTC; when the person turned WhatsApp on themselves (opt-in record)',
