...
/**
 * app/admin/billing/run.php   (was app/admin/invoices/batch.php)
 *
 * S-BATCH-INVOICING — Batch Invoicing, now the Billing module's WORKBENCH
 * (S-BILLING-MODULE): the one place a month's invoices are generated in
 * bulk. Opened from a billing cycle (?cycle=ID) it starts on that month;
 * /invoices/batch redirects here. What moved OUT of this page (so it exists
 * once): the Needs Attention exceptions list and the approval-runs list now
 * live on Billing home (Exceptions / Approvals tabs) — this page links there.
 * What it gained: billing holds (held leases are shown, never auto-selected,
 * and refused server-side) and the cycle's period-end readings (per-lease
 * "reading" chip; generation passes them to the invoice).
 *
 ...
 * @session  S-BATCH-INVOICING, S-BILLING-MODULE
 */
...
$defaultMonth = substr(ff_today(), 0, 7);
$todayIso     = ff_today();

// S-BILLING-MODULE: opened from a cycle → starts on that month.
$cycleCtx  = null;
$lockMonth = null;
if (($cid = clean_int($_GET['cycle'] ?? null)) && $cid > 0) {
    $cycleCtx = \FleetForge\Billing\Cycle\BillingCycles::find($cid);
} elseif (\FleetForge\Billing\Cycle\BillingCycles::isValidMonth((string) ($_GET['month'] ?? ''))) {
    $cycleCtx = \FleetForge\Billing\Cycle\BillingCycles::findByMonth((string) $_GET['month']);
    $lockMonth = (string) $_GET['month'];
}
if ($cycleCtx) {
    $lockMonth = $cycleCtx['month'];
}
if ($lockMonth) {
    $defaultMonth = $lockMonth;
}

$pageTitle      = 'Billing workbench';
...
<?php ob_start(); ?>
    <?= help_button('billing') ?>
    <?php if ($cycleCtx): ?>
    <a href="<?= base_url('billing/cycle') ?>?id=<?= (int) $cycleCtx['id'] ?>" class="btn btn-secondary btn-sm">← Back to <?= e($cycleCtx['label']) ?></a>
    <?php endif; ?>
    <a href="<?= base_url('billing') ?>" class="btn btn-secondary btn-sm"><?= heroicon('calendar-days', 'icon-sm') ?> All cycles</a>
<?php
$wbCrumbs = [['Dashboard', base_url('dashboard')], ['Billing', base_url('billing')]];
if ($cycleCtx) {
    $wbCrumbs[] = [$cycleCtx['reference'], base_url('billing/cycle') . '?id=' . (int) $cycleCtx['id']];
}
$wbCrumbs[] = ['Workbench', null];
?>
<?= \FleetForge\Ui\ModuleHero::render([
    ...
    'accent'   => 'primary',
    'title'    => $cycleCtx ? 'Workbench — ' . $cycleCtx['label'] : 'Billing workbench',
    'subtitle' => 'Generate a month\'s (or a custom range\'s) draft invoices for the selected leases, dry-run and review them, then send.',
    'art'      => 'invoices',
    'actions'  => ob_get_clean(),
]) ?>

<?php if ($cycleCtx && $cycleCtx['status'] === 'closed'): ?>
<div class="alert alert-warning" style="margin-bottom:14px;">
    <strong><?= e($cycleCtx['reference']) ?> is closed.</strong> The workbench cannot generate or submit runs for <?= e($cycleCtx['label']) ?>
    until the cycle is reopened on its page. Sending existing drafts still works.
</div>
<?php endif; ?>
...
<div id="batch-invoicing-app"
     x-data="BatchInvoicing({ defaultMonth: <?= e(json_encode($defaultMonth)) ?>, lockMonth: <?= e(json_encode($lockMonth)) ?>, cycleId: <?= $cycleCtx ? (int) $cycleCtx['id'] : 'null' ?>, today: <?= e(json_encode($todayIso)) ?>, canGenerate: <?= $canGenerate ? 'true' : 'false' ?>, canSend: <?= $canSend ? 'true' : 'false' ?> })">
    ...
    <div class="batch-split" :class="{ 'has-preview': previewInvoiceId }">
        ...
        <div class="batch-left">
                                ...
                                <template x-for="l in c.leases" :key="l.id">
                                    <div class="batch-lease-row" :class="{ 'is-selected': !!selected[l.id] }" @click="toggleLease(l)">
                                        ...
                                        <div class="batch-lease-info">
                                            ...
                                            <span class="badge badge-no-dot badge-neutral batch-chip" x-show="l.hold" :title="l.hold ? l.hold.description : ''">On hold</span>
                                            <span class="badge badge-no-dot badge-warning batch-chip" x-show="l.reading === 'missing' && l.billing_status !== 'billed'"
                                                  title="Manual-mileage / hourly lease with no period-end reading on the cycle's Readings tab — it will bill no usage this month.">Reading needed</span>
                                            <span class="badge badge-no-dot badge-success batch-chip" x-show="l.reading === 'entered'" title="Period-end reading entered on the cycle — generation uses it.">Reading ✓</span>
                                        </div>
            ...
            <div class="batch-action-bar" x-show="canGenerate">
                ...
                <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                    <?php if ($approvalRequired): ?>
                        <span class="badge badge-no-dot badge-warning" title="Billing → Settings → Approval">
                            Approval required
                        </span>
                    <?php endif; ?>
            ...
            <!-- ── Exceptions + approval runs (S-BILLING-MODULE) ─────────
                 Their full lists live on Billing home (Exceptions and
                 Approvals tabs) — shown once, there. The workbench only
                 counts them so nothing waiting is missed. -->
            <div class="card" x-show="exceptions.length || runs.some(r => r.status === 'pending' || r.status === 'approved')" x-cloak>
                <div class="card-body" style="display:flex; gap:18px; flex-wrap:wrap; align-items:center;">
                    <a class="link" x-show="exceptions.length" href="<?= base_url('billing') ?>#exceptions">
                        <span class="badge badge-no-dot badge-warning" x-text="exceptions.length"></span>
                        billing exception(s) need attention — leases that could not be billed
                    </a>
                    <a class="link" x-show="runs.some(r => r.status === 'pending' || r.status === 'approved')" href="<?= base_url('billing') ?>#approvals">
                        <span class="badge badge-no-dot badge-info" x-text="runs.filter(r => r.status === 'pending' || r.status === 'approved').length"></span>
                        approval run(s) waiting for a decision or to be generated
                    </a>
                </div>
            </div>
...
function BatchInvoicing(cfg) {
    return {
        ...
        init() {
            ...
            this.restoreState();
            // S-BILLING-MODULE: opened from a cycle → start on its month. A
            // restored session for a different period is discarded rather
            // than silently generating the wrong month.
            if (cfg.lockMonth) {
                if (this.periodMode !== 'month' || this.monthValue !== cfg.lockMonth) {
                    if (this.restoredFrom) this.clearSavedState();
                    this.periodMode = 'month';
                    this.monthValue = cfg.lockMonth;
                }
            }
            this.updatePeriod();
            this.loadEligible();
        ...
        runUrl(id) { return '<?= base_url('billing/approval') ?>?id=' + id; },
        ...
        async downloadBatch(format) {
            ...
            try {
                ...
                a.click();
                a.remove();
                // Revoking in the same tick as click() can cancel the download
                // in Safari / older Firefox — free the blob a minute later.
            ...
            try {
                ...
                if (res.success) {
                    ...
                    this.customers.forEach(c => c.leases.forEach(l => {
                        ...
                        if (l.hold) return; // S-BILLING-MODULE: a held lease is never pre-selected
        ...
        selectAllUnbilled() {
            this.selectionRestored = false;
            this.customers.forEach(c => c.leases.forEach(l => {
                if (l.billing_status === 'unbilled' && !l.hold) this.selected[l.id] = true;
            }));
        },
        clearSelection() {
            this.selectionRestored = false;
        ...
        // S-PDF-LETTERHEAD: open the streaming endpoint SYNCHRONOUSLY in the
        // click handler — it generates the PDF if needed and returns the bytes.
        // The old POST generate_pdf → await → window.open() was silently
        // blocked as a popup by Safari/Chrome (the open ran after an await).
        viewOrGeneratePdf(item) {
            window.open('<?= base_url('api/v1/invoices/pdf') ?>?id=' + item.invoice_id, '_blank');
            item.has_pdf = true;
        },
