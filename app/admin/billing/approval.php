...
/**
 * app/admin/billing/approval.php   (was app/admin/invoices/batch_run.php)
 *
 * S-BATCH-APPROVAL — a single batch run at its own shareable URL
 * (/billing/approval?id=N; the old /invoices/batch_run URL redirects here).
 * This is the page a manager who is NOT doing
 ...
 *
 * S-BILLING-MODULE: re-homed into Billing. The page is ALL money, so it now
 * also requires can_view_financials() (it was readable by any invoices:view
 * role, dispatchers included). A pending run can be CANCELLED (the status
 * existed but nothing set it) by its submitter or an approver; a rejected or
 * cancelled run can be RESUBMITTED as a new run over the same leases.
 *
...
// Every figure on this page is an amount — the financial predicate gates it.
if (!can_view_financials()) {
    require_permission('payments', 'view'); // renders the standard 403
}

$runId = clean_int($_GET['id'] ?? null);
if (!$runId || $runId <= 0) {
    header('Location: ' . base_url('billing') . '#approvals');
    exit;
}
...
if (!$run) {
    ...
    echo '<div class="card"><div class="card-body"><p class="text-secondary">That batch run does not exist or has been deleted.</p>'
       . '<a class="btn btn-secondary btn-sm" href="' . e(base_url('billing')) . '#approvals">Back to Billing</a></div></div>';
...
// Cancel: the submitter (withdrawing their own run) or anyone who can approve.
$canCancel   = $canApprove || ((int) ($run['submitted_by'] ?? 0) === (int) current_user_id() && $canGenerate);
$leaseIdsForResubmit = json_decode((string) $run['lease_ids'], true) ?: [];

// The billing cycle this run's month belongs to (runs are usually a full month).
$runCycle = db_row("SELECT id, reference FROM billing_cycles WHERE period_start <= ? AND period_end >= ? ORDER BY period_start LIMIT 1",
...
<nav class="breadcrumb">
    ...
    <a href="<?= base_url('billing') ?>">Billing</a>
    <span class="breadcrumb-sep">/</span>
    <?php if ($runCycle): ?>
    <a href="<?= base_url('billing/cycle') ?>?id=<?= (int) $runCycle['id'] ?>"><?= e($runCycle['reference']) ?></a>
    <span class="breadcrumb-sep">/</span>
    <?php else: ?>
    <a href="<?= base_url('billing') ?>#approvals">Approvals</a>
    <span class="breadcrumb-sep">/</span>
    <?php endif; ?>
...
<div id="batch-run-app"
     x-data="BatchRunPage({
        ...
        periodStart: <?= e(json_encode($run['period_start'])) ?>,
        periodEnd: <?= e(json_encode($run['period_end'])) ?>,
        leaseIds: <?= e(json_encode(array_values(array_map('intval', $leaseIdsForResubmit)))) ?>,
        note: <?= e(json_encode((string) ($run['note'] ?? ''))) ?>
     })">
    ...
    <div class="batch-review-surface" data-theme="light">
        <div class="batch-review-head">
            ...
            <div class="batch-review-head-actions">
                ...
                <?php if ($canCancel): ?>
                <template x-if="status === 'pending'">
                    <button type="button" class="btn btn-ghost btn-sm" :disabled="working" @click="cancelRun()">Cancel run</button>
                </template>
                <?php endif; ?>
                <?php if ($canGenerate): ?>
                <template x-if="status === 'rejected' || status === 'cancelled'">
                    <button type="button" class="btn btn-secondary btn-sm" :disabled="working" @click="resubmit()">Resubmit as a new run</button>
                </template>
                <?php endif; ?>

                <a class="btn btn-ghost btn-sm" href="<?= $runCycle ? e(base_url('billing/cycle')) . '?id=' . (int) $runCycle['id'] : e(base_url('billing')) . '#approvals' ?>">Back</a>
            </div>
        </div>
        ...
        <div class="batch-review-body">
            ...
            <div class="brc-note" data-kind="<?= in_array($run['status'], ['rejected', 'cancelled'], true) ? 'reject' : 'approve' ?>">
                <div class="batch-review-section-title">
                    <?= $run['status'] === 'rejected' ? 'Rejected' : ($run['status'] === 'cancelled' ? 'Cancelled' : 'Approved') ?>
...
function BatchRunPage(cfg) {
    return {
        ...
        // S-BILLING-MODULE — withdraw a pending run (status 'cancelled').
        async cancelRun() {
            if (this.working) return;
            const reason = await FF_Confirm.askText({
                title: 'Cancel this batch run',
                message: 'The run stays on record as cancelled; nothing is billed. Say why.',
                confirmLabel: 'Cancel run',
                placeholder: 'e.g. Submitted the wrong month',
            });
            if (!reason) return;
            this.working = true;
            try {
                const r = await FF_Api.post('<?= base_url('api/v1/invoices/batch_runs/cancel') ?>', { id: this.id, reason: reason });
                if (r.success) {
                    FF_Toast.success('Run cancelled.');
                    setTimeout(() => location.reload(), 700);
                } else {
                    FF_Toast.error(r.error?.message || 'Could not cancel the run.');
                }
            } catch (e) {
        ...
        // S-BILLING-MODULE — a rejected/cancelled run is re-proposed as a NEW
        // run (fresh dry run over the same leases, against today's data); the
        // old run stays on record untouched.
        async resubmit() {
            if (this.working) return;
            const note = await FF_Confirm.askText({
                title: 'Resubmit for approval',
                message: 'A new run is priced again from today\'s data over the same ' + cfg.leaseIds.length + ' lease(s). Add a note for the approver.',
            ...
            try {
                const r = await FF_Api.post('<?= base_url('api/v1/invoices/batch_runs/create') ?>', {
                    period_start: cfg.periodStart, period_end: cfg.periodEnd,
                    lease_ids: cfg.leaseIds, note: note || cfg.note,
                });
                if (r.success) {
                    FF_Toast.success('Resubmitted as ' + r.data.reference + '.');
                    setTimeout(() => { window.location.href = r.data.url; }, 700);
                } else {
                    FF_Toast.error(r.error?.message || 'Could not resubmit.');
                }
            } catch (e) {
                FF_Toast.error('Network error.');
            } finally {
