 ...
 *
 * Customers Sync — bidirectional FF↔QBO mapping UI. Lists every
 * row in acc_qbo_customer_map with KPI tiles + filter bar + main
 * table. Per-row actions vary by mapping_status:
 *
 *   ff_only   → "Link to QBO…" picks a QBO customer from the
 *               qbo_only side and promotes the row to 'mapped'.
 *   qbo_only  → "Link to FF…"  picks an FF customer from the
 *               ff_only side (or any active FF customer with no
 *               existing row) and promotes to 'mapped'.
 *   mapped    → "Override Match" + "Unlink" + "Ignore".
 *   ignored   → "Un-ignore" restores the natural ff_only / qbo_only /
 *               mapped state based on populated sides.
 *
 * Two top-row buttons:
 *   "Pull from QuickBooks" → POSTs api/v1/quickbooks/customers/pull.php
 *                            (real HTTP to sandbox; populates qbo_only rows)
 *   "Auto-Match"           → POSTs api/v1/quickbooks/customers/auto_match.php
 *                            (no HTTP; runs CustomerMatcher cascade against
...
<div x-data="qboCustomerMapping()">
    ...
    <div class="table-toolbar">
        ...
        <div class="table-toolbar-left table-toolbar-left--wrap">
            <input aria-label="Search" type="search" class="form-control form-control-sm" maxlength="255" placeholder="FF or QBO name…"
            ...
            <select aria-label="Filter by confidence" class="form-select form-control-sm" x-model="filters.confidence" @change="page=1; reload()">
    ...
    <template x-if="!loading && rows.length === 0 && (kpis === null || (kpis.mapped + kpis.ff_only + kpis.qbo_only + kpis.ignored) === 0)">
        <div class="card" style="padding:32px;text-align:center;">
            <div class="h5" style="margin-bottom:8px;">No customer mappings yet</div>
            <div class="text-secondary text-sm" style="margin-bottom:16px;">
                Click <strong>Pull from QuickBooks</strong> to fetch your sandbox customers,<br>
                then <strong>Auto-Match</strong> to suggest pairings against FleetForge customers.
            </div>
        </div>
    </template>
            ...
            <table class="table table-sm" style="margin:0;">
                <thead>
                    <tr>
                        ...
                        <th>Status</th>
                        <th>Confidence</th>
                        <th class="text-right">QBO Balance</th>
                        <th>Last Synced</th>
                ...
                <tbody>
                    <template x-for="row in rows" :key="row.mapping_id">
                        <tr>
                            <td>
                                <template x-if="row.ff_customer_id">
                                    <a :href="ffCustomerUrl(row.ff_customer_id)" x-text="row.ff_company_name || '#' + row.ff_customer_id"></a>
                                </template>
                                <template x-if="!row.ff_customer_id">
                            ...
                            <td class="text-right font-mono" x-text="row.qbo_balance !== null ? '$' + Number(row.qbo_balance).toFixed(2) : '—'"></td>
    ...
    return {
        ...
        async reload() {
            ...
            try {
                ...
                const j = await FF_Api.get(FF_Api.url('/api/v1/quickbooks/customers/list.php?' + params.toString()));
                if (j.success) {
                    const d = j.data;
                    this.rows         = d.rows;
        ...
        async runPull() {
            ...
            try {
                const j = await FF_Api.post(FF_Api.url('/api/v1/quickbooks/customers/pull.php'), {});
                if (j.success) {
                    const d = j.data;
                    this.flash = {
                        message: 'Pulled ' + d.pulled_count + ' QBO customers (' + d.inserted + ' new, ' + d.updated + ' updated).',
                        type: 'success',
                    };
                    await this.reload();
                } else {
        ...
        async runAutoMatch() {
            ...
            this.flash = { message: '', type: '' };
            try {
                const j = await FF_Api.post(FF_Api.url('/api/v1/quickbooks/customers/auto_match.php'), {});
                if (j.success) {
                    const d = j.data;
                    this.flash = {
        ...
        openLinkModal(row, mode) {
            // Populate the picker options from the OPPOSITE side's
            // single-sided rows. mode='pick_qbo' → list qbo_only rows
            // (the FF row already lives in `row`). mode='pick_ff' →
            // list ff_only rows.
            ...
            // Fetch a generous page of candidates so the picker has
            // most realistic choices without paging UI inside the modal.
            const params = new URLSearchParams({
                status:     wanted,
                confidence: 'all',
                q:          '',
            ...
            FF_Api.get(FF_Api.url('/api/v1/quickbooks/customers/list.php?' + params.toString()))
                .then(j => {
                    ...
                    this.linkModal.options = j.data.rows.map(r => mode === 'pick_qbo'
                        ? { id: r.qbo_customer_id, label: (r.qbo_display_name || '(no name)') + ' · qbo#' + r.qbo_customer_id }
                        : { id: r.ff_customer_id,  label: (r.ff_company_name  || '(no name)') + ' · ff#'  + r.ff_customer_id }
                    );
                })
        ...
        async saveLink() {
            ...
            try {
                const body = (this.linkModal.mode === 'pick_qbo')
                    ? { action: 'link', ff_customer_id: this.linkModal.row.ff_customer_id, qbo_customer_id: this.linkModal.selectedId, notes: this.linkModal.notes }
                    : { action: 'link', ff_customer_id: this.linkModal.selectedId,        qbo_customer_id: this.linkModal.row.qbo_customer_id, notes: this.linkModal.notes };
        ...
        ffCustomerUrl(id) {
            return FF_Api.url('/customers/show?id=' + encodeURIComponent(id));
        },
