    ...
    <form @submit.prevent="submit()" novalidate>
        ...
        <div class="card" style="margin-bottom:1.5rem;">
            ...
            <div class="card-body">
                ...
                <!-- ── Estimated engine hours per day (S-HOURS-EST-DAILY) — ALWAYS VISIBLE
                     (S-HOURS-EST-VIS), the parallel of "estimated mileage per day". Was gated
                     behind hourly_rate>0 so the feature was invisible until a rate was set. -->
                <div class="form-group" style="max-width:320px;margin-top:0.75rem;">
                    ...
                    <div class="form-hint">For reefer / hourly-billed units. Each invoice bills days &times; this &times; the <strong>Hourly Rate</strong> as an estimate, then trues up against actual (end &minus; start) engine hours at close.
                        <span x-show="!(parseFloat(form.hourly_rate) > 0)" style="color:var(--color-warning,#c47f17);">Set an Hourly Rate above to bill it.</span> 0 = bill only actual hours.</div>
                    <div class="form-error" x-show="errors.estimated_engine_hours_per_day" x-text="errors.estimated_engine_hours_per_day"></div>
                </div>

                <!-- S-LEASE-HOURLY-BILLING: engine-hours readings (meter). Shown once an
                     hourly rate is set. Start editable; end is set at close (read-only on
                     active leases). -->
