...
class ProRateCalculator
{
    ...
    /**
     * Calculate mileage charge/credit at lease close.
     *
     * adjustment = (actual - estimated) × rate
     * Positive = mileage_adjustment (customer owes more)
     * Negative = mileage_credit (customer gets credit)
     * Zero = no line item needed
     *
     * @param string $actualMileage   Actual mileage driven
     * @param string $estimatedMileage Estimated mileage pre-charged
     * @param string $rate            Rate per unit (e.g., $/km or $/mile)
     * @param string $unit            'km' or 'miles'
     * @return array{amount: string, adjustment: string, is_credit: bool, item_type: string, explanation: string[]}
     */
    public function calculateMileage(string $actualMileage, string $estimatedMileage, string $rate, string $unit): array
    {
        // If rate is zero, skip mileage entirely (spec §13)
        if (bccomp($rate, '0', 6) === 0) {
            return [
                ...
                'is_credit'   => false,
                'item_type'   => 'none',
                'explanation' => ['Mileage rate is $0 — no mileage line item'],
            ];
        }

        $actualCharge = bcmul($actualMileage, $rate, 6);
        $preCharge = bcmul($estimatedMileage, $rate, 6);
        $adjustment = bcsub($actualCharge, $preCharge, 6);
        $adjustmentRounded = bcround($adjustment, 2);

        // Positive: customer owes more
        if (bccomp($adjustment, '0', 6) > 0) {
            return [
                'amount'      => $adjustmentRounded,
                'adjustment'  => $adjustmentRounded,
                'is_credit'   => false,
                'item_type'   => 'mileage_adjustment',
                'explanation' => [
                    "Actual: {$actualMileage} {$unit} × \${$rate}/{$unit} = \$" . bcround($actualCharge, 2),
                    "Pre-charged: {$estimatedMileage} {$unit} × \${$rate}/{$unit} = \$" . bcround($preCharge, 2),
                    "Adjustment (owes more): \${$adjustmentRounded}",
                ],
            ];
        }

        // Negative: customer gets credit
        if (bccomp($adjustment, '0', 6) < 0) {
            // amount is the absolute value for the credit line
            $absAmount = bcmul($adjustmentRounded, '-1', 2);
            return [
                'amount'      => $absAmount,
                'adjustment'  => $adjustmentRounded,
                'is_credit'   => true,
        ...
        return [
            ...
            'is_credit'   => false,
            'item_type'   => 'none',
            'explanation' => ['Actual mileage matches estimate — no adjustment'],
        ];
    }

    /**
     * Calculate mileage pre-charge for Invoice 1.
     *
     * pre_charge = estimated_mileage × mileage_rate
     *
     * @param string $estimatedMileage Estimated mileage for lease
     * @param string $rate             Rate per unit
     * @param string $unit             'km' or 'miles'
     * @return array{amount: string, explanation: string[]}
     */
    public function calculateMileagePreCharge(string $estimatedMileage, string $rate, string $unit): array
    {
        if (bccomp($rate, '0', 6) === 0) {
            return [
                'amount'      => '0.00',
                'explanation' => ['Mileage rate is $0 — no pre-charge'],
            ];
        }

        $amount = bcmul($estimatedMileage, $rate, 6);
        $rounded = bcround($amount, 2);

        return [
            'amount'      => $rounded,
            'explanation' => ["{$estimatedMileage} {$unit} × \${$rate}/{$unit} = \${$rounded} (mileage pre-charge)"],
        ];
    }
}
