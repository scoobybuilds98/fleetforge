...
> **✅ SHIPPED 2026-06-01 via S-QBO-17. Phase QBO-7 / 2 of 2 — CLOSES Phase QBO-7.** Cash precharge-refund counterpart to S-QBO-16 credit memos. NEW `lib/QboPushers/RefundReceiptPusher.php` + `RefundReceiptEnqueuer.php` + `acc_qbo_refund_receipt_map` table (migrate 83→84) + admin UI `/quickbooks/refund_receipts` (D-UI-COMPLETENESS-1; nav 17→18) + enqueue hook in `api/v1/leases/mark_refund_settled.php`. **Scope (D-QBO-17-2): CASH refunds only** — credit-method refunds create a `credit_notes` row at lease close (S-MILEAGE-3 D-C) and push via the CreditMemo path (S-QBO-16) + apply→LinkedTxn (S-QBO-CREDIT-MEMO-APPLY), so they are NOT re-pushed here. **Keying (D-QBO-17-5):** a refund has no dedicated row — it lives on `leases` (precharge_refund_method='cash', precharge_balance=amount, precharge_refund_settled_at), so the queue + map key on `lease_id` (one cash refund per lease → idempotent). **Trigger (D-QBO-17-1):** `mark_refund_settled.php` (settle-time, when money moves), per the S-MILEAGE-3 D-B(i) deferred-settle design. **Refs from settings (D-QBO-17-6):** no QBO PaymentMethod map exists + the settle event captures no per-refund bank account, so `PaymentMethodRef` (cheque) + `DepositToAccountRef` come from operator-configured settings (`quickbooks.refund.payment_method_id` + `quickbooks.refund.deposit_account_id`), surfaced as preflight gates like `tax_override_code_id`. Smoke `_smoke_qbo_refund_receipt_push` 31/31 PASS. 6 D-QBO-17-* locked.
>
> **⚠️ NON tax assumption → F28 live-verify (D-QBO-17-3):** the refund pushes as non-taxable (`TaxCodeRef=NON`, `TotalTax=0`) per §8.7's payload assumption, pending accountant confirmation of refund tax treatment — tracked as a live-verify follow-up (F28, the F16/F19 pattern). Safe to ship because `sync_enabled='0'` until the S-QBO-30 cutover means nothing posts to QBO before the verify. The broader FF-side GL JE for cash refunds remains owned by the separate `S-MILEAGE-3-ACCT-SPEC` session (QBO derives its own ledger posting from the RefundReceipt entity, so it does not block this push).
...
> **✅ SHIPPED 2026-05-29 via S-QBO-21. COMPLETES PHASE QBO-10 (catch-all JE Pusher).** JournalEntryPusher + JournalEntryEnqueuer + JournalEntryService::create/post/reverse enqueue hooks + admin UI `/quickbooks/journal_entries` + new acc_qbo_journal_entry_map table. **Bridge-derived filter per D-QBO-21-1** uses the 5-item FF-ENUM canonical list from the identification rule below (invoice/payment/credit_note/ap_bill/ap_payment) at BOTH Enqueuer gate-0 AND Pusher pushImpl step 4 (defense-in-depth). No-map-row pattern (mirror PaymentPusher skipped_non_ff_origin): sync_log captures via error_code='skipped_bridge_derived'; map row NOT written. PostingType per D-QBO-21-2 (debit-line→'Debit', credit-line→'Credit', Amount=abs(populated)). Per-line AccountRef from acc_qbo_account_map per D-QBO-21-3. DocNumber from entry_number with shared QboFieldLimits::INVOICE_DOC_NUMBER_MAX=21 gate per D-QBO-21-4. pushUpdate stubbed → S-QBO-21-UPDATE-FOLLOWUP per D-QBO-21-5. Header currency + D-QBO-FIXPACK-12 multi_currency gating per D-QBO-21-6. PrivateNote with FF entry_number + source attribution + truncated description + reversal pointer per D-QBO-21-7. 7 decisions locked D-QBO-21-1..7. **K-22 caught + resolved via AskUserQuestion**: prompt's QBO-style 7-item filter list (`bill`/`credit_memo`/`refund_receipt`/`bill_payment`) rejected — those aren't real FF source_type ENUM values; spec §8.10 canonical 5-item FF-ENUM list adopted. See PROGRESS.md SESSION LOG row for full implementation trace.
...
> **Operator-directed addendum 2026-06-01 (S-QBO-FA-MAP):** a per-asset REFERENCE table `acc_qbo_fixed_asset_map` was added at operator request (reverses the S-QBO-22 D-QBO-22-3 "no FA table" sub-decision). It is NOT a push-map to a QBO entity — there is no QBO FixedAsset entity. Each row tracks one FF asset's resolved QBO GL-account refs (asset cost / accumulated depreciation / depreciation expense, via `acc_qbo_account_map`) + cost/accum-depr/NBV/status snapshots + a `sync_status` (`synced` = all three GL accounts mapped so the asset's JEs can reach QBO; `pending` = plumbing incomplete). Maintained by `lib/QboPushers/FixedAssetMapSync.php` (`sync()` / `syncOne()`), triggerable via `api/v1/quickbooks/fixed_asset_map_sync.php` + a "Refresh FA reference map" button on `/quickbooks/journal_entries`, AND auto-refreshed on every drift run — `DriftChecker::runCheck()` calls `FixedAssetMapSync::sync()` as a best-effort maintenance step (emits NO drift_events, so it never interacts with the auto-resolve machinery), so the nightly `qbo_drift_check` cron + the "Run drift check now" button both keep the reference map current. The depreciation/disposal/impairment JEs themselves still push through `acc_qbo_journal_entry_map` (S-QBO-22), unchanged.
...
> **✅ MANUAL SYNC SHIPPED 2026-05-30 via S-QBO-26 (Phase QBO-12 / 3 of 3 — CLOSES Phase QBO-12).** `app/admin/quickbooks/manual_sync.php` + `api/v1/quickbooks/manual_sync.php` (POST; `force_full_resync`-gated). Three action verbs: **force_resync** (bulk re-enqueue every mapped row of an entity type via its `Enqueuer::enqueue(ff_id, 'create')` — 8 push types; the Enqueuer's own gates decide eligibility so payment `origin='ff_native'` D-QBO-14-1 + JE bridge-derived D-QBO-21-1 + master kill-switch + per-entity sync_mode are all honored; surfaces an enqueued/skipped count + reason ladder per D-QBO-26-1/D-QBO-26-4); **force_pull** (bulk `Puller::pullAll()` + SyncToken refresh for the 5 pull-capable reference types customer/vendor/account/item/tax_code; push-only types 422 → S-QBO-27 historical pull per D-QBO-26-2); **reset_synctoken** (NULL `qbo_sync_token` for an entity type — the realm-migration recovery tool, a separate guarded action per D-QBO-26-3). Reuses the existing `force_full_resync` permission + `DriftChecker::ENTITY_CHECKS` map; NO migration. Nav child "Manual Sync" between Drift and Customers (17 children). 10/10 smoke + 6 nav-assert smokes bumped 16→17. 4 D-QBO-26-* locked. **Phase QBO-12 COMPLETE (3/3): detection (S-QBO-24) + resolution (S-QBO-25) + manual sync (S-QBO-26).**
>
> **S-QBO-25 / S-QBO-26 scope split (D-QBO-25-3 / D-QBO-26-*, both SHIPPED 2026-05-30):** the three bulk actions below split across two sessions. **S-QBO-25** owns the "Mark all drifts of category X" bulk-accept/bulk-suppress slice via `api/v1/quickbooks/drift_bulk_resolve.php` (transactional; 9-value category whitelist + 8-entity entity_type whitelist + action whitelist {accept,suppress}; per-event resolution_note carries bulk context; single audit_log summary). **S-QBO-26 (Manual Sync page)** owns the bulk SYNC actions: "Force full re-sync of entity type" + "Force pull from QBO" + SyncToken rewrite (all via `manual_sync.php`). The one sync action S-QBO-25 owns is the per-event "Resolve via FF re-sync" (single-row convenience routing to the entity's Enqueuer — see §15.4).

In addition to per-event resolutions (§15.4), operators can trigger bulk reconciliation:

- **"Force full re-sync of entity type"** *(S-QBO-26 ✅ SHIPPED)* — re-enqueues a `create` push for every mapped entity of that type (all 8 push types) via its Enqueuer. Used after a known data issue or after the accountant manually edited many QBO records. Ineligible rows (wrong status, webhook-originated payments, bridge-derived JEs) are skipped by the Enqueuer's own gates; nothing is pushed while the master sync switch is off (D-QBO-26-1 / D-QBO-26-4).
- **"Force pull from QBO"** *(S-QBO-26 ✅ SHIPPED)* — pulls the current state of every mapped reference entity from QBO and refreshes the SyncToken (used after realm migration or after a long disconnection). Available for the 5 pull-capable reference types (customer/vendor/account/item/tax_code); transactional/push-only types are inbound-migrated via the historical pull (S-QBO-27) — D-QBO-26-2. A separate **"Reset SyncToken"** action NULLs the token for an entity type as the realm-migration recovery primitive (D-QBO-26-3).
