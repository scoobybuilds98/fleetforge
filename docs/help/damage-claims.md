...
When a claim becomes **Invoiced** with an invoice linked, FleetForge records the damage-recovery accounting entry automatically. When a claim is **Written Off**, the remaining balance of its invoice is written off as bad debt: the invoice is closed, the customer's balance goes down, and the write-off entry is recorded. When QuickBooks is connected, the invoice is closed there too (a credit memo on the Bad Debt Write-off item, applied to the invoice). A claim with no invoice — or a draft, paid or already written-off one — only changes status.
...
- **Billing the customer** — moving a claim to `invoiced` while `invoice_id` is set fires `AutoEntryBridge::onDamageRecoveryBilled`, recording the `damage_recovery` journal entry; moving to `written_off` writes off the linked invoice's remaining balance through `InvoiceWriteOff` INSIDE the status-update transaction (the `damage_writeoff` entry via `onDamageWrittenOff`, an `acc_bad_debt_writeoffs` row, invoice → `written_off` with balance 0, `customers.outstanding_balance` reduced), then queues `InvoiceWriteoffEnqueuer` — so a write-off that can't be posted (e.g. Bad Debt Expense not configured) returns `WRITE_OFF_FAILED` and nothing changes. Only sent / overdue / partially-paid invoices with a balance are written off. The recovery (`invoiced`) entry still runs outside the transaction: an accounting hiccup there logs an error rather than rolling back the status change; with no linked invoice it is skipped and logged.
