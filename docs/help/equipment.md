---
description: Register and manage your fleet of trucks and trailers — specs, compliance dates, health, live tracking, and reusable equipment types.
---
...
Below the tiles, each row shows the unit's **Unit #**, **Equipment Type**, **Year**, **Status**, **Yard**, **Mileage**, and **Compliance** (shows **Expiring** when any expiry date is within 30 days or already past, otherwise **OK**).

## Adding a new unit

1. Click **+ New Unit** in the top right of the Equipment list.
2. Under **Unit Identity**, pick an **Equipment Type** (required), enter a unique **Unit Number** (required), and choose an **Ownership** type — Owned, Leased, or Brokered (required). VIN and Year are optional. Selecting an equipment type pre-fills dimensions, ownership, tracking, and compliance-interval defaults, which you can override.
...
3. Use the **All Equipment Types** dropdown to show only one equipment type.
4. Use the sort dropdown to order by **Unit #**, **Equipment Type**, **Status**, **Year**, **Date acquired**, **Date added**, **Mileage**, or **CVI expiry**, and the direction dropdown for ascending/descending. You can also click the **Unit #** or **Status** column headers to sort.
...
You can change a unit's **Equipment Type** here — existing leases and their rates are unaffected; new leases use the new type's rates. **Status is not editable from this form** — see below for how statuses change.
...
Equipment types are reusable presets (for example "53ft Dry Van"). Every unit is created from an equipment type, and the type supplies default dimensions, rental rates, and compliance intervals.

1. From the Equipment list, click **Equipment Type** (top right) to open the **Equipment Types** page, then **+ Add new equipment type**.
2. Under **Equipment Type Identity**, enter a unique **Equipment Type Name** and pick a **Category** (Chassis, Dry Van, Reefer, …; categories are managed under **Manage Categories**). Model and Description are optional — the brand/make is recorded on each unit, not on the type.
...
4. Under **Default Rental Rates**, set currency, mileage unit, and daily/weekly/monthly/mileage rates. Set the mileage rate to **0** to disable mileage billing for the equipment type.
5. Under **Compliance Renewal Intervals**, set CVI, MVI, registration, and insurance intervals in days.
6. Click **Create Equipment Type**.

On the Equipment Types list, the **Units** column links to the units of each type, and the **Status** badge shows **Active** or **Inactive**. Use **Edit** to change an equipment type; **Delete** is disabled while any units still use it.
...
- **Equipment types = templates in data** — the UI says "Equipment Type" but the table is still `equipment_templates` (URL `/equipment/templates`); units join it on `template_id`. A type can't be deleted while `unit_count > 0`. Type defaults are copied into the unit at creation and can be overridden per unit. The unit's type can be changed later; lease snapshots and frozen rates are unaffected.
