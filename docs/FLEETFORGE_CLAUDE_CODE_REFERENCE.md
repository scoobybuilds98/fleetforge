...
| D-SAMSARA-DELETE-1 | **Unit delete is one-directional — does NOT propagate to Samsara** (locked 2026-06-05 via S-SAMSARA-DELETE-DECOUPLE) | Neither `api/v1/equipment/units/delete.php` (single soft-delete) nor `api/v1/equipment/units/bulk_delete.php` (bulk soft-delete) may call `SamsaraClient::deleteTrailer()`. Removing a unit in FleetForge — e.g. a trailer we don't want to import/track — must leave the real Samsara asset untouched, so FF housekeeping can never destroy live fleet data we don't own the lifecycle of. **Intentional decoupling has a dedicated path:** `api/v1/samsara/unlink.php`, which NULLs the FF↔Samsara link without deleting either record. The create-time auto-link (`create.php`) and update-time sync (`update.php`) are RETAINED. `SamsaraClient::deleteTrailer()` is left in place (now zero callers) rather than deleted, to keep the change scoped to the two delete endpoints. Reverses the original SAMSARA-3 behavior (delete→Samsara-delete in both paths). Full rationale + carry-forward in `FLEETFORGE_PROGRESS.md` DECISIONS D-SAMSARA-DELETE-1. |
...
### Trap 76: Alpine `$refs` / `$nextTick` inside a method belong to the CALLING element — call it from inside an x-if'd block that closes itself and they silently no-op

**Symptom (S-CHAT-REBUILD, 2026-09-24):** after starting a conversation from the New-message dialog, the composer never got focus and the first keystrokes were lost. The same `open()` method focused fine when called from the inbox list or from `Alpine.$data(root)` in the console.
**Why:** Alpine binds magics (`$refs`, `$nextTick`, `$el`) to the element whose expression invoked the method, not to the component root. The handler ran from a button inside `<template x-if="newer.open">`; its first line closed the dialog, which destroyed that button. Every later `this.$refs.input` / `this.$nextTick(...)` in the awaited chain resolved against a torn-down element and did nothing, with no error.
**Rule:** a component method that can be called from inside an x-if/x-for element (especially one it removes) must not rely on magics after its first `await` or state change. Capture `this._root = this.$el` in `init()`, resolve refs with `this._root.querySelector('[x-ref="…"]')`, and defer with `setTimeout(fn, 0)` instead of `$nextTick`. Reference: `public/assets/js/chat.js` `_ref()` / `_after()`.
**Test:** use a REAL click on the in-dialog button, then check `document.activeElement`. A console call through `Alpine.$data(root)` binds magics to the root and hides the bug.

### Trap 77: Alpine boolean-attribute bindings to a MISSING key can leave the attribute ON — coerce with `!!`

**Symptom (S-ATTENTION-INBOX, 2026-09-25):** every Take it / Snooze / Done button in the new bell rendered `disabled` although `busy` was `{}` and `Alpine.evaluate(btn, 'busy[it.id]')` returned `undefined` (Alpine 3.15.12, buttons inside nested `x-for` templates). Clicking did nothing and no request was sent — the UI looked fine and silently didn't work.
...
**Billing module (S-BILLING-MODULE, D-BILLING-MODULE-2):** no module key of its own — `/billing` reuses `invoices`: view = cycles; create = open cycle / readings / generate; edit = send / review marks / holds / acknowledge / close; approve = reopen + approve runs; export = billing register (also needs financials). Every `api/v1/billing/*` read redacts amounts without `can_view_financials()`; the approval page and the register require it. Billing → Settings writes need `settings_general:edit`.
...
```sh
php tests/_smoke_migrations_reproduce_master.php   # D-GUARD-1 keystone (genuine from-zero rebuild)
...
- **D-GUARD-1 (`_smoke_migrations_reproduce_master.php`)** — upgraded by S-SCHEMA-BASELINE (2026-06-06) to GENUINE from-zero rebuild: SC1 (scratch DB → apply all `db_migrations/*.sql` sorted → dump → normalize → diff vs master → 0 lines = CLEAN); SC2 negative (phantom `_sc2_phantom_table_guard_test` appended to master COPY → diff NON-EMPTY = keystone catches tables-in-master-with-no-delta); SC3/SC4 (master-loaded scratch DB → apply 000_baseline.sql → diff → CLEAN = proves baseline is a genuine no-op on a full-schema DB). Made possible by `db_migrations/000_baseline.sql` (all 156 tables as `CREATE TABLE IF NOT EXISTS`), which is what makes the from-zero invariant achievable. The prior static-parse subset-relation check (S-SCHEMA-GUARD-1) is superseded. Pre-baseline deltas are archived as `.sql.txt` (Runner skips them; `verify()` treats `.sql.txt` sibling as "archived ok").
...
- **Go-forward rule (D-BASELINE-6):** new schema changes → new `db_migrations/YYYYMMDDHHII_label.sql` delta + update master. `000_baseline.sql` is a fixed 2026-06-06 snapshot, NEVER updated. **Prod reconcile (operator step):** execute `docs/runbooks/baseline_reconcile.md` once on the first deploy that includes the baseline (pre-mark INSERT → `--apply` no-op → health gate). See S-SCHEMA-BASELINE SESSION LOG row.

### Resilience complement — a missing schema table must never fatal auth (S-AUTH-FAILOPEN-1, D-FAILOPEN-1/2/3)

The guards above stop schema drift from SHIPPING; this is the runtime safety net for when it slips through anyway. The 2026-06-05 prod 500 was `_ff_load_role_overrides()` throwing an uncaught `PDOException` on a missing `role_permission_overrides` table on every session start — one missing CONFIG table took down ALL auth. **Rule: permission/override-CONFIG reads in the session-bootstrap path fail OPEN to `config/permissions.php` factory defaults (catch `\Throwable` → `error_log` + `Sentry::captureException` → `return []`); auth-IDENTITY reads (session/remember-me validity, user-active) fail CLOSED (→ unauthenticated, never silently authenticated).** The two override loaders in `includes/auth.php` (`_ff_load_role_overrides` + `_ff_load_user_overrides`) are wrapped per this rule; super_admin stays always-true; every trip is LOUD (alarmed, never silent) so the root cause is fixed not masked. Mirror `Mailer::isEmailDisabled()` for the shape. Guarded by `tests/_smoke_auth_failopen.php` (in the precommit gate). SECURITY NOTE: during the degraded window DENY overrides are not enforced — availability-favoring + temporary, operator-approved. **Still queued (operational half):** atomic/blue-green deploy + a pre-cutover health migrate-assert (apply-then-verify before traffic) so a schema lag is caught at deploy, not at first request.
...
- D131 — extension: parity + invariants + doc freshness + qbo client + qbo queue + qbo admin UI + oauth state + qbo customer mapping + qbo customer push + qbo vendor mapping + qbo vendor push + settings roundtrip + settings form keys + settings endpoints smokes all run pre-commit (S-BILLING-RATE-FIX 2026-05-06 added the parity + invariants pair; S-DOC-FRESHNESS-DISCIPLINE 2026-05-13 added `tests/_smoke_doc_freshness.php`; **S-DOC-FRESHNESS-EXPAND 2026-05-27 extended the smoke from 17 to 24 sub-checks** with CLASS 5/6/7/8/9 cross-validating canonical-doc completeness — recent SESSION LOG ↔ CURRENT_SESSIONS presence, recent QBO-relevant sessions ↔ QUICKBOOKS_PROGRESS presence, D-* attribution links advisory, QUICKBOOKS_PROGRESS "Next session up" freshness, K-22 Trap #50+ attribution; D131 smoke-file count unchanged at 22; D-DOC-FRESHNESS-EXPAND-1 locks the rule set + cutoffs; **S-QBO-13 2026-05-27** added `tests/_smoke_qbo_payment_webhook.php` — 20 sub-checks for QBO→FF payment webhook pull (Exception #1 per D-QBO-CORE-2): C1 class surfaces (QboWebhookSignature + PaymentWebhookHandler), C2 acc_qbo_payment_map schema (18 cols + 2 UNIQUE + 4 idx + FK ON DELETE SET NULL + pulled_from_qbo terminal state per D-QBO-13-2), C3 acc_qbo_webhook_events schema (10 cols + UNIQUE on webhook_event_id for idempotency), C4 payments.origin ENUM column + default 'ff_native', C5-9 QboWebhookSignature verify/compute (valid + invalid + empty-token-fail-closed + empty-sig-rejected + round-trip), C10 handle() wrong_realm skip, C11 already_mapped short-circuit, C12 createFfPaymentFromQbo happy path (FF payment + allocation + map + invoice counters + customer counter all updated atomically per D-QBO-13-4), C13 currency_mismatch (D18), C14 invoice_void, C15 overpayment split-allocation (S-FIX-2 Bug #2 pattern), C16 payment_number gap-free format PAY-YYYY-NNNNN (D15), C17 reference_number fallback to "QBO-{id}" per D-QBO-13-6, C18 invoice flips paid+0.00 when balance hits zero, C19 origin/push_status verification (D-QBO-13-1 + -2), C20 webhook endpoint file structure (lint + helper-class refs + 403 fail-closed + fastcgi_finish_request). **D131 gate bumped 23→24** (+qbo_payment_webhook). **S-QBO-14 2026-05-28** added `tests/_smoke_qbo_payment_push.php` — 20 sub-checks for the FF→QBO payment push flow (Phase QBO-6 / 2 of 3, pairs with S-QBO-13 webhook puller): C1 class surfaces (PaymentPusher + PaymentEnqueuer + RESULT_BASE const), C2 acc_qbo_payment_map schema verified (S-QBO-13 columns + origin/push_status ENUM values PaymentPusher writes — **no new migration this session**; reuses S-QBO-13 schema), C3 buildQboPayload happy-path multi_currency='1' (CustomerRef + TotalAmt + TxnDate + CurrencyRef='CAD' + ExchangeRate='1.0' + DepositToAccountRef from critical_category='undeposited_funds' + PaymentRefNum + Line[].LinkedTxn[{TxnId, TxnType='Invoice'}] per D-QBO-14-3), C4 multi_currency='0' CurrencyRef + ExchangeRate absent (D-QBO-FIXPACK-12), C5 throws on missing customer mapping, C6 throws on missing linked-invoice mapping (D-QBO-14-3), C7 throws on missing UndepositedFunds account (D-QBO-14-4), C8 PaymentRefNum from reference_number (omitted when empty per D-QBO-14-6), C9 sync_mode='disabled' → skipped_by_mode + map row + non-HTTP sync_log, **C10 skipped_non_ff_origin — webhook-originated payment not re-pushed; NO map row written (D-QBO-14-1 invariant; closes D-QBO-13-1/2 loop at dispatch layer; mirrors BillPusher skipped_unmapped_void no-map-row D-QBO-18-4 pattern to avoid needing a 'skipped_non_ff_origin' ENUM value — no migration this session)**, C11 status='pending' → failed_preflight (D-QBO-14-2), C12 already_mapped → outcome='created', C13 missing FF payment → ff_not_found, C14 defensive skip when map row push_status='pulled_from_qbo' (defense-in-depth), C15 PaymentEnqueuer gate-0 rejects missing payment, **C16 PaymentEnqueuer gate-0 rejects origin='qbo_payments_webhook' (CRITICAL D-QBO-14-1 at enqueue layer)**, C17 gate-0 rejects status='pending' (D-QBO-14-2), C18 gate-3 rejects 'update' op (v1 allowlist per D-QBO-14-5), C19 happy-path queue insert (entity_type='payment', op='create', status='queued'), C20 pushUpdate stub → unsupported_in_session pointing to S-QBO-14-UPDATE-FOLLOWUP. Self-cleaning via id=999990-999999 sentinels for payments+allocations+customers+invoices+accounts+maps + settings snapshot/restore for sync_enabled/sync_mode.payment/multi_currency_enabled/tax_override_code_id/connection_status/realm_id. **Also updated `tests/_smoke_qbo_queue.php`** C8 to assert hasImplementation TRUE for customer/vendor/invoice/bill/payment (S-QBO-6/7/11/18/14 shipped — closes the S-QBO-18 gap that never updated this list for bill) + FALSE for credit_memo/journal_entry/item; C9 worker pusher_not_implemented test entity swapped from 'payment' → 'credit_memo' (next-up unbuilt Pusher; S-QBO-15 future). **D131 gate bumped 24→25** (+qbo_payment_push). **S-QBO-15 2026-05-29** added `tests/_smoke_qbo_payments_embed.php` — 28 sub-checks for the customer-portal QBO Payments embed (Phase QBO-6 / 3 of 3, COMPLETES PHASE QBO-6): C1 class surfaces (PaymentInitiator + RESULT_BASE + QboClient::generatePaymentsHostedUrl), C2 acc_qbo_payment_initiations schema (17 cols + UNIQUE on initiation_token + 5 indexes + 2 FKs ON DELETE CASCADE/SET NULL + 5 status ENUM states), C3 3 settings seeded (payments.success_url + cancel_url + url_ttl_minutes), C4-C10 7 preflight gate scenarios (feature_disabled when payments_enabled='0' / not_connected when connection_status not connected / invoice_not_found / unauthorized when portal user doesn't own customer per Trap-8 / invoice_not_payable when status='paid' / invoice_no_balance when balance_due=0 / invoice_not_synced when no QBO mapping), C11 idempotency D-QBO-15-5 (existing pending+unexpired returns same URL), C12 expired pending row marked status='expired' before re-gen, C13-C15 matchByQboInvoice (happy path marks completed+sets qbo_payment_id / no-match returns null / picks LATEST pending when multiple per D-QBO-15-1), C16 findByToken happy + invalid hex format rejects, C17 markCancelled idempotent (already cancelled = no-op), C18 PaymentWebhookHandler extension via source scan (PaymentInitiator::matchByQboInvoice call + initiation_handshook + initiation_id annotations), C19 persistFailedRow helper present for operator-visible QBO API errors, C20 initiate_qbo_payment.php structure (portal auth + PaymentInitiator::generate + lint), C21 portal/payments/status.php (portal auth + Trap-8 portal_customer_id check + findByToken), C22 payment_success.php (Alpine paymentSuccessPoller factory + 30s timeout per D-QBO-15-6 race-handling + pending/completed/timed_out state UI), C23 payment_cancel.php (markCancelled + portal auth), C24 portal view Pay Online button + 4-gate visibility ($showPayOnline + payments_enabled gate + initiate_qbo_payment endpoint + payOnline() Alpine method), C25 admin /quickbooks/payments/initiations endpoint (permission gate + acc_qbo_payment_initiations table reference + live_expired computed column + lint), C26 admin payments.php initiations sub-view (qboPaymentInitiationsAdmin Alpine factory + "Payment Initiations" header + initiations endpoint reference), C27 expire-before-insert invariant via source scan (status='expired' UPDATE + WHERE ff_invoice_id=? AND status='pending' lookup), C28 PaymentWebhookHandler graceful no-match ($initiationMatch !== null conditional annotation; existing flow unchanged when no initiation row). Self-cleaning via id=999990-999999 sentinels for invoices+customers+portal_users+maps+initiations + settings snapshot/restore for payments_enabled/connection_status/multi_currency_enabled/url_ttl_minutes. **K-22 catches**: (1) `schema_migrations` columns are `version`+`filename` (not `migration_name`) — DDL auto-commits before INSERT meant the table+settings landed partially on first failed apply; fix removed manual INSERT (migrate.php auto-records); idempotent CREATE TABLE IF NOT EXISTS made re-apply clean. (2) C15 LATEST-wins test had stale C13 row competing on generated_at ordering; fixed via DELETE-before-insert in test setup. **D131 gate bumped 25→26** (+qbo_payments_embed). **S-QBO-19 2026-05-29** added `tests/_smoke_qbo_bill_payment_push.php` — 25 sub-checks for FF→QBO bill payment push (Phase QBO-8 / 2 of 2; COMPLETES PHASE QBO-8): C1 BillPaymentPusher + BillPaymentEnqueuer class surfaces + RESULT_BASE; C2 acc_qbo_bill_payment_map schema (19 cols + 2 UNIQUE + 2 idx + FK CASCADE on acc_ap_payments); C3 buildQboPayload Check pay_type happy-path (VendorRef + Check.BankAccountRef + LinkedTxn TxnType='Bill'); C4 CreditCard pay_type → CreditCardPayment.CCAccountRef (NOT CheckPayment) per D-QBO-19-2; C5 6-method PayType mapping per D-QBO-19-2; C6 multi_currency gating both directions per D-QBO-FIXPACK-12; C7 throws on missing vendor mapping; C8 throws on missing bank chain (BOTH modes: missing bank_accounts row + unmapped gl_account_id) per D-QBO-19-3 chain lookup; C9 throws on missing per-allocation bill mapping per D-QBO-19-4; C10 throws on no allocations; C11 sync_mode='disabled' → skipped_by_mode; C12 status='void' + no mapping → skipped_unmapped_void NO map row (D-QBO-18-4 mirror); C13 status='void' + mapping → skipped_voided; C14 status='pending' → failed_preflight (D-QBO-19-1); C15 already_mapped idempotency; C16 ff_not_found; C17 pushUpdate stub → S-QBO-19-UPDATE-FOLLOWUP (D-QBO-19-5); C18 gate 7 typed failed_preflight_field_too_long (D-QBO-19-7); C19 gate 8 SKIPS when multi_currency='0' (D-QBO-FIXPACK-12); C20-C23 Enqueuer gate-0/gate-3/happy-path; C24 PrivateNote includes payment_number+reference+check_number; C25 BankAccountRef D-QBO-19-3 chain verification. **K-22 catch documented**: ap_payment.bank_account_id is FK to acc_bank_accounts.id (NOT acc_accounts.id) — initial smoke run failed with FK constraint; fixed by seeding acc_bank_accounts row with gl_account_id pivot + Pusher does 3-step chain lookup (bank_account_id → bank_accounts.id → gl_account_id → qbo_account_map). **PER OPERATOR ASK "test extensively everything u build, including stuff already built in this session"** also added 2 coverage-gap smokes: (a) **`tests/_smoke_qbo_admin_retry_endpoints.php`** (NEW 20 sub-checks) — cross-cutting behavioral coverage for ALL 4 retry endpoints (invoices + bills + payments + bill_payments) closing the gap that S-QBO-PAYMENT-SYNC-UI + S-QBO-14 + S-QBO-18 + S-QBO-19 all shipped retry endpoints without dedicated tests: exist+lint+POST+permission+audit_log structure (C1-C4), per-endpoint structure (C5-C8), gate-refusal reason ladder verification incl. payments cites origin per D-QBO-14-1 + bills cites sync_enabled/sync_mode (C9-C11), 422/404 error handling (C12-C13), payments 409 INVALID_STATE on non-ff_native origin admin layer (C14), bill_payments handles all 4 typed states (C15), audit_log entity_type (C16), no silent error swallow (C17), gate-refusal reasons surfaced (C18), Enqueuer correct wiring + Pusher bypass prevention (C19-C20); (b) **EXTENDED `tests/_smoke_qbo_payments_embed.php` 28→31** with 3 NEW integration tests for S-QBO-15: C29 (full webhook handshake against real pending row — closes gap from C18/C28 source-scan-only), C30 (preflight failure no orphan), C31 (full portal flow simulation: pending → webhook → completed per D-QBO-15-6). **CLASS 12 mapping table updated** in `_smoke_doc_freshness.php` — BillPaymentPusher → bill_payments.php now in hardcoded map; CLASS 12 confirms 6/6 Pushers have matching admin UI surfaces (CustomerPusher / VendorPusher / InvoicePusher / BillPusher / PaymentPusher / BillPaymentPusher); negative case still verified. **6 nav-asserting smokes updated 13→14** via bulk sed (account/customer/vendor/tax_code/item mapping + admin_ui). **NEW table** acc_qbo_bill_payment_map (19 cols + 2 UNIQUE + 2 indexes + FK CASCADE); migrate 72→73/0/0; SCHEMA_QUICK_REF regen 143→**144 tables / 2273→2292 cols**. **D131 gate bumped 26→28** (+qbo_bill_payment_push + qbo_admin_retry_endpoints). **7 decisions locked**: D-QBO-19-1 (FF-origin only; mirrors D-QBO-18-6), D-QBO-19-2 (PayType mapping FF payment_method → QBO PayType: check/eft/wire→Check, credit_card→CreditCard, cash/other→Check fallback), D-QBO-19-3 (BankAccountRef via 3-step chain lookup through gl_account_id pivot; K-22 catch documented), D-QBO-19-4 (LinkedTxn.TxnType='Bill' per allocation; bills must be S-QBO-18-pushed first), D-QBO-19-5 (pushUpdate stubbed → S-QBO-19-UPDATE-FOLLOWUP), D-QBO-19-6 (currency-mismatch gate 8 mirrors D-QBO-BILL-GOTCHAS-1 + D-QBO-14-7), D-QBO-19-7 (reference_number 21-char gate via QboFieldLimits::PAYMENT_REF_NUM_MAX shared with PaymentPusher per D-QBO-FIXPACK-4 single-source). **NEW table** acc_qbo_payment_initiations (17 cols + UNIQUE + 5 indexes + 2 FKs); migrate 71→72/0/0; SCHEMA_QUICK_REF regen 140→**143 tables / 2227→2273 cols**. **5 decisions locked**: D-QBO-15-1 (match by qbo_invoice_id + latest pending; UNIQUE-pending via app-layer expire-before-insert because MySQL lacks partial unique indexes), D-QBO-15-2 (QuickBooksClient::generatePaymentsHostedUrl abstracts Intuit Payments API; separate base URL with save/restore; operator live-test note for endpoint verification), D-QBO-15-3 (portal 4-gate visibility + 7 server-side preflight gates incl. Trap-8 portal user ownership + currency-mismatch when multi_currency='1' mirrors D-QBO-14-7), D-QBO-15-4 (PaymentWebhookHandler step 6.5 calls matchByQboInvoice BEFORE existing flow; graceful null-match preserves backward compat with S-QBO-13), D-QBO-15-5 (idempotency: existing pending+unexpired returns same URL; expired → marked + new URL). **S-QBO-PAYMENT-SYNC-UI 2026-05-29** added `/quickbooks/payments` admin surface (`app/admin/quickbooks/payments.php` + `api/v1/quickbooks/payments/list.php` + `api/v1/quickbooks/payments/retry.php`) mirroring `/quickbooks/bills` for the bidirectional payment sync surface. Debt-paydown shipped same-morning after operator escalation: "why are you forgetting about stuff?" (S-QBO-14 backend shipped without UI; same structural gap as S-QBO-18 → S-QBO-BILL-SYNC-UI). 8 KPI tiles + status + origin dual filter + 9-column paginated table with `Origin` badge column + retry button DISABLED for non-ff_native rows (D-QBO-14-1 enforced at UI layer). Retry endpoint enforces D-QBO-14-1 + D-QBO-13-2 invariants with TRIPLE guard (origin check + push_status check + retryable status whitelist). Nav +Payments child between Bills and Settings (12→13 children). 6 nav-asserting smokes bumped from 12→13 via bulk sed (account/customer/vendor/tax_code/item mapping + admin_ui — count + expectedOrder array + PASS message). **NEW DISCIPLINE LAYER — CLASS 12 in `tests/_smoke_doc_freshness.php`** strict-fails when any `lib/QboPushers/*Pusher.php` glob match lacks matching `app/admin/quickbooks/{entity}.php` surface. Hardcoded mapping table for 5 shipped Pushers (CustomerPusher→customers.php, VendorPusher→vendors.php, InvoicePusher→invoices.php, BillPusher→bills.php, PaymentPusher→payments.php); 4 future Pushers TODO-commented for next session to update. Verified negative case (moving payments.php aside makes CLASS 12 strict-fail with actionable message). doc_freshness extended 27→**28/28** with the new class. **NEW MEMORY FILE `memory/feedback_ui_completeness_with_backend.md`** codifies the discipline (rule + canonical incidents + how-to-apply checklist for PART A pre-flight + 6-step UI-shipping template + CLASS 12 enforcement reference + anti-patterns observed); MEMORY.md index updated with new bullet pointing to memory + bumped doc_freshness class count (11→12) in canonical-doc bullet. **D131 smoke-file count unchanged at 25** (no new smoke; existing file extended). **No schema motion** — migrate held at 71/0/0. **2 decisions locked**: D-UI-COMPLETENESS-1 (every QBO Pusher MUST ship matching admin UI same commit; enforced by CLASS 12), D-QBO-PAYMENT-SYNC-UI-1 (admin retry endpoint enforces bidirectional dedup invariant at admin layer; defense-in-depth quadruple-layer with Enqueuer gate 0 + Pusher pushImpl step 5 + retry endpoint + UI button gate). **K-22 catch documented**: acc_qbo_payment_map.push_status ENUM does NOT include 'skipped_non_ff_origin' — initial smoke run failed "Data truncated for column 'push_status'" because recordSkipped tried to write the new code; fix was adopting BillPusher skipped_unmapped_void no-map-row pattern (sync_log captures via error_code which has no ENUM constraint; no migration this session). **S-DOC-FRESHNESS-EXPAND-2 2026-05-27** extended `_smoke_doc_freshness.php` 24→27 sub-checks with CLASS 10/11a/11b: CLASS 10 reads last 30 git commit subjects via `git log --pretty=format:%s -30`, extracts S-XXX session labels, strict-fails when any committed session lacks PROGRESS.md presence (closes anchor blind spot CLASS 5/6 had — first run caught S-SES-DIAG which shipped 2 days earlier with zero docs); CLASS 11a + 11b advisory cross-presence for CLAUDE_CODE_REFERENCE.md + QBO_REMAINING_WORK_2026-05-26.md. New const `DOC_FRESHNESS_GIT_COMMIT_TAIL=30`. Also ships `scripts/precommit_doc_check.sh` (wraps doc_freshness + parity + migrate verify + visual mental-checklist) + `memory/feedback_canonical_doc_checklist.md` (survives context wipes). D-DOC-FRESHNESS-EXPAND-2-1/2/3 locked. D131 smoke file count unchanged at 23. **S-QBO-BILL-SYNC-UI 2026-05-27** added `/quickbooks/bills` admin surface (`app/admin/quickbooks/bills.php` + `api/v1/quickbooks/bills/list.php` + `api/v1/quickbooks/bills/retry.php`) mirroring `/quickbooks/invoices` for bills — 8 KPI tiles + filter + retry button + nav entry between Invoices and Settings. 6 nav-asserting smokes bumped from 11→12 QuickBooks children. No new smoke file (UI mirror; behavior verified via qbo_admin_ui C4 + the 5 mapping smokes' nav assertions). No schema change, no D-* decision lock (pattern-mirror session). **S-QBO-BILL-GOTCHAS-PAYDOWN 2026-05-27** extended `tests/_smoke_qbo_bill_push.php` from 20→23 sub-checks (C21 typed `failed_preflight_field_too_long` per D-QBO-BILL-GOTCHAS-2; C22 currency-mismatch gate SKIPS when multi_currency_enabled='0' per D-QBO-FIXPACK-12; C23 acc_qbo_bill_map.push_status ENUM verification — both new typed sub-states). Fixture extended with synthetic ap_clearing-tagged account (FF#999991 → A-9991, is_critical=1) so AccountValidator gate 2 passes in smoke regardless of live operator state. D131 smoke-file count unchanged at 23 (no new file; existing file extended). **S-QBO-18 2026-05-27 added `tests/_smoke_qbo_bill_push.php`** — 20 sub-checks for the FF→QBO bill push flow (acc_qbo_bill_map schema shape + BillPusher/BillEnqueuer class surfaces with RESULT_BASE const, buildQboPayload happy-path with multi_currency='1' emitting VendorRef + DocNumber + CurrencyRef + ExchangeRate + AccountBasedExpenseLineDetail per line + TxnTaxDetail.TotalTax + PrivateNote JSON, multi_currency='0' CurrencyRef omitted, DocNumber fallback to bill_number when vendor_bill_number empty per D-QBO-18-7, throws on missing vendor mapping / missing per-line account mapping / no lines, pushImpl all 6 outcome paths — skipped_by_mode + skipped_unmapped_void + skipped_voided + draft-failed_preflight + already_mapped + ff_not_found, pushUpdate stub returns unsupported_in_session pointing to S-QBO-19, BillEnqueuer gate-0 rejects missing/draft bill + gate-3 rejects 'update' op + happy-path queue insert; self-cleaning via id=999990+ sentinels for bills+vendors+accounts+mappings + settings snapshot/restore for sync_enabled/sync_mode.bill/multi_currency_enabled/tax_override_code_id/connection_status). **D131 gate bumped 22→23** for the new qbo_bill_push smoke file; S-QBO-2 2026-05-20 added `tests/_smoke_qbo_client.php`; S-QBO-3 2026-05-20 added `tests/_smoke_qbo_queue.php`; S-QBO-4 2026-05-20 added `tests/_smoke_qbo_admin_ui.php` for the QBO admin UI surface — 4 pages + 9 API endpoints lint + permission gates + nav structure + empty/synthetic reachability + CSRF/POST verification + dashboard_metrics JSON shape, self-cleaning; S-QBO-OAUTH-FIX 2026-05-20 added `tests/_smoke_oauth_state.php` — 9 sub-checks covering acc_oauth_states table shape + StateManager class surface + generate→verifyAndConsume round-trip + single-use + expiry + provider-mismatch + tamper + cleanup + callback.php auth-context-free structural assertion, self-cleaning; S-QBO-5 2026-05-20 added `tests/_smoke_qbo_customer_mapping.php` — 12 sub-checks covering acc_qbo_customer_map shape + CustomerPuller/CustomerMatcher surfaces + normalizeName collapses corporate suffixes + findBestMatch cascade (exact/Levenshtein/null) + UNIQUE NULL semantics + 4 endpoint linting + customers.php page lint + nav has 7 children incl. Customers (count bumped to 7 in S-QBO-7) + normalize() against representative QBO JSON, self-cleaning via sentinel mapping ids; S-QBO-6 2026-05-21 added `tests/_smoke_qbo_customer_push.php` — 10 sub-checks covering CustomerPusher + CustomerEnqueuer class surfaces + buildQboPayload full/minimal cases + pushCreate sync-mode gate + soft-delete skip + idempotency on already-mapped + Enqueuer master kill / mode kill / happy-path queue insert, self-cleaning via id=999990+ sentinels + settings save+restore; S-QBO-7 2026-05-21 added `tests/_smoke_qbo_vendor_mapping.php` — 12 sub-checks mirroring the customer mapping smoke for vendors (acc_qbo_vendor_map shape with 21 cols + qbo_balance ABSENT + qbo_given_name/qbo_family_name/qbo_v4v_status PRESENT, VendorPuller/VendorMatcher surfaces, vendor-flavored matcher uses vendors.name not company_name) + `tests/_smoke_qbo_vendor_push.php` — 10 sub-checks mirroring the customer push smoke for vendors (VendorPusher + VendorEnqueuer surfaces, buildQboPayload with contact_name split per D-QBO-7-3 + Country='CA' default + no postal_code, single-name contact → GivenName only, gating + idempotency + queue happy-path with entity_type='vendor', self-cleaning via id=999990+ sentinels + vendor_type='other' on synthetic inserts to satisfy NOT NULL ENUM); S-QBO-8 2026-05-21 added `tests/_smoke_qbo_account_mapping.php` — 17 sub-checks for the chart of accounts mapping flow (acc_qbo_account_map shape with 22 cols + is_critical/critical_reason + idx_critical lookup index, AccountPuller/AccountMatcher/AccountValidator class surfaces, normalizeAccountName collapses casing+punctuation variants, exact_code wins over name match, type-compatibility gating prevents cross-type matches per D-QBO-8-3, ChartOfAccountsIncompleteException extends QuickBooksException, markCriticalAccounts flags live AR account, unmappedCritical returns the 7 live bridge accounts identified in pre-flight (codes 1030/1050/1060/2010/2030/2040/4122), assertReadyForInvoicePush throws+passes correctly, 4 endpoint linting, accounts.php Alpine factory + view gate, nav has 8 children incl. Accounts; defensive self-cleaning via 'TEST-SMOKE-A-*' qbo_account_id prefix + post-test markCriticalAccounts re-run — later bumped to 19 sub-checks in S-QBO-8 follow-up F1 fix adding C18 link-preserves-is_critical + C19 unlink-moves-flag); S-QBO-9 2026-05-21 added `tests/_smoke_qbo_tax_code_mapping.php` — 14 sub-checks for the tax code mapping flow (acc_qbo_tax_code_map shape with 22 cols + is_override_target NULLABLE + uq_ff_tax_rate/uq_qbo_tax_code/uq_override_target indexes verified, TaxCodePuller/TaxCodeMatcher class surfaces, identifyOverrideTarget('NON') case-insensitive + whitespace-tolerant per D-QBO-9-2, exact_name cascade case-insensitive, normalizeName collapses GST/HST variants, **uq_override_target UNIQUE-on-NULLABLE behavior verified** — multiple NULLs allowed + single 1 enforced + second-'1' UPDATE rejected with duplicate-key error confirming the D-QBO-9-2 design works as intended, set_override_target transactional clear-then-set flips flag correctly without UNIQUE violation, 4 endpoint linting, tax_codes.php Alpine factory + view gate, nav has 9 children incl. Tax Codes; defensive self-cleaning via 'TEST-SMOKE-T-*' qbo_tax_code_id prefix + override-target snapshot/restore + settings.quickbooks.tax_override_code_id snapshot/restore so D-CPA-5 invariant holds across runs); S-QBO-10 2026-05-21 added `tests/_smoke_qbo_item_mapping.php` — 18 sub-checks for the item / product mapping flow (acc_qbo_item_map shape with 24 cols + uq_ff_item_type_variant multi-column UNIQUE + uq_qbo_item + idx_status/idx_ff_item_type/idx_last_synced indexes verified, ItemPuller/ItemMatcher/ItemCreator class surfaces, **ffItemTypes() introspects INFORMATION_SCHEMA — not hardcoded — and yields 18 (item_type, variant) tuples for the 17 ENUM values + 2 GPS variants** per D-QBO-10-2, displayNameFor mapping correctness for representative tuples incl. GPS net/gross variants and Rental Reconciliation Credit per D-QBO-10-1, findBestMatch exact_name cascade + null-when-no-signal, **uq_ff_item_type_variant accepts (gps,NULL)+(gps,'net')+(gps,'gross') coexistence + duplicate-variant rejection** confirming multi-column UNIQUE works for variant routing, matchAll emits mapped + qbo_only decisions correctly, ItemPuller::normalize HTTP-free (pure normalization), **resolveIncomeAccount happy-path returns mapped revenue + throws ChartOfAccountsIncompleteException when no revenue mapped** with snapshot/restore around the test mutation, 5 endpoint linting (pull+auto_match+save_mapping+create_qbo_item+list), items.php Alpine factory + view gate, nav has 10 children incl. Items between Tax Codes and Settings, ItemPuller::normalize against representative QBO Item JSON, UI_CATEGORIES covers all 17 ENUM values exactly once; self-cleaning via 'X-SMOKE-GPS' sentinel ff_item_type + post-test deletion + revenue-mapping backup/restore loop to leave acc_qbo_account_map untouched after run); S-QBO-11 2026-05-24 added `tests/_smoke_qbo_invoice_push.php` — 29 sub-checks for the FF→QBO invoice push flow (acc_qbo_invoice_map shape with 18 cols + 5 indexes + FK to invoices ON DELETE CASCADE, all 5 lib class surfaces (InvoicePusher / InvoiceEnqueuer / InvoiceLineBuilder / InvoiceTaxOverride / InvoicePreflightGate), php -l clean on all 5 files, InvoiceTaxOverride bcmath sum + throw on empty + line-level structure, InvoiceLineBuilder emits Lines in sort_order + resolves QBO Item + GPS net/gross variant per customer.gps_revenue_presentation + throws on unmapped item_type + throws on period_independent invoice with recon_credit line (D-QBO-11-5 integrity guard — compiles but never triggers until invoices.engine_version column ships), InvoicePreflightGate 4 reason variants (validator failure / customer-unmapped / tax_override missing / connection disconnected), InvoicePusher skipped_by_mode + skipped_voided + failed_preflight (mapping table updated) + idempotency already_mapped on existing qbo_invoice_id, InvoiceEnqueuer sync_enabled=0 → false + 'update' op → false (S-QBO-12 deferred), PrivateNote JSON includes ff_invoice_id+engine_version+ff_tax_breakdown+audit + omits audit when columns NULL + engine_version='unknown' fallback, buildQboPayload CAD omits CurrencyRef/ExchangeRate + USD with exchange_rate_to_cad=1.35 emits both; self-cleaning via id=999990+ sentinels for invoices+customers+mappings + settings snapshot/restore for connection_status/sync_enabled/sync_mode.invoice/tax_override_code_id); S-QBO-11-POSTVERIFY-FIXES 2026-05-25 added `tests/_smoke_settings_roundtrip.php` — 95 sub-checks for settings persistence round-trip (save+restore pattern across all settings groups including QBO / notifications / billing / cron / security / AI / currency; boolean unchecked semantics; secret masking — placeholder never overwrites real secret, empty string never overwrites, real new secret does overwrite; currency form validation — markup 0-20% accepted, outside rejected; snapshot restored at end), `tests/_smoke_settings_form_keys.php` — 24 sub-checks for per-form-keys selective-save semantics (sidecar field preserved when sibling group saves — ai.briefing_recipient_roles NOT clobbered by Budget Alert save; AI Core form with _form_keys preserves sidecar fields across 3 different sibling groups; backward-compat path: no _form_keys = full-group save fires correctly for notifications.smtp_host), `tests/_smoke_settings_endpoints.php` — 22 sub-checks for settings-adjacent API endpoints (system tab: cron run endpoint + cron toggle; intelligence tab: ai/test-connection, intelligence/briefing_history, intelligence/token_analytics, intelligence/ai_request_log, intelligence/briefing_audit_log, intelligence/brief_content, intelligence/test_briefing, intelligence/set_user_preferences, intelligence/set_opt_in, intelligence/set_snooze; email template list; all verified http=200 with correct response shape); **D131 gate bumped 19→22** (settings_roundtrip 95/95 + settings_form_keys 24/24 + settings_endpoints 22/22 all PASS). **S-OPERATOR-FOLLOWUPS-TRACKING 2026-05-29** extended `_smoke_doc_freshness.php` 28→29 sub-checks with CLASS 13 (advisory) — extracts "Last updated: YYYY-MM-DD" from OPERATOR_FOLLOWUPS.md + scans tail of SESSION LOG for follow-up keyword matches + flags orphan sessions where row mentions follow-ups but doc 'Last updated' predates the row's date. Advisory (always passes; surfaces orphan list in detail) rather than strict-fail because catalog reflects discretion. D-OPERATOR-FOLLOWUPS-TRACKING-1/-2 locked. **S-QBO-21 2026-05-29** added `tests/_smoke_qbo_journal_entry_push.php` — 31 sub-checks for FF→QBO journal entry push (Phase QBO-10 / 1 of 1; COMPLETES PHASE QBO-10): C1 JournalEntryPusher + JournalEntryEnqueuer class surfaces + RESULT_BASE; C2 acc_qbo_journal_entry_map schema (17 cols + 2 UNIQUE + 2 idx + FK CASCADE on acc_journal_entries); C3 buildQboPayload happy-path 2-line balanced JE (TxnDate + DocNumber + Line[0].JournalEntryLineDetail.PostingType='Debit' + AccountRef + Amount=abs(debit) + Line[1].PostingType='Credit'); C4 multi-line balanced (4 lines: 2 debit + 2 credit); C5 throws on missing per-line account mapping (D-QBO-21-3); C6 throws on unbalanced JE (defense-in-depth); **C7 skipped_bridge_derived for all 5 source_types per spec §8.10 verbatim filter list (invoice/payment/credit_note/ap_bill/ap_payment); NO map row written + sync_log captures via error_code='skipped_bridge_derived' (D-QBO-21-1 + no-map-row pattern mirroring PaymentPusher skipped_non_ff_origin)**; C8-C10 entry_status='draft'/'submitted'/'approved' → failed_preflight (spec §8.10 only 'posted' pushes); C11 sync_mode='disabled' → skipped_by_mode + map row + sync_log SKIP; C12 already_mapped idempotency; C13 ff_not_found; C14 pushUpdate stub → S-QBO-21-UPDATE-FOLLOWUP (D-QBO-21-5); C15 multi_currency='1' + CAD → CurrencyRef + ExchangeRate='1.0' (D-QBO-FIXPACK-12); C16 multi_currency='0' → both absent; C17 non-CAD JE missing exchange_rate → buildQboPayload throws; C18 gate 6 failed_preflight_field_too_long (entry_number > 21 chars; D-QBO-21-4 defense-in-depth — JE-YYYY-NNNNN=12 chars normally); **C19 gate 7 failed_preflight_currency_mismatch (multi_currency='1' + empty header currency forced via SET SESSION sql_mode bypass since ENUM constrains to CAD/USD; D-QBO-21-6)**; C20 PostingType mapping per D-QBO-21-2 (debit-line='Debit' + credit-line='Credit' + throws on both-populated + throws on both-zero); C21 PrivateNote includes entry_number + source=manual (D-QBO-21-7); C22 Enqueuer gate-0 rejects missing JE; C23 Enqueuer gate-0 rejects non-posted entry_status; **C24 Enqueuer gate-0 rejects all 5 bridge-derived source_types (defense-in-depth; D-QBO-21-1)**; C25 Enqueuer gate-3 rejects 'update' op (v1 allowlist; D-QBO-21-5); C26 Enqueuer happy-path queue insert; C27 AccountValidator::assertReadyForJournalEntryPush gate at preflight (D-QBO-VALIDATOR-3 tax_receivable + tax_payable); **C28 BRIDGE_DERIVED_SOURCE_TYPES constant alignment Pusher vs Enqueuer + matches spec §8.10 verbatim**; **C29 integration: Enqueuer queue insert → Pusher::pushCreate from queue row + map row written (worker-dispatch simulation)**; **C30 integration: bridge-derived JE auto-skip end-to-end (Enqueuer + Pusher both reject; no map row at either layer; sync_log SKIP at Pusher layer)**; C31 per-line AccountRef sourced from acc_qbo_account_map via line.account_id (D-QBO-21-3 explicit). **K-22 catches surfaced via AskUserQuestion at pre-flight**: (1) acc_journal_entries.source_type ENUM uses FF naming (ap_bill/credit_note/ap_payment), NOT QBO-style names from the prompt — operator confirmed spec §8.10 canonical 5-item list; (2) acc_journal_entries has TWO status columns (legacy `status` + AJE-workflow `entry_status`) — operator confirmed gating on `entry_status` per spec §8.10. **CLASS 12 mapping table updated** — JournalEntryPusher → journal_entries.php now in hardcoded map; CLASS 12 confirms **7/7 Pushers have matching admin UI surfaces** (CustomerPusher / VendorPusher / InvoicePusher / BillPusher / PaymentPusher / BillPaymentPusher / JournalEntryPusher). **6 nav-asserting smokes updated 14→15** via bulk sed. **NEW table** acc_qbo_journal_entry_map (17 cols + 2 UNIQUE + 2 indexes + FK CASCADE); migrate 73→74/0/0; SCHEMA_QUICK_REF regen 144→**145 tables / 2292→2309 cols**. **D131 gate bumped 28→29** (+qbo_journal_entry_push). **7 decisions locked**: D-QBO-21-1 (bridge-derived source_type filter per spec §8.10 canonical 5-item FF-ENUM list; defense-in-depth at BOTH Enqueuer gate-0 AND Pusher pushImpl step 4; no-map-row pattern mirrors PaymentPusher skipped_non_ff_origin), D-QBO-21-2 (PostingType debit→'Debit' / credit→'Credit' / Amount=abs(populated) + throws on both-populated + both-zero), D-QBO-21-3 (per-line AccountRef from acc_qbo_account_map lookup; throws at preflight + buildQboPayload defense-in-depth), D-QBO-21-4 (DocNumber from entry_number; shared QboFieldLimits::INVOICE_DOC_NUMBER_MAX=21 gate per D-QBO-FIXPACK-4 single-source), D-QBO-21-5 (pushUpdate stubbed → S-QBO-21-UPDATE-FOLLOWUP matching S-QBO-11/14/18/19 stub-then-implement pattern), D-QBO-21-6 (header currency from acc_journal_entries.currency + D-QBO-FIXPACK-12 multi_currency gating + gate 7 typed failed_preflight_currency_mismatch when missing/invalid), D-QBO-21-7 (PrivateNote includes entry_number + source attribution + truncated description + reversal pointer for QBO-side audit drill-down). **S-PORTAL-REQUEST-ROUTING 2026-05-29** added `tests/_smoke_portal_request_notifications.php` — 26 sub-checks for portal service request notification dispatcher + settings-based routing config: C1 class surfaces + REQUEST_TYPES alignment with portal create.php $validTypes (catches form-handler/notifier drift); C2 all 17 routing settings seeded (1 toggle + 16 routing entries for 8 buckets × 2 fields); C3 NotificationService TYPE_TO_CATEGORY includes 'service_request' prefix; C4 fallback to default bucket when type-specific routing empty (D-PORTAL-REQUEST-ROUTING-4); C5 role_slugs → user_ids resolution; C6 explicit user_ids resolution; C7 union dedup (K-22 fixture-vs-real-DB user adjustment: switched from set equality to presence + dedup-count check after real accountant users in DB caused false-fails); C8 deleted/inactive users filtered out at resolution + dispatch layer; C9 always_include_super_admin='1' adds super_admin to set regardless of routing (D-PORTAL-REQUEST-ROUTING-3 safety net); C10 always_include_super_admin='0' excludes super_admin from auto-add; C11 unknown request_type normalizes to 'general' rather than dropping; C12 resolveRoleSlugsToUserIds returns active users in given roles only; C13 filterActiveUsers excludes deleted/inactive/missing from explicit ID list; **C14 end-to-end notify() integration — inserts 1 row per recipient with correct type/category/entity_type/entity_id**; C15 title + message + url shape includes customer company + submitted-by name + subject + drill-down URL `/requests/view?id=N`; C16 severity heuristic — damage_report + early_return → 'warning'; others → 'info' (K-22 caught: entity_id filter alone matched stale QBO invoice notifications with entity_id=999991, entity_type='invoice', severity='critical'; added entity_type='service_request' qualifier); C17 missing request_id returns 0 + logs; C18 zero-recipients (empty routing both layers) returns 0 + logs; **C19 portal create.php hook position — verified between db_insert and redirect using strpos position comparison**; C20 settings UI source scan for Routing card + 8 type entries + safety-net toggle (K-22 caught: literal key strings don't appear in source because UI uses PHP-tag interpolation; switched to scanning `$portalReqTypes` map keys); C21 settings save handler preg_match branches for role_slugs + user_ids JSON-encoding; C22 best-effort never-throws on missing/invalid IDs (-1, 0, missing); C23 admin/requests/index.php exists + customers/view gate + portal_service_requests query + lint passes; C24 admin/requests/view.php exists + view + edit permission gates + respond form references; C25 api/v1/requests/respond.php exists + POST method gate + customers/edit + audit_log insert + resolved_at handling; C26 nav config has Service Requests entry under customers module pointing to /requests (router strips admin/ prefix). Fixtures use sentinel IDs 999990-999999 + 4 sentinel admin users (active super / deleted / inactive / active accountant). Self-cleaning via finally; settings snapshot/restore for 13 keys. **NEW migration** db_migrations/202605300000_S-PORTAL-REQUEST-ROUTING.sql seeds 17 settings keys (mirrors `ai.briefing_recipient_roles` JSON-array-in-settings convention per D-PORTAL-REQUEST-ROUTING-1); migrate 74→75/0/0. **D131 gate bumped 29→30** (+portal_request_notifications). **4 decisions locked**: D-PORTAL-REQUEST-ROUTING-1 (settings-keys storage; no new table; mirrors briefing_recipient_roles convention), D-PORTAL-REQUEST-ROUTING-2 (role_slugs + user_ids stored separately; union at dispatch; matches multi-checkbox UI shape), D-PORTAL-REQUEST-ROUTING-3 (always_include_super_admin='1' safety-net default to prevent silent-drop bug class), D-PORTAL-REQUEST-ROUTING-4 (default fallback bucket used when type-specific empty; seed default.role_slugs='["super_admin"]' for out-of-box behavior).
- D131 — S-QBO-FA-MAP 2026-06-01 (operator-directed) added `tests/_smoke_qbo_fixed_asset_map.php` (10 sub-checks, per-function): A surfaces+schema C1-C2; B resolveQboAccount C3; C buildReference all-mapped→synced + one-unmapped→pending C4-C5; D syncOne insert/idempotent/missing C6-C8; E sync() aggregate C9; F endpoint presence C10; +C11 integration (DriftChecker::runCheck refreshes the FA map — the engine is auto-called on every drift run as best-effort maintenance, emits no drift_events; cron summary gains FA-ref line; drift_check smoke 28/28 re-verified no-regression). Creates `acc_qbo_fixed_asset_map` (migrate 87→88; reverses S-QBO-22 D-QBO-22-3 "no FA table") + `lib/QboPushers/FixedAssetMapSync.php` populate engine + `api/v1/quickbooks/fixed_asset_map_sync.php` + Refresh button on /quickbooks/journal_entries. NOT a QBO push-map (no QBO FixedAsset entity §8.13) — per-asset reference row (QBO GL-account refs + cost/depr snapshots + sync_status). DATABASE_MASTER + SCHEMA_QUICK_REF synced; PARITY OK.
- D131 — S-QBO-24 2026-05-30 (Phase QBO-12 / 1 of 3 drift-detection cron — DETECTION half) added `tests/_smoke_qbo_drift_check.php` (15 sub-checks) with **per-function coverage discipline** per [[extensive-test-and-full-report]] memory: Module A surfaces+infra C1-C2; Module B tolerance()/qboEntityName()/liveModeAvailable() C3-C5; Module C checkPushFailed happy+negative C6-C7; Module D checkSnapshotAmountDrift beyond-tolerance/within-tolerance/aggregate-accumulation C8-C10; Module E recordDrift idempotency incl. NULL-entity_id keys-on-qbo_entity_id C11-C12; Module F runCheck disabled-toggle/snapshot-only/live-gate-decision C13-C15. K-22 during smoke build: runCheck(forceLive=true) would issue real QuickBooksClient::query HTTP with multi-minute retry backoff × 8 entities (no fixture/mock HTTP layer) → hang; C15 rewritten to test liveModeAvailable() gate deterministically; live-HTTP path → operator F24 cutover verify. Migration 80→81/0/0 (settings-only — NO schema motion; acc_qbo_drift_events already exists S-QBO-3). 4 D-QBO-24-* locked. Reuses live drift_events schema (D-QBO-24-2); spec §15.3 sketch corrected. NO PARITY motion (settings seed only).
- D131 — S-QBO-25 2026-05-30 (Phase QBO-12 / 2 of 3 drift resolution workflows — RESOLUTION half) extended `tests/_smoke_qbo_drift_check.php` from 15 → **28 sub-checks** (13 new sub-checks; 15 S-QBO-24 sub-checks re-verified green — no regression to the detection engine) with **per-function coverage discipline** per [[extensive-test-and-full-report]] memory: Module G column+backfill (C16 resolution_type ENUM exists with 3 values + idx_resolution + backfill resolved_at NOT NULL → 'resolved' is complete); Module H state transitions via UPDATE simulating endpoint (C17 resolve sets resolved_at + resolution_type='resolved' + note + user_id; C18 accept; C19 suppress; C24 reopen clears all 4 fields); **Module I CRITICAL terminal-state survival** (C20 a suppressed event SURVIVES a recordDrift re-run with the same key — no duplicate OPEN row; C21 same invariant for accepted — these protect against the core trap where the cron would otherwise re-create suppressed/accepted events as fresh OPEN rows because resolved_at SET → no OPEN match → INSERT); **Module J auto-resolve-on-parity** (C22 a previously-OPEN drift_cron amount_drift event for a sentinel whose drift is now gone auto-resolves with resolution_type='resolved' + note='auto-resolved%' + runCheck.resolved counter ≥ 1; C23 a push_failure-sourced event for a sentinel with no current drift is NOT auto-resolved because autoResolveParity scopes to detection_source='drift_cron' only); Module K endpoint surface metadata via file_get_contents grep (C25 drift_resolve.php contains entity_type → Enqueuer FQCN map covering all 8 ENTITY_CHECKS entity types + 5 action verbs resolve/accept/suppress/reopen/resync; C26 drift_bulk_resolve.php exists with require_permission('quickbooks','edit_credentials') + allowedActions + allowedCategories whitelists + db_transaction wrap); Module L bulk-by-category mutation (C27 a bulk-accept UPDATE simulating the endpoint affects 2 amount_drift sentinels of credit_memo, leaves 1 push_failed sentinel of credit_memo OPEN); Module M visibility filter (C28 default 'unresolved' WHERE = `resolved_at IS NULL AND resolution_type IS NULL` excludes suppressed; explicit 'suppressed' WHERE selects only suppressed; drift_list.php source contains both branches). Migration 81→82/0/0 ALTER `acc_qbo_drift_events` ADD `resolution_type` ENUM('resolved','accepted','suppressed') NULL AFTER resolved_at + KEY `idx_resolution` + backfill resolved_at NOT NULL → 'resolved'. 4 D-QBO-25-* locked (D-QBO-25-1 resolution_type ENUM + terminal-state guard in recordDrift; D-QBO-25-2 auto-resolve-on-parity scoped to drift_cron + categories actually scanned this run; D-QBO-25-3 S-QBO-25/S-QBO-26 bulk-action scope split — bulk-accept/suppress here, force-resync/force-pull → S-QBO-26; D-QBO-25-4 reopen action for operator-undo). PARITY OK (DATABASE_MASTER +resolution_type col + idx_resolution; SCHEMA_QUICK_REF regen). doc_freshness 29/29.
- D131 — S-QBO-26 2026-05-30 (Phase QBO-12 / 3 of 3 Manual Sync page — CLOSES Phase QBO-12) added `tests/_smoke_qbo_manual_sync.php` (10 sub-checks) with **per-function coverage discipline** per [[extensive-test-and-full-report]] memory: Module A surfaces (C1 endpoint exists + lints + force_full_resync POST gate; C2 admin page exists + lints + qboManualSync Alpine factory + view/force_full_resync gates; C3 Manual Sync present after Drift in BOTH config/navigation.php and quickbooks-nav.php partial); Module B endpoint structure (C4 3 action verbs force_resync/force_pull/reset_synctoken; C5 8-entity Enqueuer FQCN map + reuses DriftChecker::ENTITY_CHECKS; C6 pull-capable whitelist = exactly customer/vendor/account/item/tax_code with no push-only leakage; C7 force_pull on push-only type → 422 INVALID_STATE → S-QBO-27); Module C reset_synctoken DB behavior (C8 NULLs qbo_sync_token on a populated sentinel map row — uses array_key_exists so a genuine NULL isn't ??-collapsed, the bug that made the first run report 9/10); Module D force_resync map iteration (C9 the map SELECT ff_fk-NOT-NULL finds the mapped sentinel; C10 with sync_enabled=0 CustomerEnqueuer::enqueue returns false + no queue row → the D-QBO-26-4 reason-ladder path). ALSO bumped 6 nav-assert smokes 16→17 for the new Manual Sync child: `_smoke_qbo_admin_ui` C4, `_smoke_qbo_item_mapping` C16, `_smoke_qbo_customer_mapping` C11, `_smoke_qbo_vendor_mapping` C11, `_smoke_qbo_tax_code_mapping` C13, `_smoke_qbo_account_mapping` C17 (count 16→17 + 'Manual Sync' inserted into each `$expectedOrder` between 'Drift' and 'Customers'). **NO migration** (force_full_resync permission + acc_qbo_sync_queue + qbo_sync_token columns all pre-exist) — migrate held at 82/0/0; NO PARITY motion. 4 D-QBO-26-* locked. doc_freshness 29/29.
- D131 — S-QBO-BILL-ITC-TAX-RATE 2026-06-01 (closes F9 — opt-in per-rate bill-tax ITC emission; gated default-off) added NEW `tests/_smoke_qbo_bill_itc_tax_rate.php` (7 checks): C1 surfaces (BillPusher::buildBillTaxDetail/qboTaxRateId + acc_qbo_tax_rate_map schema); C2 tax_mode='override' → `{TotalTax}` only, NO TaxLine (byte-identical to pre-F9); C3 'per_rate' + gst+hst mapped, pst zero → 2 TaxLines (TR-GST/TR-HST) + TotalTax 18.0; C4 'per_rate' + a non-zero unmapped component → FALLS BACK to override (no TaxLine); C5 'per_rate' + zero tax → plain TotalTax; C6 qboTaxRateId mapped→id / unmapped→null; C7 save_tax_rate_map endpoint (edit_credentials gate + acc_qbo_tax_rate_map) + tax_codes.php hosts the section. Also re-verified `_smoke_qbo_bill_push` 25/25 (override path untouched). **Migration 86→87** (`db_migrations/202606010400_S-QBO-BILL-ITC-TAX-RATE.sql`): CREATE acc_qbo_tax_rate_map (gst/pst/hst → QBO TaxRate.Id; UNIQUE ff_tax_component) + seed quickbooks.bill.tax_mode='override'. BillPusher's inline TxnTaxDetail block was extracted into `buildBillTaxDetail()` (the override branch returns the exact prior shape) + `qboTaxRateId()` looks up the map; per_rate emits TaxLine[] per non-zero mapped component (spec §8.8) and falls back to override on any unmapped non-zero component (never ships a partial/incorrect tax). Admin UI: an ITC Tax-Rate Mapping section on /quickbooks/tax_codes (mode toggle + per-component inputs) + NEW api/v1/quickbooks/save_tax_rate_map.php. **GATED default-off (D-QBO-BILL-ITC-1)** — the QBO TaxRate pull + live per-rate push are verified at cutover; operator enters ids manually + keeps tax_mode='override' until then. DATABASE_MASTER + SCHEMA_QUICK_REF synced (new table) — PARITY OK; doc_freshness 29/29. 1 D-QBO-BILL-ITC-1 locked. F9 ✅ CLOSED.
- D131 — S-QBO-OFFLINE-TESTBED 2026-06-01 (offline fixture HTTP layer + demo-seed; the WHOLE /quickbooks/* surface is now testable WITHOUT a live Intuit connection; NO migration) added NEW `tests/_smoke_qbo_fixture_pipeline.php` (13 checks) — **the FIRST smoke in the suite to deliberately cross the QuickBooksClient::dispatch() boundary**, sanctioned because dispatch is intercepted by the new fixture short-circuit when `quickbooks.fixture_mode='1'`: F1-F2 fixtureMode default-off + ON under sandbox+sentinel realm; F3 PRODUCTION GUARD hard-refused with environment='production'; F4 hard-refused when a non-sentinel real Intuit realm is connected — D-QBO-FIXTURE-2 safety property; F5 QboFixture::respond create echoes payload + synthetic `QBO-FIX-customer-<n>` Id + SyncToken='0' + CurrencyRef preserved; F6 update bumps SyncToken (4→5) via `array_merge` (caught + fixed the `+` array union bug where update would have shadowed the synthetic SyncToken); F7 void returns `{Id,SyncToken,status:'Voided',Voided:true}`; F8 query returns 2-row Customer array + 1-row Invoice as bare object → `QuickBooksClient::normalizeQueryResponse` coerces (K-22 Trap #60 both branches exercised); F9 getEntity returns `{Account:{Id,Name}}`; F10 `injectError('customer:create', 400, …)` single-shot — first call throws `QuickBooksValidationException`, next call hits happy path; F11 END-TO-END enqueue→setWorkerContext→QboPusherDispatcher::dispatch→fixture→assert `acc_qbo_customer_map.push_status='pushed'` + qbo_customer_id LIKE 'QBO-FIX-customer-%' + `acc_qbo_sync_log` POST row realm='FIXTURE-DEMO' response_status=200; F12 `QboDemoSeed::load()` populates ≥1 row per /quickbooks/* surface (pushed=2/failed=1/skipped=2/sync_log=22/drift=3); F13 `QboDemoSeed::wipe()` real-row diff=0 across 16 tables + 0 fixture remnants — the wipe is provably safe. **NEW** `lib/QboFixture.php` (deterministic in-code responder + REALM_SENTINEL='FIXTURE-DEMO' + ID_PREFIX='QBO-FIX-' + injectError single-shot fault injection + per-entity canned collections honoring K-22 #60); `QuickBooksClient::fixtureMode()` + `fixtureRefusalReason()` + private `executeFixture()` short-circuit at top of `executeRequest()` running the SAME downstream writeSyncLog + classifyError + buildException pipeline on synthetic responses (sync_log realm='FIXTURE-DEMO'); `lib/QboDemoSeed.php` orchestrator drives the REAL Enqueuer→worker→Pusher pipeline (FF id range 999000-999989; manufactures pushed/failed via injectError/skipped_by_mode/skipped_soft_deleted); super_admin-gated "Load QBO demo data" + "Wipe" buttons on `/quickbooks/manual_sync` + NEW `api/v1/quickbooks/qbo_demo_seed.php`. Fixture mode default-off ⇒ ZERO behaviour change for live traffic + 25-smoke QBO sweep ALLGREEN. NO migration / DATABASE_MASTER / SCHEMA_QUICK_REF change — migrate held 87/0/0; PARITY OK; doc_freshness 29/29. 4 D-QBO-FIXTURE-* locked (D-QBO-FIXTURE-1 intercept at dispatch chokepoint; D-QBO-FIXTURE-2 production + real-realm hard refuse; D-QBO-FIXTURE-3 deterministic in-code generator over recorded JSON; D-QBO-FIXTURE-4 real-pipeline demo-seed over direct INSERTs). **Operationally unblocks F16/F19/F24/F26/F28/F29 live-verify rehearsals — these can now be dry-run against the fixture before the accountant-seeded sandbox lands at cutover.**
- D131 — S-QBO-24-GL-BALANCE-FOLLOWUP 2026-06-01 (closes F23 — GL-account-balance drift check; DriftChecker extension, NO migration) added NEW `tests/_smoke_qbo_gl_balance_drift.php` (8 checks): C1 surfaces (ffAccountNaturalBalance/glBalanceDrifts/checkGlAccountBalances + DEBIT_NORMAL_TYPES const + 'gl_account' tolerance default); C2 asset natural balance = SUM(debit)−SUM(credit); C3 liability = SUM(credit)−SUM(debit); C4 unknown account → null; C5 glBalanceDrifts tolerance decision (sign-agnostic |Δ|); C6 checkGlAccountBalances no-op while gl_balance_enabled='0'; C7 records a balance_drift event on a drifting asset when enabled (real mapped accounts temporarily parked as 'ignored' so the global scan sees only the 2 sentinels — exactly 1 drift; restored in finally → hermetic); C8 no drift on a matching account. Added to `DriftChecker`: `ffAccountNaturalBalance()` (FF account balance from posted acc_journal_entry_lines, signed per acc_accounts.account_type — debit-normal asset/cost_of_revenue/operating_expense/other_expense vs credit-normal — to align with QBO's positive-normal CurrentBalance), `glBalanceDrifts()`, `checkGlAccountBalances(&$stats, $live)` (snapshot compare FF vs acc_qbo_account_map.qbo_current_balance → category='balance_drift' entity_type='gl_account'; LIVE layer refreshes qbo_current_balance from QBO Account.CurrentBalance first), wired into runCheck() after the entity loop + into autoResolveParity scope (only when the check ran), tolerance default 'gl_account'=$1.00. **GATED default-off via quickbooks.drift.gl_balance_enabled='0' (D-QBO-GL-BALANCE-1)** — GL balances diverge intra-period so it's noise until the accountant sets the reconciliation cadence; operator enables at cutover; default-off keeps the existing drift cron + `_smoke_qbo_drift_check` identical. NO migration (qbo_current_balance + balance_drift category + settings all pre-existed) — migrate held 86/0/0; PARITY OK; doc_freshness 29/29; qbo_queue 9/9. Events render on /quickbooks/drift (dynamic filters). 1 D-QBO-GL-BALANCE-1 locked. F23 ✅ CLOSED.
- D131 — S-QBO-CREDIT-APP-UNAPPLY 2026-06-01 (closes F27 — credit-application un-apply, the inverse of apply.php) added NEW `tests/_smoke_qbo_credit_app_unapply.php` (16 checks): Module A surfaces+schema (C1 CreditApplicationReversal::reverse + ERR consts; C2 credit_note_applications status/reversed_at/reversed_by + map push_status 'voided'; C3 pushVoid + onCreditNoteUnapplied); Module B the financial ROUND-TRIP (C4 credit remaining+status restored 60→100/active; C5 invoice credits_applied+balance+status 40→0/160→200/sent; C6 customer OB +40; C7 application marked reversed; C8 second reverse → already_reversed; C9 unknown → not_found; C10 voided-parent edge — credit untouched, invoice+customer restored); Module C QBO+Enqueuer (C11 pushVoid skipped_unmapped_void; C12 already_voided; C13 skipped_by_mode; C14 Enqueuer gate-0 rejects void on 'applied'; C15 gate-3 rejects update + void on reversed inserts; C16 unapply.php wiring). accounting.enabled forced off in the smoke so the reversing GL JE is a hermetic no-op. **Migration 85→86** (`db_migrations/202606010300_S-QBO-CREDIT-APP-UNAPPLY.sql`): credit_note_applications += status('applied','reversed')+reversed_at+reversed_by (append-only; FK reversed_by→users) + acc_qbo_credit_application_map.push_status += 'voided'. NEW `lib/CreditApplicationReversal.php` (the testable reversal service — apply.php untouched per D-QBO-UNAPPLY-2) + `AutoEntryBridge::onCreditNoteUnapplied` (reversing DR-1030/CR-2060 JE) + `api/v1/credit_notes/unapply.php` (thin endpoint) + `CreditApplicationPusher::pushVoid` (voidEntity('payment'); idempotent on 'voided') + `CreditApplicationEnqueuer` gate-3 +void + gate-0 status invariant + Un-apply UI on credit_notes/show.php. `_smoke_qbo_queue` C8 updated (credit_application now create+void). NO cascade change to credit_notes/void.php (already leaves applied portions intact). DATABASE_MASTER + SCHEMA_QUICK_REF synced (2 tables) — PARITY OK; doc_freshness 29/29. 3 D-QBO-UNAPPLY-* locked. F27 ✅ CLOSED.
- D131 — S-QBO-SHOW-PANEL-PAYDOWN 2026-06-01 (closes F8 — rich QBO sync panel on FF entity show pages; UI-only, NO migration) added NEW `tests/_smoke_qbo_show_panels.php` (8 checks: C1 partial lints, C2 whitelist covers 6 map tables, C3-C7 each of the 5 show pages wires the right entity_type+map_table, C8 partial renders a sentinel config without fatal). Generalized the invoices/show.php "QuickBooks Sync" panel (S-QBO-INVOICE-SHOW-RICH-PANEL) into a reusable partial `includes/partials/qbo-sync-panel.php` — 6-state badge (Not Synced/Synced/Voided/Pending/Failed/Failed Pre-flight/Skipped) + identifiers row (QBO id deep-link with sandbox↔production host swap + pushed-relative + currency + sync token) + last-20 Push History from acc_qbo_sync_log + Retry (per-entity retry endpoint)/View-in-QBO actions; per-page `$qboPanel` config (entity_type/map_table/qbo_id_col/ff_fk/ff_id/deep_link/retry_url) is whitelist-validated against the 6 known map tables (fail-closed) and renders nothing when QBO is disconnected. Wired into `app/admin/accounting/bills/show.php` (bill), `app/admin/payments/show.php` (payment), `app/admin/accounting/ap-payments/show.php` (bill_payment), `app/admin/accounting/journal-entries/show.php` (journal_entry), `app/admin/credit_notes/show.php` (credit_memo — had ZERO QBO mentions, now at parity with invoices). NO migration / DATABASE_MASTER / SCHEMA_QUICK_REF change — migrate held 85/0/0; PARITY OK; doc_freshness 29/29; qbo_queue 9/9. F8 ✅ CLOSED.
- D131 — S-QBO-PAYDOWN-NAV-VENDOR-UI 2026-06-01 (two fully-buildable QBO paydowns; NO migration, NO new smoke) closed F21 + F10. **F21** (nav drift since S-QBO-20): added the `Bank Accounts` child to `config/navigation.php` between Tax Codes and Items (icon building-library, url /quickbooks/bank_accounts) — `includes/partials/quickbooks-nav.php` already had it but the sidebar config had drifted. Bumped the 6 nav-assert smokes 18→19 (`_smoke_qbo_admin_ui` C4 + `_smoke_qbo_{item,customer,vendor,account,tax_code}_mapping`) with 'Bank Accounts' inserted into each `$expected`/`$expectedOrder` after 'Tax Codes'; config/navigation.php + partial + smokes now agree at 19 children. **F10** (deferred UI from S-VENDOR-CURRENCY-COLUMN / D-VENDOR-CURRENCY-COLUMN-4): added a vendor Currency selector (CAD/USD) to `app/admin/vendors/create.php` + `edit.php` Alpine forms (form markup + `form`-object init [create default 'CAD'; edit seeded from `$vendor['currency']` via `$formSeed`] + submit payload) + a Currency display row on `show.php`. UI-only — the vendors.currency ENUM + VendorPusher CurrencyRef emission + API `currency` accept all shipped in S-VENDOR-CURRENCY-COLUMN. create form notes QBO locks vendor currency at creation; edit form notes a change only reaches QBO on a re-create (VendorPusher strips CurrencyRef from UPDATE payloads per Intuit policy). NO migration / DATABASE_MASTER / SCHEMA_QUICK_REF change — migrate held 85/0/0; PARITY OK. doc_freshness 29/29; qbo_queue 9/9. Both follow-ups marked ✅ CLOSED in OPERATOR_FOLLOWUPS.
- D131 — S-QBO-27 2026-06-01 (Historical Backfill MACHINERY — Phase QBO-13; spec §16 one-time inbound migration; **dry-run scaffold**, live runs + GL remediation deferred to the accountant-seeded sandbox F29) added `tests/_smoke_qbo_historical_pull.php` (22 sub-checks, NEW): Module A surfaces+schema+order (C1 HistoricalPuller orchestration methods + ENTITY_ORDER/REFERENCE_PULLERS/TRANSACTIONAL_TYPES consts; C2 ArDriftRemediator detect/buildPlan/detectAndReport/postApprovedPlan + AR_ACCOUNT_CODE/H6_RATIO; C3 acc_qbo_historical_pull_runs schema; C4 ENTITY_ORDER 12 entries reference-first + transactional subset); Module B orchestration offline (C5 startRun(dry_run); C6 startRun('live') refused under dry-run gate; C7 isDryRun+batch=100; C8 assertLiveAllowed throws; C9 mapMeta invoice/refund_receipt/null-account; C10 resumePoint=MAX(pushed_at); C11 alreadyMapped; C12 tally+checkpoint JSON merge; C13 pullTransactional rejects non-transactional + writeFfRowFromQbo refuses F29); Module C H5/H6 detection sentinel-scoped (C14 H5 missing_dr_ar=total; C15 H6 excess from DR-AR=1.375×total; C16 clean invoice in neither bucket; C17 buildPlan tagged DR/CR [A1-FIX-invoice-N]; C18 detectAndReport='reported'+plan stored+ZERO JEs posted; C19 postApprovedPlan refused — gated); Module D endpoints (C20 start.php detectAndReport + force_full_resync gate; C21 status+remediation_plan lint; C22 manual_sync hosts qboHistoricalPull). **Migration 84→85** (`db_migrations/202606010200_S-QBO-27.sql`): CREATE acc_qbo_historical_pull_runs (run/checkpoint/remediation state) + seed historical_pull.dry_run='1' + batch_size='100'. NEW `lib/QboPushers/HistoricalPuller.php` (orchestrator — 12-step ENTITY_ORDER per §16.6, batch-100 + MAX(pushed_at) resume, dry-run safety gate refusing FF writes + JE posting, reference-pull delegation to shipped pullers, transactional dispatch + idempotency, gated writeFfRowFromQbo F29 seam) + `lib/QboPushers/ArDriftRemediator.php` (H5/H6 detection from pure FF subledger-vs-GL queries; tagged compensating-JE plan; hard-stop-and-report — never auto-posts per D-QBO-27-5). UI shares /quickbooks/manual_sync (D-QBO-27-7 — no new nav child; orchestrators aren't `*Pusher` so CLASS 12 stays 10 Pushers) + api/v1/quickbooks/historical_pull/{start,status,remediation_plan}.php. 7 D-QBO-27-* locked. doc_freshness 29/29; qbo_queue 9/9; PARITY OK. **Live pull (27.A–E) + QBO→FF transforms + H5/H6 posting deferred to the seeded sandbox (F29); all build phases QBO-1..QBO-13 now have shipped machinery; remaining QBO = the S-QBO-27 live run + S-QBO-28/29/30 cutover.**
- D131 — S-QBO-17 2026-06-01 (Refund Receipt push — Phase QBO-7 / 2 of 2, **CLOSES Phase QBO-7**; all build phases QBO-1..QBO-12 now complete) added `tests/_smoke_qbo_refund_receipt_push.php` (31 sub-checks, NEW) with per-function coverage: Module A surfaces+schema (C1 RefundReceiptPusher methods + REFUND_ITEM_TYPE; C2 RefundReceiptEnqueuer; C3 acc_qbo_refund_receipt_map schema cols + UNIQUE uq_ff_lease + uq_qbo_refund_receipt; C4 queue ENUM has refund_receipt); Module B buildQboPayload (C5 happy single SalesItemLineDetail line Amount=precharge_balance + PaymentMethodRef + DepositToAccountRef + ItemRef + TaxCodeRef + TotalTax=0 + DocNumber=RFD-<id> + PrivateNote; C6 multi_currency; C7-C11 throws on missing customer/item/deposit/payment-method/tax; C12 throws on precharge_balance≤0); Module C buildPrivateNoteJson (C13); Module D runPreflight per-gate (C14 customer / C15 item=mileage_credit / C16 tax-override / C17 deposit account / C18 payment method / C19 ok+resolved refs); Module E pushImpl (C20 sync_mode=disabled→skipped_by_mode; C21 ff_not_found; C22 unsettled→skipped_not_eligible; C23 already_mapped; C24 failed_preflight→map row); Module F Enqueuer (C25 missing lease / C26 non-cash / C27 unsettled / C28 sync_enabled=0 / C29 non-create / C30 happy insert refund_receipt/lease_id/create/queued); Module G C31 mark_refund_settled.php hook present. **HTTP-trap discipline**: every check short-circuits before createEntity. ALSO bumped `_smoke_qbo_queue` C3 14→15 sync_mode (+`quickbooks.sync_mode.refund_receipt`) + C8 hasImplementation('refund_receipt','create')===true + update/void===false; `_smoke_doc_freshness` CLASS 12 10 Pushers (RefundReceiptPusher→refund_receipts.php); **6 nav-assert smokes 17→18** (admin_ui C4 + customer/vendor/account/tax_code/item mapping) for the new "Refund Receipts" nav child after "Credit Memos". **Migration 83→84** (`db_migrations/202606010100_S-QBO-17.sql`): CREATE `acc_qbo_refund_receipt_map` (keyed on ff_lease_id — refund lives on `leases`, one cash refund per lease per D-QBO-17-5; FK CASCADE on leases) + seed `sync_mode.refund_receipt='sync'` + `refund.deposit_account_id=''` + `refund.payment_method_id=''` (the queue ENUM motion is a no-op — refund_receipt was already a planned ENUM value). NEW RefundReceiptPusher + RefundReceiptEnqueuer + post-commit `enqueue('create')` hook in `mark_refund_settled.php`. UI NEW `/quickbooks/refund_receipts` + `api/v1/quickbooks/refund_receipts/{list,retry}.php`. Cash-branch only (D-QBO-17-2; credit refunds ride CreditMemo path). PaymentMethodRef=cheque + DepositToAccountRef from settings (D-QBO-17-6 — no QBO PaymentMethod map exists). TaxCodeRef=NON assumption → live-verify F28 (safe pre-cutover). 6 D-QBO-17-* locked. doc_freshness 29/29; qbo_queue 9/9; admin_retry 20/20; PARITY OK. **PHASE QBO-7 COMPLETE; remaining QBO work = S-QBO-27 historical pull + S-QBO-28/29/30 cutover.**
- D131 — S-QBO-CREDIT-MEMO-APPLY 2026-06-01 (closes OPERATOR_FOLLOWUPS F25 — credit-memo apply→LinkedTxn propagation; the LAST open QBO update-debt item) added `tests/_smoke_qbo_credit_application_push.php` (26 sub-checks, NEW) with per-function coverage per [[extensive-test-and-full-report]]: Module A class+schema (C1 CreditApplicationPusher surfaces: pushCreate+buildQboPayload+buildPrivateNoteJson+runPreflight; C2 CreditApplicationEnqueuer::enqueue surfaces; C3 acc_qbo_credit_application_map schema — cols + UNIQUE uq_ff_credit_application + UNIQUE uq_qbo_payment_apply; C4 acc_qbo_sync_queue.entity_type ENUM extends with 'credit_application'); Module B buildQboPayload (C5 happy: TotalAmt=0 + single Line with Amount=amount_applied + 2 LinkedTxns CreditMemo+Invoice + CustomerRef + PrivateNote; C6 multi_currency='1' → CurrencyRef + ExchangeRate; C7-C9 throws on empty qbo_customer_id / qbo_credit_memo_id / qbo_invoice_id; C10 throws on amount_applied ≤ 0); Module C buildPrivateNoteJson (C11 valid JSON with 5 required keys); Module D runPreflight per-gate (C12 invalid ids; C13 parent credit memo unmapped — qbo_credit_memo_id NULL; C14 target invoice unmapped; C15 customer unmapped; C16 all good → ok + returns resolved QBO IDs); Module E pushImpl (C17 sync_mode='disabled' → skipped_by_mode + map row + sync_log; C18 ff_not_found; C19 already_mapped → no second POST; C20 failed_preflight → map row written with typed status); Module F Enqueuer gates (C21 gate-0 rejects missing application; C22 gate-1 rejects sync_enabled='0'; C23 gate-2 rejects sync_mode='disabled'; C24 gate-3 rejects non-'create' op — v1 create-only; C25 happy queue insert credit_application/create/queued); Module G C26 apply.php integration assertion (post-commit CreditApplicationEnqueuer::enqueue hook present). **HTTP-trap discipline**: every check short-circuits before createEntity (no fixture HTTP layer). ALSO bumped `_smoke_qbo_queue` C3 13→14 sync_mode keys (+`quickbooks.sync_mode.credit_application`) + C8 hasImplementation widened to assert `('credit_application','create')===true` + `('credit_application','update'|'void')===false`; `_smoke_doc_freshness` CLASS 12 9 Pushers (CreditApplicationPusher mapped to shared `credit_memos.php` surface — applications are operationally a sub-view of their parent credit memo per D-QBO-CREDIT-MEMO-APPLY-5). **Migration 82→83** (`db_migrations/202606010000_S-QBO-CREDIT-MEMO-APPLY.sql`, 3 motions): ALTER `acc_qbo_sync_queue.entity_type` ENUM += `'credit_application'` + CREATE `acc_qbo_credit_application_map` (UNIQUE ff_credit_application_id ↔ qbo_payment_id, FK CASCADE on credit_note_applications, slimmer push_status ENUM — pending/pushed/failed/skipped_by_mode/failed_preflight only, no soft-delete/field/currency/void paths) + INSERT `quickbooks.sync_mode.credit_application='sync'` (separate from credit_memo so apply propagation can be paused independently). NEW CreditApplicationPusher + CreditApplicationEnqueuer + post-commit `enqueue('create')` hook in `api/v1/credit_notes/apply.php` (mirrors create/void hooks). UI shares `app/admin/quickbooks/credit_memos.php` — new "Applications → QBO LinkedTxn" sub-section with 5 KPI tiles + status filter + table (Credit Note/Invoice/Customer/Amount Applied/QBO Payment Id/Status/Action) + retry + operator pre-req note for QBO "Automatically apply credits" must-be-OFF (D-QBO-CREDIT-MEMO-APPLY-3, F26 BLOCKING). Backed by NEW `api/v1/quickbooks/credit_applications/{list,retry}.php`. v1 forward-create only (D-QBO-CREDIT-MEMO-APPLY-4); un-apply tracked as new F27. 5 D-QBO-CREDIT-MEMO-APPLY-* locked. doc_freshness 29/29; qbo_queue 9/9; admin_retry 20/20; credit_memo_push 30/30; new credit_application_push 26/26. **BIG-PICTURE MILESTONE: every QBO entity sync path — create, update, void, AND apply — now complete. QBO update-debt FULLY paid down.**
- D131 — S-QBO-PUSHVOID-TRIO 2026-06-01 (closes OPERATOR_FOLLOWUPS F7 — the pushVoid trio; first QBO update-debt slice that is NOT a pushUpdate mirror) added `pushVoid` to PaymentPusher / BillPaymentPusher / CreditMemoPusher (Payment + BillPayment had no void method; CreditMemo replaced its stub), each modeled on InvoicePusher::pushVoidImpl (D-QBO-12-3/4/5): separate pipeline (not pushImpl); idempotent on push_status='voided' → already_voided; no map / null qbo_*_id → skipped_unmapped_void (no row written); voidEntity via the uniform QuickBooksClient::voidEntity(type,id,syncToken). **Per-entity void trigger (D-QBO-PUSHVOID-TRIO-1): bill_payment + credit_note key on status='void'; payment keys on deleted_at IS NOT NULL** (payments have no clean status='void'; the soft-delete path payments/delete.php IS the void — refunded/partially_refunded → future RefundReceipt, out of scope). PaymentPusher::pushVoid also carries the D-QBO-14-1 origin guard (pulled_from_qbo → skipped_non_ff_origin). 3 Enqueuers gate-3 widened += 'void' + per-entity gate-0 void eligibility; enqueue('void') hooks wired post-commit into payments/delete.php + ap-payments/void.php + credit_notes/void.php. Smokes: `_smoke_qbo_payment_push` 23→26 (C24 void_status_mismatch / C25 skipped_unmapped_void / C26 Enqueuer-accepts-void), `_smoke_qbo_bill_payment_push` 27→30 (C27 REPURPOSED accepts-void / C28 void_status_mismatch / C29 skipped_unmapped_void / C30 already_voided), `_smoke_qbo_credit_memo_push` 28→30 (C22 REPURPOSED void_status_mismatch / C28 REPURPOSED accepts-void / C29 skipped_unmapped_void / C30 already_voided). **HTTP-trap discipline: every void check short-circuits at a gate/idempotent path BEFORE the voidEntity call** (no fixture HTTP layer). qbo_queue 9/9 + admin_retry 20/20 + doc_freshness 29/29 re-green. **NO migration** ('voided'/'skipped_voided' pre-exist in all 3 map ENUMs; skipped_unmapped_void/skipped_non_ff_origin are transient return statuses) — migrate held 82/0/0; NO PARITY motion. 1 D-QBO-PUSHVOID-TRIO-1 locked. **Every entity Pusher now supports create + update + void; remaining QBO update-debt = ONLY F25 (credit-memo apply→LinkedTxn, needs a migration).**
- D131 — S-QBO-JE-UPDATE 2026-05-31 (FIFTH + FINAL slice of S-QBO-PUSHER-UPDATE-FOLLOWUPS-PAYDOWN — closes F13 / D-QBO-21-5 JournalEntryPusher::pushUpdate stub; **mechanical paydown COMPLETE 5/5**) extended `tests/_smoke_qbo_journal_entry_push.php` from 31 → 33 sub-checks: C14 REPURPOSED (pushUpdate now delegates to pushImpl — returns skipped_by_mode under sync_mode='disabled', proving no-longer-a-stub; **the 'disabled' is REQUIRED to avoid the HTTP trap** — the fixture JE 999990 is seeded+mapped so a non-disabled pushUpdate would reach a real updateEntity 404); C25 REPURPOSED (JournalEntryEnqueuer gate-3 now ACCEPTS 'update' — allowlist widened ['create']→['create','update'] — + inserts a queue row); C32 NEW (pushUpdate on an unmapped JE demotes to create per D-PUSHER-DEMOTION-RULE — fails at the account-mapping preflight gate before the HTTP boundary, no qbo_journal_entry_id persisted); C33 NEW (Enqueuer rejects 'void' — not a JE concept; JEs reverse via a companion posted JE). JournalEntryPusher::pushUpdate implemented via updateEntity full-payload re-send + step 5b demote-to-create — **CRITICAL placement: step 5b sits AFTER the bridge-derived gate (step 4)**, so an update of a bridge-derived JE is rejected as skipped_bridge_derived before the operation branch (the D-QBO-21-1 double-accounting guard covers the UPDATE verb without extra code; JE analog of PaymentPusher's step-7b-after-origin). No SELECT change (step 3 already selected qbo_sync_token); no idempotency change (step 5 already operation==='create'-guarded). **D2: NO enqueue hook** — there is no journal_entries/update.php (JEs immutable post-posting; endpoints are approve/post/recall/reverse/submit); the mechanical update rides manual-sync force_resync. NO migration (push_status ENUM + qbo_sync_token pre-exist) — migrate held at 82/0/0; NO PARITY motion. qbo_queue 9/9 + admin_retry 20/20 + doc_freshness 29/29 re-green. 1 D-QBO-JE-UPDATE-1 locked. **This CLOSES the pusher-update mechanical arc: Bill + Payment + BillPayment + CreditMemo + JE pushUpdate all shipped. Remaining QBO update-debt = pushVoid trio (F7) + carved-out credit-memo apply→LinkedTxn (F25).**
- D131 — S-QBO-CREDIT-MEMO-UPDATE 2026-05-31 (fourth slice of S-QBO-PUSHER-UPDATE-FOLLOWUPS-PAYDOWN — closes the literal D-QBO-16-2 CreditMemoPusher::pushUpdate stub, part of F20) extended `tests/_smoke_qbo_credit_memo_push.php` from 26 → 28 sub-checks: C21 REPURPOSED (pushUpdate now delegates to pushImpl — returns skipped_by_mode under sync_mode='disabled', proving no-longer-a-stub; **the 'disabled' is REQUIRED to avoid the HTTP trap** — 999990 is active+mapped so a non-disabled pushUpdate would demote-to-create and hit a real createEntity 404); C22 KEPT (pushVoid STILL a stub → unsupported_in_session, now asserts the error references the F7 pushVoid trio — pushVoid is NOT closed this slice); C25 REPURPOSED (CreditMemoEnqueuer gate-3 now ACCEPTS 'update' — allowlist widened ['create']→['create','update'] — + inserts a queue row); C27 NEW (pushUpdate on an unmapped credit note demotes to create per D-PUSHER-DEMOTION-RULE — fails at the customer-mapping preflight gate BEFORE the HTTP boundary, no qbo_credit_memo_id persisted); C28 NEW (Enqueuer still rejects 'void'). CreditMemoPusher::pushUpdate implemented via updateEntity full-payload re-send + step 5b demote-to-create + a fix to the previously-unguarded step-5 idempotency (now guarded on operation==='create' so updates fall through). **SCOPE DECISION (operator-confirmed): the apply→LinkedTxn credit-APPLICATION propagation is CARVED OUT to NEW follow-up F25** (zero-dollar QBO Payment + migration + apply.php hook) — credit notes have no editable header so the mechanical pushUpdate is a near-noop refresh; only the literal stub is closed here. NO enqueue hook wired (rides manual-sync force_resync). NO migration (push_status ENUM + qbo_sync_token pre-exist) — migrate held at 82/0/0; NO PARITY motion. qbo_queue 9/9 + admin_retry 20/20 + doc_freshness 29/29 re-green. 1 D-QBO-CREDIT-MEMO-UPDATE-1 locked. Remaining update-stub paydown: JournalEntryPusher F13 (the last clean mechanical mirror).
- D131 — S-QBO-BILL-PAYMENT-UPDATE 2026-05-31 (third slice of S-QBO-PUSHER-UPDATE-FOLLOWUPS-PAYDOWN — closes F6 / D-QBO-19-5 BillPaymentPusher::pushUpdate stub) extended `tests/_smoke_qbo_bill_payment_push.php` from 25 → 27 sub-checks: C17 REPURPOSED (pushUpdate now delegates to pushImpl — returns skipped_by_mode under sync_mode='disabled', proving it's no longer the `unsupported_in_session` stub); C22 REPURPOSED (BillPaymentEnqueuer gate-3 now ACCEPTS 'update' — allowlist widened ['create']→['create','update'] — + inserts a queue row); C26 NEW (pushUpdate on an unmapped ap_payment demotes to create per D-PUSHER-DEMOTION-RULE — fails at the vendor-mapping preflight gate before the HTTP boundary, no qbo_bill_payment_id persisted); C27 NEW (BillPaymentEnqueuer still rejects 'void' — pushVoid trio remains in F7). BillPaymentPusher::pushUpdate implemented via updateEntity full-payload re-send + SyncToken round-trip + step 5b demote-to-create when unmapped (placement: SAME AS BILL TEMPLATE, NOT the payment-specific step 7b — acc_ap_payments has no `origin` column per D-QBO-19-1 so no payment-style dedup-gate dance is needed). **D2 sub-decision: WIRE enqueue('update') hook in api/v1/accounting/ap-payments/update.php** (mirrors S-QBO-BILL-UPDATE pattern; diverges from S-QBO-PAYMENT-UPDATE no-hook decision) — 3 of 4 editable fields (payment_date / reference_number / check_number) directly affect the QBO BillPayment payload as TxnDate + PrivateNote; only `notes` is FF-only, so the hook propagates real QBO-relevant changes (vs payment where 4 of 5 fields were FF-only and the hook would queue near-noop re-pushes). NO migration (push_status ENUM + qbo_sync_token pre-exist) — migrate held at 82/0/0; NO PARITY motion. qbo_queue 9/9 + admin_retry 20/20 + doc_freshness 29/29 re-green. 1 D-QBO-BILL-PAYMENT-UPDATE-1 locked. Remaining update-stub paydowns: credit_memo F20 + JE F13 + the pushVoid trio F7 (3 of 5 update slices shipped).
- D131 — S-QBO-PAYMENT-UPDATE 2026-05-31 (second slice of S-QBO-PUSHER-UPDATE-FOLLOWUPS-PAYDOWN — closes F5 / D-QBO-14-5 PaymentPusher::pushUpdate stub) extended `tests/_smoke_qbo_payment_push.php` from 20 → 23 sub-checks: C18 REPURPOSED (PaymentEnqueuer gate-3 now ACCEPTS 'update' — allowlist widened ['create']→['create','update'] — + inserts a queue row); C20 REPURPOSED (pushUpdate now delegates to pushImpl — returns skipped_by_mode under sync_mode='disabled', proving it's no longer the `unsupported_in_session` stub); C21 NEW (pushUpdate on an unmapped payment demotes to create per D-PUSHER-DEMOTION-RULE — fails at the customer-mapping preflight gate before the HTTP boundary, no qbo_payment_id persisted); C22 NEW (PaymentEnqueuer still rejects 'void' — pushVoid trio remains in F7); **C23 NEW CRITICAL** (pushUpdate of a webhook-originated payment with origin='qbo_payments_webhook' returns skipped_non_ff_origin — proves the D-QBO-14-1 bidirectional dedup invariant covers the UPDATE verb too, because pushImpl steps 5+6 origin/pulled_from_qbo gates run BEFORE the operation branch; this was the key risk-surface difference vs the Bill template). PaymentPusher::pushUpdate implemented via updateEntity full-payload re-send + SyncToken round-trip + step 7b demote-to-create when unmapped (placement: AFTER origin/pulled_from_qbo gates by design — that placement is what makes C23 hold). **D2 sub-decision: NO enqueue hook in api/v1/payments/update.php** (diverges from BillPusher which DID wire the hook in bills/update.php) — of the 5 editable metadata fields, only reference_number→PaymentRefNum is QBO-pushable; auto-enqueuing on every metadata edit would queue near-noop re-pushes. Operators trigger reference_number sync via /quickbooks/manual_sync → Force re-sync (payments) (S-QBO-26 force_resync). Doc-only divergence captured as inline NOTE in update.php for the next maintainer. NO migration (push_status ENUM + qbo_sync_token pre-exist) — migrate held at 82/0/0; NO PARITY motion. qbo_queue 9/9 + admin_retry 20/20 + doc_freshness 29/29 re-green. 1 D-QBO-PAYMENT-UPDATE-1 locked. Remaining update-stub paydowns: bill_payment F6 / credit_memo F20 / JE F13 + the pushVoid trio F7.
- D131 — S-QBO-BILL-UPDATE 2026-05-31 (first slice of S-QBO-PUSHER-UPDATE-FOLLOWUPS-PAYDOWN) extended `tests/_smoke_qbo_bill_push.php` from 23 → 25 sub-checks: C16 repurposed (pushUpdate now delegates to pushImpl — returns skipped_by_mode under sync_mode='disabled', proving it's no longer the `unsupported_in_session` stub); C19 repurposed (BillEnqueuer gate-3 now ACCEPTS 'update' — allowlist widened ['create']→['create','update'] — + inserts a queue row); C24 NEW (pushUpdate on an unmapped bill demotes to create per D-PUSHER-DEMOTION-RULE — fails at the vendor preflight gate before the HTTP boundary, no qbo_bill_id persisted); C25 NEW (BillEnqueuer still rejects 'void'). BillPusher::pushUpdate implemented via updateEntity full-payload re-send + SyncToken round-trip (mirrors InvoicePusher D-QBO-12-1); enqueue('update') wired into bills/update.php. NO migration (push_status ENUM + qbo_sync_token pre-exist) — migrate held at 82/0/0; NO PARITY motion. 1 D-QBO-BILL-UPDATE-1 locked. The proven template for the remaining payment/bill_payment/credit_memo/JE pushUpdate paydowns.
- D131 — S-QBO-16 2026-05-29 (Phase QBO-7 / 1 of 2 Credit Memo push — first entity Pusher since S-QBO-19) added `tests/_smoke_qbo_credit_memo_push.php` (26 sub-checks) with **per-function coverage discipline** per [[extensive-test-and-full-report]] memory: Module A class+schema (C1-C3); Module B resolveItemType all 9 source ENUM values + 'other' fallback (C4-C5); Module C buildQboPayload happy + 3 throws + currency-gating (C6-C10); Module D runPreflight per-gate — tax-override/customer/item/DocNumber-too-long/all-good (C11-C15); Module E pushImpl behaviors skipped_by_mode/voided/soft_deleted/already_mapped/ff_not_found (C16-C20); Module F update+void stubs (C21-C22); Module G Enqueuer gates (C23-C26). Also updated `_smoke_qbo_queue.php` C8 (credit_memo hasImplementation flips false→true) + C9 (unbuilt-entity fixture credit_memo→item) + 6 nav-asserting smokes (child count 15→16, +Credit Memos) + CLASS 12 in doc_freshness (CreditMemoPusher→credit_memos.php; 8/8 Pushers mapped). Migration 79→80/0/0 (acc_qbo_credit_memo_map). 3 D-QBO-16-* locked. PARITY OK (DATABASE_MASTER +1 table; SCHEMA_QUICK_REF regen 148→149).
- D131 — S-QBO-23 2026-05-29 (Phase QBO-11 / 2 of 2 Tax Remittance JE sync — CLOSES Phase QBO-11) added `tests/_smoke_qbo_tax_remittance_je_push.php` (19 sub-checks) with **per-function coverage discipline** per [[extensive-test-and-full-report]] memory: Module A constants + D-QBO-23-4 bridge regression guard incl. enrichedSourceTypes() union (C1-C4); Module B `buildTaxRemittanceNoteSection` happy/missing/invalid (C5-C7); Module C `buildSourceNoteSection` dispatcher incl. CRITICAL C9 regression — dispatcher still routes depreciation → FA-DEP (C8-C11); Module D PrivateNote integration incl. CRITICAL C13 regression — FA JE PrivateNote STILL contains FA-DEP after the D-QBO-23-1 dispatcher refactor (C12-C15); Module E Enqueuer accepts tax_remittance (C16); Module F marker setting (C17); Module G API contract (C18-C19). The dispatcher refactor generalized S-QBO-22's FA-only PrivateNote block — S-QBO-22 FA smoke 24/24 + S-QBO-21 JE smoke 31/31 RE-RUN GREEN post-refactor to prove non-breaking. Migration 78→79/0/0 (settings seed only — `quickbooks.sync_mode.tax_remittance='inherit_je'` marker; NO schema motion, NO ENUM ALTER since tax_remittance already present). 4 D-QBO-23-* locked.
- D131 — S-QBO-22 2026-05-29 (Phase QBO-11 / 1 of 2 Fixed Asset JE sync) added `tests/_smoke_qbo_fixed_asset_je_push.php` (24 sub-checks) with **per-function coverage discipline** per [[extensive-test-and-full-report]] memory: Module A constants + bridge regression guard (C1-C3); Module B `buildFixedAssetNoteSection` × 3 source types × happy/missing/edge/sanitization (C4-C12; 9 sub-checks); Module C PrivateNote integration via `buildQboPayload` (C13-C17); Module D Enqueuer accepts FA types (C18-C20); Module E `FixedAssetService::impair` literal change (C21); Module F schema migration (C22); Module G API contract (C23-C24). NOTE: this paragraph's predecessor entry stopped at S-QBO-13 — D131 history bullet was not updated by S-QBO-14/15/18/19/20/21 even though those added smokes; gap acknowledged + tracked here for S-QBO-22 as the first session to close the lag. Migration 77→78/0/0 (`acc_journal_entries.source_type` ENUM +impairment per D-QBO-22-2). 4 D-QBO-22-* locked.
- D131 — S-QBO-14 2026-05-28 (Phase QBO-6 / 2 of 3 Payment push FF→QBO) added `tests/_smoke_qbo_payment_push.php` (**20 sub-checks** originally; now 26 after the S-QBO-PAYMENT-UPDATE + pushVoid-trio slices): PaymentPusher 8 inline pre-flight gates (UndepositedFunds resolution + currency-mismatch + 21-char PaymentRefNum) + PaymentEnqueuer gate-0 origin='ff_native'+status='cleared' bidirectional-dedup (D-QBO-13-1/2 closed at the enqueue layer, defense-in-depth at pushImpl). NO migration — reused acc_qbo_payment_map from S-QBO-13. 7 D-QBO-14-* locked. *(Backfilled by F18 / S-CLAUDE-CODE-REFERENCE-D131-BACKFILL 2026-06-01 — the S-QBO-22 entry above flagged that 14/15/18/19/20/21 were never indexed.)*
- D131 — S-QBO-15 2026-05-29 (Phase QBO-6 / 3 of 3 QBO Payments portal embed — CLOSES Phase QBO-6) extended `tests/_smoke_qbo_payments_embed.php` **28 → 31 sub-checks**: PaymentInitiator single-pending-per-invoice (expire-before-insert per D-QBO-15-1) + generatePaymentsHostedUrl + PaymentWebhookHandler initiation handshake + acc_qbo_payment_initiations table (17 cols + UNIQUE on initiation_token + 5 idx + 2 FKs). 5 D-QBO-15-* locked. *(Backfilled by F18 2026-06-01.)*
- D131 — S-QBO-18 2026-05-27 (Phase QBO-8 / 1 of 2 Bill push FF→QBO, first AP-direction Pusher) added `tests/_smoke_qbo_bill_push.php` (**20 sub-checks** originally; now 25 after S-QBO-BILL-GOTCHAS-PAYDOWN + S-QBO-BILL-UPDATE): BillPusher + BillEnqueuer + acc_qbo_bill_map + AccountBasedExpenseLineDetail per line + tax-override (D-QBO-CORE-6) + DocNumber from vendor_bill_number (D-QBO-18-7). 7 D-QBO-18-* locked. *(Backfilled by F18 2026-06-01.)*
- D131 — S-QBO-19 2026-05-29 (Phase QBO-8 / 2 of 2 Bill Payment push — CLOSES Phase QBO-8) added `tests/_smoke_qbo_bill_payment_push.php` (**~25 sub-checks** originally; now 30 after S-QBO-BILL-PAYMENT-UPDATE + pushVoid-trio) + 2 coverage-gap smokes (`_smoke_qbo_admin_retry_endpoints` + a `_smoke_qbo_payments_embed` extension 28→31): BillPaymentPusher + BillPaymentEnqueuer + acc_qbo_bill_payment_map + BankAccountRef 3-step chain (ap_payment.bank_account_id → acc_bank_accounts.gl_account_id → acc_qbo_account_map). 7 D-QBO-19-* locked. *(Backfilled by F18 2026-06-01.)*
- D131 — S-QBO-20 2026-05-29 (Phase QBO-9 — bank account mapping + read-only CDC mirror, Exception #2 per D-QBO-CORE-2; CLOSES Phase QBO-9) added `tests/_smoke_qbo_bank_mapping.php` (**14 sub-checks**) + `tests/_smoke_qbo_bank_cdc.php` (**16 sub-checks**): BankAccountMatcher + BankTransactionPuller + cron/qbo_bank_cdc.php @ 02:30 + 2 new tables (acc_qbo_bank_account_map + acc_qbo_bank_transaction_map composite {entity}:{id} key) + ALTER acc_bank_transactions (source ENUM +qbo_cdc + is_readonly + qbo_bank_txn_id). Migration 76→77. 7 D-QBO-20-* locked; F14 (crontab) + F15 (FX) surfaced. *(Backfilled by F18 2026-06-01.)*
- D131 — S-QBO-21 2026-05-29 (Phase QBO-10 — Journal Entry push; CLOSES Phase QBO-10) added `tests/_smoke_qbo_journal_entry_push.php` (**31 sub-checks** originally; now 33 after S-QBO-JE-UPDATE): JournalEntryPusher + JournalEntryEnqueuer + acc_qbo_journal_entry_map + the bridge-derived source_type skip list (invoice/payment/credit_note/ap_bill/ap_payment per D-QBO-21-1, defense-in-depth at Enqueuer gate-0 AND Pusher step 4). 7 D-QBO-21-* locked; F12 (tax_receivable/tax_payable mappings) + F13 surfaced. *(Backfilled by F18 2026-06-01.)*
- D131 — S-TRAINING-VIDEO-BUGFIX 2026-09-16 added 8 smokes: `tests/_smoke_close_mileage_semantics.php` (21; HTTP — honours `FF_SMOKE_BASE_URL` + new `FF_SMOKE_SESS_DIR` for a `php -S` server whose session dir is the CLI temp dir, not Herd's /var/tmp), `_smoke_payment_overpayment_credit.php` (20), `_smoke_balance_sheet_balances.php` (33), `_smoke_recurring_entries_catchup.php` (27), `_smoke_fleet_utilization_ar_aging.php` (30), `_smoke_vendor_spend_and_bill_numbering.php` (21), `_smoke_compliance_tracked_docs.php` (18), `_smoke_email_plaintext_body.php` (11) — all hermetic (rolled-back txn or shutdown cleanup). `_smoke_api_envelope_contract.php` get()-envelope regex now accepts `return await res.json()`. **Never run `_smoke_golive_reset.php` in a sweep — it wipes transaction tables by design.** Known data-coupled failure: `_smoke_payoff_design.php` equipment_show_linked/unlinked expect unit 6 ↔ asset 17 from an older demo dataset — FIXED 2026-09-24 by S-PAYOFF-SMOKE-DATA-INDEPENDENT (units resolved at run time).
- D131 — S-CASHFLOW-TIE 2026-09-17 added `tests/_smoke_cash_flow_ties.php` (**160 sub-checks**, hermetic — one rolled-back txn, seeded entries dated 2031, before/after deltas): Part A ties `ReportingService::cashFlow()` on 8 real-ledger ranges to raw SQL (opening + net change = closing cash on every cash account, net income = P&L report, every `is_bank_account` account listed as cash) and checks `workingTrialBalance()` on 4 real periods (balanced; BS rows cumulative, P&L rows fiscal YTD; Unadj + AJEs = Adj; Var = Adj − PY; unclosed-earnings row = balance sheet's); Part B seeds one entry per classification path (checking-linked + undeposited cash, contra allowance, loan-financed equipment, depreciation / disposal / impairment / FX revaluation / sales-type lease inception, current-portion reclass, LOC, drawings, credit note, reversed pair, draft, year-end close) and asserts exact rows; Part C seeds an AJE, a prior unclosed year and an inactive account for the WTB. Mutation-tested: dropping the bank-account link, counting the year-end close, since-inception P&L in the WTB, and dropping disposal/FX cash lines each fail it.
- D131 — S-SQL-LOCAL-DATE 2026-09-17 added `tests/_smoke_local_date_overdue.php` (9): real `api/v1/invoices/kpis.php` + `index.php?aging=` run in a CLI subprocess inside a rolled-back transaction with the SQL clock pinned via MySQL `SET TIMESTAMP = UNIX_TIMESTAMP('<local today+1> 01:30:00')` — the reusable way to reproduce the evening UTC-rollover class deterministically at any wall-clock time. Business-DATE SQL binds `ff_today()`; never `CURDATE()`.
- D131 — S-SQL-LOCAL-DATE-FOLLOWUPS 2026-09-17 added `tests/_smoke_insights_compliance_export.php` (10): runs the real `api/v1/admin/intelligence/insights.php` (report_cache fixture, row-timestamp fallback) plus a static sweep that no `report_cache` SELECT names `created_at`, and compares the unit set of `app/admin/compliance/index.php?export=csv` with `api/v1/compliance/index.php` for yard / +expired_only / +q / +q+expired_only (a doc expiring today must be 'Expiring Soon'). Extended `tests/_smoke_local_date_overdue.php` 9 → 22: the harness fixture mode `'2'` adds partially_paid + overdue invoices, and section P asserts, for every aging bucket, list ids == expected, list count == tile count delta, list balance_due sum == tile total delta. Rule it guards: the `aging=` status sets in `api/v1/invoices/index.php` must equal `api/v1/invoices/kpis.php`'s.
- D131 — S-LOCAL-DAY-TS 2026-09-17 added `tests/_smoke_local_day_timestamps.php` (17): helper DST cases + real `payments/kpis.php`, `notifications/index.php?date_range=today`, `TokenTracker` budget window and a real `app/auth/forgot_password.php` POST → `reset_password.php` reader, each in a rolled-back CLI subprocess with the SQL clock pinned past the UTC rollover (R section auto-skips unless mail is in log mode). `_smoke_qbo_payments_embed.php` fixtures now `gmdate` — Pacific-wall-time fixtures made C11 call the live QBO sandbox. Rule: UTC DATETIME windows use `ff_local_day_start_utc()`; PHP writes into DATETIMEs use `ff_now_utc()`.
- D131 — S-UTC-STAMPS 2026-09-17 added `tests/_smoke_utc_stamps.php` (12): `LocalStampMigrator` value mode on real rows across both DST transitions (last PST second, spring-forward gap, ambiguous fall-back hour, post-fall-back PST), values at/above the cutover untouched, settings-marker idempotency, ON UPDATE `updated_at` pinned; row-filter mode for future-dated expiries (pre-deploy row converts, post-deploy row untouched, missing predicate refused); `ff_now_utc('+24 hours')` exactly 86,400s and equal to SQL `UTC_TIMESTAMP()`; `CreditMemoPusher::buildQboPayload` TxnDate = local day for a 10pm-Pacific credit note. All inside one rolled-back transaction; clears any `data_migration.utc_stamps.*` markers inside it so it also passes on a database the real repair already ran on.
- D131 — S-QBO-GOLIVE-AUDIT 2026-09-23 added `tests/_smoke_qbo_golive_audit.php` (42): one rolled-back transaction — currency guard (C1-3), realm guard + reset (C4-9, C39), Qty/UnitPrice reconcile + `is_credit` sign (C10-11), JE Entity on AR/AP (C12-13), legacy + CloudEvents webhook envelopes + FF-echo skip (C14-16), worker release / transient policy / no requeue of payload failures (C17-18, C30), requestid determinism (C19-20), encrypted secrets + minorversion 75 (C21-22), pay-link source (C23), post-send enqueue states + pending-job dedupe (C24-26, C29), damage bridge types (C27), drift cutover filter + pagination (C28), linker matcher tiers + link/unlink refusals (C31-33, C37), linked-document pusher gates (C34), pre-go-live guard (C35), QBO payment import + re-sync (C36, C38), shared-file drift ownership (C40), per-rate tax (C41), rounding settlement (C42). NEW helper `tests/_qbo_smoke_env.php`: `ff_qbo_smoke_env([...])` pins `quickbooks.*` settings for a run and restores them at shutdown; `ff_qbo_smoke_preserve_tables([...])` snapshots + restores whole tables. **Trap (hermeticity vs a live-configured install):** push smokes written against a never-connected dev DB assumed empty mappings / no go-live date — against a mapped install they pushed to the real sandbox, unlinked real critical accounts and leaked GL rows. Smokes that need an unmapped/un-cut-over state must CREATE it (and restore it), never assume it; any smoke deleting a payment deletes its `acc_journal_entries` (source_type='payment') first. `_smoke_qbo_invoice_push` C16 / `_smoke_qbo_journal_entry_push` C27 / `_smoke_qbo_account_mapping` C13 / `_smoke_payment_overalloc` / `_smoke_qbo_payments_embed` / `_smoke_qbo_payment_webhook` updated accordingly.
- D131 — S-QBO-BILLPAY-MIRROR 2026-09-23 added `tests/_smoke_qbo_bill_payment_mirror.php` (19): one rolled-back transaction, QuickBooks answered by `QboFixture::cannedGet()` (NEW — realistic GET bodies per endpoint, cleared by `reset()`) so `BillPaymentWebhookHandler::handle()` runs end to end — plan (C1), bank-account resolution (C2), Create / replay / Update-unchanged / Update-remirror / Void (C3-7), FF-push echo (C8), unlinked bank account + drift dedupe (C9), $0 vendor-credit application (C10), FF-pushed payment edited in QBO (C11), bill already paid (C12), enqueuer + pusher origin gates (C13-14), `ApPaymentService::void` refusals (C15), go-live import from a Bill's `BillPaymentCheck` LinkedTxn (C16), catch-up sweep + idempotent re-run (C17), unlink guard (C18), webhook routing (C19). **Trap (smoke sweep on the live-configured dev):** `_smoke_payment_overalloc` / `_smoke_cash_flow_ties` / `_smoke_bank_match_guard` enqueue QBO pushes for payments they then delete, and `_smoke_qbo_queue` runs the worker over the whole queue — together they leave `failed` queue rows, 'FF payment N not found' drift events and push_failed notifications, and make `_smoke_qbo_queue` C9 fail when run after them (passes solo). Snapshot `acc_qbo_sync_queue` / `acc_qbo_drift_events` / `notifications` before such a sweep and clean up after.
- D131 — S-QBO-CUSTOMER-TERMS-ADDR 2026-09-23 added `tests/_smoke_qbo_customer_terms_addr.php` (7): rolled back; `TermResolver::dueDays` (C1) + `match` name-first / unique-days / no guess (C2), `CustomerPusher::buildQboPayload` billing address → BillAddr lines + ShipAddr (C3), fixture-mode create sends SalesTermRef + BillAddr and sets `ff_created_in_qbo` (C4), linked-customer update sends no BillAddr/ShipAddr/SalesTermRef/DisplayName (C5), FF-created update keeps the address in sync (C6), no term match → no SalesTermRef (C7). Sent payloads are read from `acc_qbo_sync_log.request_payload` → `body` (outside the worker the log row has no entity_id — match on a sentinel value). `QboFixture::cannedGet('query:<Entity>', …)` answers every fixture query FROM that entity. **Leak note:** `_smoke_qbo_fixture_pipeline` / `_smoke_qbo_drift_check` leave 2 'Customer push failed drift detected' notifications ("Synthetic injected failure for offline demo") per run on this dev.
- D131 — S-QBO-INVOICE-WRITEOFF 2026-09-23 added `tests/_smoke_qbo_invoice_writeoff.php` (12): rolled back; `InvoiceWriteOff` bad-debt + damage paths and refusals (C1-3), enqueuer gates (C4), pusher preflight blocks (C5), fixture push of CreditMemo + $0 apply Payment read back from the sync log (C6), idempotency (C7), failed apply step resumes without a second memo via `QboFixture::injectError('payment:create', …)` (C8), webhook echo/void (C9), drift ownership (C10), JE bridge filter (C11), Bad-debt item (C12). Changed expectations: `_smoke_qbo_golive_audit` C27 and `_smoke_qbo_journal_entry_push` C28 now REQUIRE `damage_writeoff` in `BRIDGE_DERIVED_SOURCE_TYPES`; `_smoke_qbo_item_mapping` C5 expects 29 tuples (+ `bad_debt`). **Fixture trap:** `QboFixture::pascalEntity` built the response key from the endpoint ('creditmemo' → 'Creditmemo'), so every fixture create of a CreditMemo / BillPayment / JournalEntry returned no Id under the real key — now mapped to Intuit's names. **Realm trap:** a new `acc_qbo_*_map` table must be added to `RealmGuard::REALM_SCOPED_TABLES` (golive C4 asserts list = schema).
- D131 — S-QBO-NAME-CLASH 2026-09-24 added `tests/_smoke_qbo_name_clash.php` (7): rolled back; `DisplayNameClash` naming/escaping (C1), vendor clash with another kind → "X (Vendor)" with CompanyName + PrintOnCheckName kept and the rename noted (C2), same-kind (inactive) holder → link refusal, no second create (C3), both names taken (C4), customer side (C5), failed lookup → no blind rename (C6), other errors untouched (C7). `QboFixture::injectError` now QUEUES per key (inject twice → the next two matching calls fail, in order). **Trap:** inject a 4xx, not a 5xx, to make a fixture call fail for good — the client retries transient 5xx/429 in web context, so one injected 503 is consumed by the first attempt and the retry succeeds.
- D131 — S-QBO-HISTPULL-SHARED 2026-09-24 extended `tests/_smoke_qbo_historical_pull.php` 22 → 24: C23 (shared company file → `assertLiveAllowed` / `startRun('live')` / `pullTransactional` refused with dry-run off + sync on, no sync-log row written = no QuickBooks call; unshared → gate opens), C24 (`status.php` `shared_file_block` + Manual Sync alert). The smoke's settings snapshot now includes `quickbooks.shared_company_file`. Note C6/C8 still pass on this dev, but now for the shared-file reason first (dev is shared='1').
- D131 — S-QBO-MONEY-JSON 2026-09-24 added `tests/_smoke_qbo_money_json.php` (4, pure): `QboMoney` rounding/validation (C1), `QuickBooksClient::encodeBody` exact under `serialize_precision=17` + ini restored (C2), no `=> (float)` in any `*Pusher.php` / `InvoiceTaxPerRate` payload (C3 — a new pusher must use `QboMoney::amount()`), both dispatch paths use `encodeBody` (C4). **Rule:** money into a QuickBooks payload = `QboMoney::amount($bcmathString)`; arithmetic on an amount read back from QuickBooks = `QboMoney::decimal($float)` first.
- D131 — S-QBO-ITEM-ACCOUNT-CHECK 2026-09-24 added `tests/_smoke_qbo_item_account_check.php` (8): `ItemAccountCheck::checkRow` on synthetic item rows with FF→QBO account links set in a rolled-back txn — ok / different / ff_account_unmapped / 'other' fallback / GPS gross+net / bad_debt / unknown item account / checkAll + Items API/page wiring. **Trap:** `AutoEntryBridge::resolveRevenueAccount` caches `accounting.revenue_account_map` in a static on first use — a smoke that changes the map must do so BEFORE the first resolver call in that process.
- D131 — S-SOP-MODULE 2026-09-24 added `tests/_smoke_sop_module.php` (10): every SOP chapter renders with complete front matter; every `{{… @/path}}` chip and checklist `@/path` is a real admin route and every `sop:` cross-link hits a real chapter + heading; diagrams are well-formed, token-coloured and used; the 15 month-end live checks run on the real schema with no error state; ticks + reads rolled back; role gates via simulated sessions; wiring. **Trap:** an Alpine `:disabled="a || obj[key]"` that evaluates to `undefined` DISABLES the element in this Alpine build — coerce to a boolean (`!!obj[key]`). **Trap:** `animateMotion` with a positive `begin` draws the circle at the SVG origin until it starts (a dot in the corner) — use a negative begin.
- D131 — S-QBO-INVOICE-PAYNOW 2026-09-24 added `tests/_smoke_qbo_invoice_paynow.php` (12): PaymentTerms parse/default; generated due dates follow terms (real `generateForLease` in a rolled-back txn); `TermResolver::matchDays`; `buildQboPayload` email/term/bill-to/PO/online flags; `CompanyInfoSync::poCustomField`; PayLink rules + button; `EmailService::withPayNow` (once, payable only); reminder pay_url; PDF pay box; dunning pay column (reflection on `renderHtml`); `PaymentBackstop` end-to-end against the fixture (cannedGet `cdc` + `payment/{id}`) → FF payment + invoice paid, other business ignored, no double import; wiring. **Trap:** `settings_write_qbo()` does NOT flush the settings cache — a smoke reading a key it just wrote must `settings_cache_flush()` first. **Trap:** `buildQboPayload` must stay offline (smokes call it directly) — anything that may call QuickBooks is resolved in pushImpl and passed in.
- D131 — S-REMINDERS-SMOKE-HERMETIC 2026-09-24 extended `tests/_smoke_customer_reminders.php` 20 → 26: portal opt-out checks moved into the rolled-back txn with `customer_notifications.respect_portal_optout` pinned (on → blocks, off → ignored); T1 reads registry `default_enabled`; T1b fresh-install fallback (settings rows deleted in-txn → every type OFF, opt-outs honoured); T2c schema-real `portal_users.notification_preferences` round-trip via the compliance cron's JOIN; T3 toggles `registration` (the retired `insurance` slug always reads OFF, so it proved nothing). **Rule:** dev is a production copy, so a smoke must never assert on a live operator setting. Pin every setting an assertion depends on inside its transaction. A "pure" predicate check can still read settings (`customerAllowed()` → `respectPortalOptOut()`).
- D131 — S-MODULE-CHROME 2026-09-24 added `tests/_smoke_module_chrome.php` (6): ModuleHero list/entity contract + escaping + class/accent sanitising + initials; every `lib/Ui/art/*.svg` well-formed, titled, colour-free, uses only defined `dg-*` classes and is used; `module-chrome.css` linked by BOTH header shells, token-only colours, no KPI top bar, reduced motion; every converted page calls the hero with its accent and has one breadcrumb source; all 10 pages + the embedded invoice RENDER as super_admin in a rolled-back subprocess with no page warnings; dispatcher hero hides the money fact, accountant sees it. **Trap:** Alpine's `x-show` restores visibility by REMOVING the inline `display`, so an `x-show` wrapper with `style="display:flex"` or `display:contents` comes back as `block` — put the display in a class (`.ff-contents`). **Trap:** `includes/header_embed.php` is a separate shell with its own stylesheet list — a new global stylesheet must be linked there too or embedded pages (batch invoicing) render unstyled.
- D131 — S-PAYOFF-SMOKE-DATA-INDEPENDENT 2026-09-24 made `tests/_smoke_payoff_design.php` (4) data-independent: equipment_show_linked/unlinked no longer hard-code unit ids 6/7 (soft-deleted in dev 2026-06-25). `resolve_unit_fixture()` picks a live unit with / without a non-disposed `acc_fixed_assets` row using show.php's own predicates (incl. the INNER JOIN on `equipment_templates`), reads the expected `linkedAssetId` with show.php's exact query (newest non-disposed asset) and passes the unit id to the child as argv[2]. No qualifying unit → `[SKIP]` (exit 0). **Trap:** an admin page that `header('Location')` + `exit`s inside a render-smoke child ends the child with exit 0 and ZERO output — the parent must treat empty child output as a failure, not parse it as 0 bytes. **Rule:** a render smoke must never hard-code row ids in the dev DB (a production copy / re-seeded demo set); resolve them from the page's own predicates. **Trap:** requiring `config/app.php` after the parent has printed anything warns ("Session ini settings cannot be changed after headers have already been sent") — do DB lookups before the first echo.
- D131 — S-DASH-CHART-REDACT 2026-09-24 extended `tests/_smoke_dashboard_authz_redaction.php` 4 → 13: new real-login subprocess harness (`auth_login()` of the test dispatcher/accountant/super_admin users, `FFSMOKE-FIN` = `can_view_financials()` precondition per role); chart cache cleared once, super_admin first so the dispatcher reads the cache it warmed; dispatcher combined `charts.php` omits all 6 `$moneyCharts` (incl. top_customers, weekly_heatmap) and keeps the 6 operational charts, `?chart=top_customers|weekly_heatmap` → 403 FORBIDDEN with no dataset; accountant + super_admin get all 12 and 200 on both. Pre-fix 10/3, post-fix 13/13. A new dashboard chart that sums invoice/payment amounts must join `$moneyCharts` in `charts.php` AND the smoke's mirror list.
- D131 — S-SOP-KNOWN-ISSUES 2026-09-24 added `tests/_smoke_sop_known_issues.php` (38, hermetic — one rolled-back txn, real services): I1 revenue resolver per category, I2 asset purchase/betterment entries + refusal without a basis, I3 AP reconciliation nets unapplied credits, I4 GST remittance clears 2030+1050 incl. refund + wrong-amount refusal, I5 transfer CAD values/foreign amount/spread, I6 year_end into a closed (not locked) month, I7 FX preview refuses closed, I8 opening balance post + re-post, I9 reversal block rules, I10 bank routing, I11 reconciliation formula (audit example), I15 unit-tagged direct cost vs overhead; `tests/_smoke_qbo_sync_controls.php` (14) and `tests/_smoke_dunning_email_gate.php` (19); updated `_smoke_qbo_journal_entry_push` C28 (bridge list + `bank_opening_balance`, `credit_note_refund`) and `_smoke_qbo_bill_payment_mirror` C19 (routing now in `WebhookReplay::dispatch`); `_smoke_doc_freshness` C4 raises `pcre.backtrack_limit` (the IN-FLIGHT block regex exhausted PCRE's default limit as SHIPPED entries accumulated — false red).
- D131 — S-MODULE-CHROME-2 2026-09-24 extended `tests/_smoke_module_chrome.php` to 26 pages (16 more module list pages render one accented hero each) and C3 now fails if `.ff-hero` sets `--acc`. **Trap:** `.ff-hero { --acc }` and `.ff-acc--x { --acc }` tie on specificity — the later rule wins silently, so every hero was primary; default accents via `:where()`.
- D131 — S-DASHBOARD-REDESIGN / S-DASHBOARD-VIZ 2026-09-24: `_smoke_module_chrome.php` C7 renders the dashboard as super_admin, accountant and dispatcher (money charts/cards only with financial access, each chart container once, retired chart ids gone, `?charts=` subset requested, walkthrough selectors intact); NEW `tests/_smoke_dashboard_viz_data.php` (52) checks every new dataset against independent SQL / ArAging, lease_flow/idle_units rules and the role gate; `_smoke_dashboard_authz_redaction.php` covers 9 money + 8 operational charts (22). **Trap:** ff-chart-theme's `rethemeAll()` pushes the default palette into every live chart on a theme toggle, overwriting per-series `colors` — pages with semantic colours must redraw on `ff:theme-changed`. **Trap (browser verification):** with the Browser pane hidden, `html{scroll-behavior:smooth}` never advances and scroll events never fire — set scroll-behavior auto and call scroll handlers directly.
- D131 — S-SHELL-REDESIGN 2026-09-24 added `_smoke_module_chrome.php` C8 (8 checks total): shell.css linked + token-only + never sets a sidebar width; every menu item has a valid accent and the main menu keeps its sections/items; a rendered page carries the sidebar brand/find/section/accent markup and the topbar context block + tray; no icon inside a nav item loses its `nav-icon` class. **Trap:** `heroicon()` caches by icon NAME — a custom class passed once leaks onto every later render of that icon on the page.
- D131 — S-SHELL-FIND-REMOVE / S-DASHBOARD-ATTN-2 / S-SETTINGS-REDESIGN / S-TABLES-REDESIGN / S-BACKGROUNDS / S-LIST-COMPACT / S-RECORD-REDESIGN / S-PAYOFF-ONE-PAGE 2026-09-24 took `_smoke_module_chrome.php` to **14 checks** (C8 fails if the removed Find-a-page box returns; C7 checks the kpis `open_service_requests` field and the requests / idle cards): C9 settings nav ↔ `$tabPermMap` ↔ panels, Lockout super-admin only, the nav never calls `FF_TabHash.onSwitch` (page-top jump); C10 tables.css in both shells, token-only, only short-value columns nowrap (the JS header regexes are run against real labels), every column class has CSS, overflowing tables pin actions; C11 backgrounds registry ↔ stylesheet ↔ 9 shells ↔ brand API ↔ Design picker, dark never leaks into light, palettes set only `--sidebar-bg`; C12 list tabs + compact pager in the toolbar, leases strip and `?focus=` share `_focus.php`, full-size hero, separate tiles; C13 records.css + RecordUi contract, customer/lease/invoice/unit pages carry layout/rail/More/sticky tabs with every tab visible (no `.tab-more`, `.tab-anchor` before the bar), embed invoice has no rail; C14 payoff one page (redirect, `?detail=1`, USD conversion, tab hidden from dispatchers). C6 now checks the customer money strip/Account card gate. `_smoke_payoff_design` markers follow the sticky tabs + merged sections; `_smoke_lease_show_generate_invoice_button` accepts `btn-sm`. **Traps:** (1) the Atelier `.card` has its own 24px padding — cards with header/body double-indent unless the page zeroes it; (2) a page's own `<style>` loaded after tables.css re-declares `.spec-table` rules — out-rank with `table.`/`.card.` prefixes; (3) `FF_TabHash.onSwitch()` scrolls to the page TOP — wrong for any sticky/side nav (use `onSwitchKeep`); (4) the key-number strip styling only reaches a `.stat-grid.ff-stats` that is a later sibling of `.ff-hero` (or a direct child of one) and C3 bans pseudo-elements on `.ff-stats`; (5) records.css `display:flex !important` on More-menu rows beat Alpine's inline `display:none` — hidden rows need the explicit `[style*="display: none"]` rule; the hero's `overflow:hidden` clipped the menu until `:has(.rec-more > [aria-expanded=true])` opens it; (6) header buttons calling Alpine methods need the component's `x-data` opened above the ModuleHero render.
- D131 — S-SIDEBAR-LOGO / S-TOPBAR-CHROME 2026-09-24: NEW `tests/_smoke_sidebar_logo.php` **18 checks** (real images through `StorageClient`: black canvas → trimmed + dark + icon mark; white canvas → light, no mark; transparent → clear; photo corners → untrimmed, capped; SVG + missing file fall back, the missing-file fallback retries after the window; endpoint serves only the current logo, immutable; sidebar/CSS hooks) — hermetic (zz-smoke-* keys + their cache files deleted). `_smoke_module_chrome` C8 +4 assertions (topbar dark chrome out-ranks the no-blur fallback; controls re-tokened; never `.topbar` itself or a dropdown — mutation-tested). **Traps:** (1) re-tokening `.topbar` itself darkens every menu (they are descendants) — override on the controls, menus are siblings of their triggers; (2) app.css's `@supports not (backdrop-filter)` `[data-theme="light"] .topbar` (0,2,0) out-ranks a bare `.topbar`; (3) the dev DB's `brand.logo_path` points at a file that only exists on prod (dev renders the truck-tile fallback) — test logo work with a stand-in under that key, then delete it; (4) a 12000×6750 PNG needs ~400 MB in GD — `BrandLogo` refuses >24 MP and falls back to the original.
- D131 — S-CCA-PREFILL 2026-09-24: NEW `tests/_smoke_cca_prefill.php` **47 checks** (pure mapping of `form_data` and the customer row onto the form's input names; real HTTP GET/POST of `/credit-application?token=` on fleetforge.test: first send pre-fills from the customer record HTML-escaped, needs-info re-send prefers the last submission with customer-record gap fill, POST with errors keeps a cleared field cleared, no cross-customer leak, soft-deleted previous ignored, expired link shows no data, signature/date/printed name/terms never pre-filled). Hermetic: zz-smoke customers + their applications + the one rate-limit bucket deleted in `finally`. Needs the Herd dev site up.
- D131 — S-PDF-LETTERHEAD 2026-09-24: NEW `tests/_smoke_pdf_letterhead.php` **53 checks** (PdfKit address de-dup, logo presentation on real GD images, bcmath money/period/filename, render with/without logo; InvoicePdfGenerator old-layout/missing-file rebuild + reuse; HTTP as a minted super admin: invoice PDF inline/download/404, statement, P&L, balance sheet, dunning letter stored/lost; credit application rebuilt from its snapshot + documents row repaired; portal (session files in /var/tmp): own invoice/application stream, another customer's → 404, non-advance draft → 404; source guards — no generator builds its own mPDF, no pop-up-after-await, no presigned PDF URLs, pdf.* settings read + editable). Hermetic (zz-smoke-pdf rows + stored files deleted, the exercised invoice's pdf_* columns and pdf.show_logo restored). `_smoke_portal_payments_cca` T12 now asserts the scoped portal PDF endpoint instead of `StorageClient::url()`.
- D131 — S-BILLING-MODULE 2026-09-24: NEW `tests/_smoke_billing_module.php` **98 checks** (one rolled-back txn, invoice counter bumped first): schema + moved settings (B1); cycle month helpers, ensure() idempotent, arrears/advance target, coverage = universe (B2); all 26 readiness checks run on the REAL schema with none erroring — the column-drift guard for every query in `BillingReadiness` (B3); review flags + marks, mileage_only never duplicate_period (B4); holds create/heldMap/activeFor/duplicate/release (B5) and refused by the REAL `BatchPreviewService` (B6); readings in km, below-previous refused, month-end-only `generatorParams`, and the REAL dry run's period-end odometer = the reading (B7/B8); close hard/soft/override, `closedCycleFor` lock, reopen (B9); register width + CSV formula neutralising + categories = pre-tax subtotal (B10); `InvoiceDelivery` recipient order (B11); `ff_run_billing_cycle_open()` opens once (B12); static wiring — every billing endpoint gated, money endpoints consult `can_view_financials()`, workbench endpoints honour holds/readings/lock, bulk_send uses InvoiceDelivery, nav/redirects/Re-send/audit gate (B13). **Trap:** lib classes called from a cron must not call `current_user*()` (auth.php is not loaded in CLI) — use `BillingCycles::actor()`. **Trap:** a close-time `mileage_only` / `adjustment` invoice legitimately shares days with the rental invoice — "billed" and "double billing" tests must use `BillingCycles::RENTAL_BILLING_TYPES`.
- D131 — S-RATES-MODULE 2026-09-24: NEW `tests/_smoke_rates_module.php` **108 checks** (R1–R6 in one rolled-back txn; R8 read-only HTTP against fleetforge.test with minted sessions copied to `/var/tmp`, dry runs only): R1 `RateResolver` parity with the legacy inline lookup SQL on EVERY customer × equipment type (the guard that the lease form's pre-fill did not move); R2 lookup_rates response keys/order; R3 `RateCardItems` messages (D132 trio, column limits, min days), snapshot/diff/adjust/rounding/status/validDate; R4 `explain()` tiers + reasons, `quote()` = `HolisticLeaseEngine::cumulativeCorrect` with the category-gated minimum; R5 `RateCardRevision` dry run untouched, apply ends-day-before + flag move + audit links, all-or-nothing on a ConflictGuard refusal, $5-rounding D132 refusal; R6 `RateInsights`; R7 endpoint gates + shared rules + assets + no interpolated sort; R8 every GET 200, create/revise dry_run, dispatcher 403, clean CSV. `_smoke_hourly_gps_rates` T16b now reads the shared rules. **Trap:** `RateInsights` caches templates/standards in statics — call `RateInsights::reset()` after seeding equipment in a test. **Trap:** a dry run that must leave nothing behind runs inside `SAVEPOINT rc_rev_all` (plus its own transaction when none is open) — never rely on `db_transaction()` for rollback-on-success. **Trap:** FF_TableFit tags every table, and `.ff-act > tbody > tr > td:last-child` right-aligns a colspan detail row's content — pin `text-align:left` on nested tables inside a row.
- D131 — S-PORTAL-REDESIGN 2026-09-24: NEW `tests/_smoke_portal_redesign.php` **19 checks**, HTTP against fleetforge.test with a hermetic temp portal user (session file in `/var/tmp`) on the busiest real customer: R1 all 16 portal pages render with no PHP warning; R2 portal JSON endpoints 401 JSON without a session; R3 payable rows = `pt_account_summary()` count + balance; R4–R6/R9/R12–R14 Trap 8 with another customer's invoice/payment/document ids (payable, zip, receipt, search, documents page + file stream, payments/go); R7 CSV BOM + one row per invoice; R8 statement PDF + bad range 422 + staff endpoint shares `CustomerStatement`; R10 payment notice (CSRF 403, amount 422, foreign invoices dropped, billing_inquiry request, NO payment row); R11 request conversation; R15 initiate_qbo_payment error text is customer-safe; R16 reply > 5,000 → 422; R17 reminder prefs round-trip; R18 wiring (portal.js before Alpine, `data-no-auto-label` on every `.pt-table`, no dead serve.php link, messenger guard, no stray hex in portal.css, every `pt_icon()` exists). Rebased to the new markup (same intent): `_smoke_portal_payments_cca` T3/T4/T8–T10, `_smoke_portal_request_notifications` C1/C19/C37/C38, `_smoke_qbo_payments_embed` C22/C24. **Trap:** app.js auto-adds `.table-stack` (a swipe carousel, `!important` everywhere) to every table on phones — portal tables opt out with `data-no-auto-label`. **Trap:** CLI `php://input` is empty, so portal POST endpoints are smoked over HTTP, not by requiring the file. **Trap:** `_smoke_portal_api_suspended_lockout` picks portal user 1, whose customer is soft-deleted on dev — its 2/5 is a fixture problem, not a regression.
- D131 — S-AI-KNOWLEDGE 2026-09-24: NEW `tests/_smoke_ai_knowledge.php` **50 checks** (A one shared prompt: chat.php + stream.php call `ChatPrompt::build`, no `<<<PROMPT` left, role/today/page map/page path, Making-changes follows `ai.write_enabled` both ways in a rolled-back txn, `cleanPagePath` rejects prose + other hosts; B `search_help` finds the right guide/SOP section for 5 real questions, catalog = every docs/help + docs/sop file, path tricks ignored; C every registered tool dispatches, names unique, knowledge first; D revenue = sent only, trial balance before the first JE is empty, unit NUMBER resolves to that unit, unknown number filters to nothing, customer rates priced, invoice/customer status enums = DB; E dispatcher: rates tools denied, unit/maintenance/vendor/lease-balance money hidden, lease rates visible, prompt says no money, page map hides Payments; F stream passes the session + emits `proposal`, /ai page handles it, widget sends page_path). NEW `tests/_smoke_ai_billing_tools.php` **50** (5 billing tools, admin + dispatcher, `get_billing_cycle` writes nothing — billing_cycles row + 7 table counts identical; holds/charges fixtures in a rolled-back txn). NEW `tests/_smoke_ai_ops_tools.php` **90** (4 ops tools, no secret/token/hash fields or stored secret values in any output, dispatcher/accountant QBO refusal). `_smoke_ai_write_records` refusal check accepts "switched off". **Trap:** `(int) "36V203" === 36` — any AI tool taking a unit id must go through `resolveUnitId()`. **Trap:** `BillingReadiness::run()` WRITES (stamps readiness_checked_at = Prepare done); read-only callers use `evaluate()`.
- D131 — S-AI-SUMMARY-MONEY-KEY 2026-09-24: NEW `tests/_smoke_ai_summary_money_tier.php` **30 checks** — A in-process (stubbed super_admin/dispatcher sessions, one rolled-back txn, ai.enabled off so a miss never bills): `ai_summaries.with_money` NOT NULL DEFAULT 1 + in `uq_entity_type`; `viewerWithMoney()` / `moneyTierFor()` (null user → 1); admin write → with_money=1; dispatcher `cachedSummary()` + `generate()` MISS the admin row; dispatcher row separate, its regenerate leaves the admin row; column-less legacy insert → tier 1, invisible to the dispatcher. B runs the REAL `api/v1/ai/summary-stream.php` in a subprocess per REAL `auth_login()` (54/57/56; `FFSMOKE-FIN` precondition; POST body via php:// stream wrapper; txn opened before login so settings/rate-limit writes roll back) against a committed admin-generated sentinel row (entity_id 2000000703, deleted in `finally`): financial roles get the cached text, the dispatcher gets none and ends at "AI features are not enabled". `_smoke_ai_low_fixes.php` C reads now pass tier `true` explicitly (8/8).
- D131 — S-RATES-MINIMUM-OVERLAY 2026-09-24: NEW `tests/_smoke_rates_minimum_overlay.php` **46 checks** (M1–M10 in one rolled-back txn on synthetic category slugs: minimum-only line over type defaults / over a lower priced card / below a priced line (no effect) / customer minimum over general / minimum 0 counts / none tier keeps the minimum / empty line skipped; `hasPrices` / `priceWinnerIndex` / `minimumIndex`; `explain()` `used_for` + `priceless` + `minimum_card_id` + tier states; `quote()` bills the overlaid minimum on the type's prices; `RateInsights` priced card name, `leasesOnCard` never on a minimum-only card, `lineIssues` shapes incl. the prod #64 all-$0/min-0 line; M11 the REAL `api/v1/leases/lookup_rates.php` in a child process on committed-then-deleted fixtures = `resolve()` with exact keys). `_smoke_rates_module.php` → **109 checks** (R1 parity counts minimum-only overlays separately and requires the legacy winner's minimum to carry over; R6 attention flags an empty line and NOT a minimum-only line).
- D131 — S-CHAT-REBUILD 2026-09-24: NEW `tests/_smoke_chat_rebuild.php` **99 checks** (A schema: 5 conversation tables, 8 retired tables gone; B DMs: one per pair either order, member-only, CRLF/trim, unread 2 → markRead 0, mark never moves back, bell coalesced to 1 + cleared on read, unsend own only + placeholder + preview; C groups; D customer thread: one per customer, portal own-customer only, customers.view gate; E records: customer-thread types = lease/invoice/payment, portal-visible only (drafts/void refused for staff AND portal), other customer's refused, forged type refused, max 5, dispatcher card without amount + payment "Not available", live status Paid/void, portal card links /portal/…; F unread: customer-only counting in customer threads, portal unread; G all 8 searches on the real schema; H static: no code references retired tables/endpoints/components, every new file present + lints). All in one BEGIN/ROLLBACK.
- D131 — S-CHAT-DELETE 2026-09-24: `tests/_smoke_chat_rebuild.php` extended 99 → **118 checks** (section I: Delete chat is mine-only — other member / colleague / customer keep every message, bell cleared, a new message brings it back with only the new message, double delete harmless, empty DM stays hidden; Leave group removes access, the last leaver deletes the group + messages).
- D131 — S-CHAT-SEEN 2026-09-24: `tests/_smoke_chat_rebuild.php` extended 118 → **134 checks** (section J: Sent → Seen, receipts only on my side's newest message, gone after a reply, unsent → none, deleting a chat is not reading it, group "Seen by <first name>" → "everyone", customer thread "Seen by <portal user>" incl. for colleagues, portal "Seen" with staff unnamed).
