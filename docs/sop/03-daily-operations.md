...
4. **Rate cards**: {{Rates › New rate card @/rates/create}}. Choose **One customer** (and pick them) or **Everyone** for a standard price list, tick the equipment (each line starts from today's price), set daily, weekly and monthly (all three, or none), distance, engine-hours and GPS prices and minimum days, the dates, then **Create rate card**. A line with only a minimum days (no prices) adds that minimum on top of the next price in line (usually the equipment type's own prices); it never blanks the prices. Before quoting a price, use {{Rates › Price check @/rates#check}}. To change prices from a date, open the card → **Change prices** → **Preview** → **Save new prices** (the old prices end the day before).
