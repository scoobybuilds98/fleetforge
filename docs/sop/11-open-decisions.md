...
- [ ] **F84 — Process rules, in writing.** Rental invoices and credit notes are created in FleetForge only, and credits are applied in FleetForge (an application made in QuickBooks is not copied back). Customer payments are recorded in QuickBooks only. Bills live in one place, with each bill payment in one place. The accountant stops booking fleet depreciation in QuickBooks. Someone checks QuickBooks → Drift weekly.
- [ ] **F85 — Revenue account plan.** FleetForge books rental revenue by equipment category (4010–4050) and mileage to 4060; engine hours, cartage, sweep, wash and fuel still go to 4110 Other Revenue until mapped. QuickBooks items all post to one Sales account. Decide one of: one Sales account (set **Rental — every category** and the other rows in Revenue Mapping to the account linked to Sales); or separate accounts per kind of revenue, matched in QuickBooks items and linked in FleetForge.
