...
### F100 — Deploy the new Messages, then text a customer from it 🟡 OPEN (next deploy)
...
**Operator action:**
1. Deploy `main` and apply the migration in the same step (`sudo /var/www/fleetforge/bin/deploy.sh`, which runs `php bin/migrate.php --apply`; confirm `202609250300_S-AI-SUMMARY-MONEY-KEY_with_money_cache_key.sql` shows as applied). Deploying the code WITHOUT the migration makes every AI summary card fail to read or write its cache.
