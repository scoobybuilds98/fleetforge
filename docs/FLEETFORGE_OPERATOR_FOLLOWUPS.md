...
**Operator action:**
1. Deploy `main` and apply the migration in the same step (`sudo /var/www/fleetforge/bin/deploy.sh`, which runs `php bin/migrate.php --apply`; confirm `202609250500_S-CHAT-REBUILD_conversations.sql` is applied). The migration **drops the old chat and messenger tables** after copying staff direct messages across. Prod had one message; the deploy's pre-migration backup is the undo. New CSS/JS (`chat.css`, `chat.js`): if prod serves assets by `.env` `FF_ASSET_VERSION` rather than the git hash, bump it.
2. Open **Messages** (chat bubble in the top bar). Check that the old "what's up" direct message is there, then send a teammate a message with an invoice attached (paper clip).
