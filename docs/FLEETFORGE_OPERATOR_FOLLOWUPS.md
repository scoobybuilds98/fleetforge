...
2. Open **Messages** (chat bubble in the top bar). Check that the old "what's up" direct message is there, then send a teammate a message with an invoice attached (paper clip). The trash icon at the top of a conversation is **Delete chat** (removes it from your Messages only) or **Leave group**.
