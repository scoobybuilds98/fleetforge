...
### F51 — Deploy S-LUX-1 (Atelier design foundation) + bump prod FF_ASSET_VERSION 🟢 DEFERRED (non-blocking, cosmetic)

**Surfaced by:** S-LUX-1 (2026-07-11).
**Affects:** visual layer only — fonts, palette, component chrome. No schema, no PHP logic, no billing.
**Operator action:**
1. `git pull` + `ff-deploy` as usual.
2. Bump `FF_ASSET_VERSION` in the prod `.env` (dev went 1.0.46→**1.0.47**) so browsers drop the cached pre-Atelier `app.css`.
3. Spot-check the two font URLs return 200 under nginx (filenames contain literal `[wght]` brackets; nginx decodes %5B→`[` before file lookup so both request forms should work, but verify once):
   `curl -g -so /dev/null -w '%{http_code}' https://mainlandrentals.com/assets/fonts/Geist[wght].woff2`
4. Eyeball dashboard + one lease + one invoice in both themes; the Design-tab brand color (#2596be currently on dev) keeps working — buttons/focus/active-nav all follow it now.

**Why deferred:** pure CSS/font change; old cached CSS keeps rendering the previous design until the asset version bump, nothing breaks.
