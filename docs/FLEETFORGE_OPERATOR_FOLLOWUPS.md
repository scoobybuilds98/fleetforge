...
2. **(Fixed by S-SOP-KNOWN-ISSUES for new invoices: rental now books per equipment category 4010–4050 and mileage to 4060; the Revenue Mapping tab edits the map posting reads.)** Before that fix, **FleetForge put most revenue in 4110 "Other Revenue".** Its revenue map (Accounting → Settings → Revenue Mapping) has rental keys per equipment category (`base_rental_chassis`, `base_rental_dry_van`, …) but invoice lines are typed plain `base_rental`, so all rental revenue ($54,079 on the production copy), plus estimated / actual mileage usage, engine hours, cartage, sweep, wash and fuel, falls back to 4110.
...
### F102 — Connect WhatsApp, then have staff turn it on 🟢 OPTIONAL (after F101; ships off)

**Surfaced by:** S-ATTENTION-WHATSAPP (2026-09-25), from the operator's "any way we can get all the notifications, leases, invoices, credit applications on a whatsapp group or broadcast with all our team members?"
**Why:** staff rarely open FleetForge (most prod managers last logged in June–August), so urgent items need to reach phones. It's built on Meta's official WhatsApp Business Platform: 1:1 messages from a business number (a WhatsApp group would hold only 7 staff and show everyone every amount). Cost ≈ $0.004 USD per message — roughly $2/month at prod's volume. Nothing is sent until this is done.
**Operator action:**
1. Deploy (the F101 deploy covers it if done after this commit): migration `202609250900_S-ATTENTION-WHATSAPP_deliveries.sql` adds the WhatsApp columns + delivery table.
2. Add to **www-data's** crontab: `* * * * * /usr/bin/php /var/www/fleetforge/cron/whatsapp_dispatch.php >> /var/www/fleetforge/logs/cron.log 2>&1` (does nothing while WhatsApp is off).
3. In **Meta Business Suite → WhatsApp Manager**: add a phone number for FleetForge that is NOT used in the WhatsApp app, and verify the business.
4. Create the two **message templates** exactly as shown in **Settings → Notifications → WhatsApp → How to connect** (category **Utility**, language **English**): `fleetforge_alert` (4 variables) and `fleetforge_summary` (5 variables). Wait for approval (usually minutes to a day).
5. In **Meta for Developers** → your app → WhatsApp → **API setup**: copy the **Phone number ID**; create a **System User** permanent token (`whatsapp_business_messaging`, `whatsapp_business_management`); copy the **App secret** (App settings → Basic). Paste all three into Settings → Notifications → WhatsApp (the token and secret are stored encrypted and never shown again).
6. Webhook (for delivered / read / failed in the log): in WhatsApp → Configuration, Callback URL = the one shown on the card (`https://mainlandrentals.com/fleetforge/api/v1/webhooks/whatsapp.php`), Verify token = click **Generate** on the card and paste the same value; subscribe to **messages**.
7. Tick **Send staff WhatsApp messages**, **Save WhatsApp**, put your own number in **Profile → Notifications**, then **Send me a test**. A failure shows Meta's reason in plain words.
8. Ask each person to open **Profile → Notifications → WhatsApp**, enter their number and choose **Urgent items + morning summary** (or summary only). Turning it on themselves is the opt-in Meta requires.
9. Optional: in Settings → Notifications, change which kinds go **Right away / Morning summary / Never**.
**Done when:** the test message arrives, and the team has turned it on.

---

### F101 — Deploy the new notifications (Needs attention + Updates), add its hourly job, then switch over 🟡 OPEN (next deploy)

**Surfaced by:** S-ATTENTION-INBOX (2026-09-25), from the operator's "redesign the notifications … so we dont lose hold of imp notifications and they dont get lost in the noise".
**Why:** on prod every manager has ~7,000 unread notifications and none has ever opened one; the few real problems (overdue customers, a credit application, a customer request) are buried under repeats. The bell now has **Needs attention** (one shared item per problem, until it's fixed) and **Updates** (activity, never counted). Until the switch-over script runs, the old 75k rows still sit in everyone's Updates as unread.
**Operator action:**
1. Deploy `main` and apply the migration in the same step: `sudo /var/www/fleetforge/bin/deploy.sh` — confirm `202609250800_S-ATTENTION-INBOX_attention_items.sql` is applied (2 new tables + 2 columns on `notifications`). Don't pull the code without the migration: the bell reads the new tables. New CSS (`attention.css`) — if prod serves assets by `.env` `FF_ASSET_VERSION` rather than the git hash, bump it.
2. Add the hourly job to **www-data's** crontab: `5 * * * * /usr/bin/php /var/www/fleetforge/cron/attention_sweep.php >> /var/www/fleetforge/logs/cron.log 2>&1` (it keeps the list current, brings snoozed items back, escalates urgent items nobody takes). It is not switchable from Settings on purpose.
3. Switch over — dry run first, then apply: `sudo -u www-data php /var/www/fleetforge/scripts/notifications_switchover.php` (prints what it would do), then add `--apply`. It archives the old alert-type rows (soft delete, kept for history), marks the old updates read, and builds today's list (expect ~20 items: 14 overdue customers, units with documents expiring this week, the open customer request). It refuses to run twice.
4. Open the bell as yourself: **Needs attention** should show the real items; try **Take it** and **Snooze**. Open **Settings → Notifications** and check which roles see which kind (defaults: money → managers + accountants, fleet → managers + dispatchers, customer requests → Portal & Requests routing, balance check → super admins only). Change the escalation time if 24 hours is wrong.
5. Tell the team: the bell's number is now the team's to-do list, not every event; take what you handle, and "Done" needs a one-line note if the problem is still there. Their own update categories are under **Profile → Notifications**.
6. Re-record training chapters 1 (the bell) and 25 (notifications) with F90.
**Done when:** deployed + migrated, the cron line is in, the switch-over has run, and the list on prod shows the real open problems.

---

### F100 — Deploy the new Messages, then text a customer from it 🟡 OPEN (one more deploy + checks + decision)

**Surfaced by:** S-CHAT-REBUILD (2026-09-24), from the operator's "completely rebuild the chat feature … a simple texting divided by team and customers … add any lease/invoice/payments,etc in the chat".
**Why:** Messages replaces Team Chat, Messenger and the mini chat widget with one simple screen (Team | Customers) where any record can be attached as a live card. Customers get one **Messages** page in the portal.
**Operator action:**
1. ✅ **Done 2026-09-24** — prod verified read-only at 18:25 UTC: HEAD `ef6c6f7` (S-CHAT-SEEN), both chat migrations applied (17:53 / 17:58 UTC), 0 pending, php-fpm reloaded after the pull, old chat/messenger tables gone, the one DM carried over, live `chat.js`/`chat.css` current, `/messenger` 404. Original step: deploy `main` and apply the migration in the same step (`sudo /var/www/fleetforge/bin/deploy.sh`, which runs `php bin/migrate.php --apply`; confirm `202609250500_S-CHAT-REBUILD_conversations.sql` and `202609250600_S-CHAT-DELETE_cleared_message_id.sql` are applied). The migration **drops the old chat and messenger tables** after copying staff direct messages across. Prod had one message; the deploy's pre-migration backup is the undo. New CSS/JS (`chat.css`, `chat.js`): if prod serves assets by `.env` `FF_ASSET_VERSION` rather than the git hash, bump it.
1b. **Deploy again for the seen time (S-CHAT-SEEN-TIME, 2026-09-25):** `sudo /var/www/fleetforge/bin/deploy.sh` — it applies `202609250700_S-CHAT-SEEN-TIME_last_read_at.sql` (one new column; the deploy backs up first). Don't pull the code without the deploy script: until the column exists, opening a conversation fails.
2. Open **Messages** (chat bubble in the top bar). Check that the old "what's up" direct message is there, then send a teammate a message with an invoice attached (paper clip). The trash icon at the top of a conversation is **Delete chat** (removes it from your Messages only) or **Leave group**. Under your newest message you'll see **Sent**, then **Seen** once it's read.
3. On a customer who has a portal user, choose **More → Message customer** and send a short hello. Sign in to the portal as that customer (Users → Portal) and check it shows under **Messages**, with a notification.
4. **Decision — email for new messages:** customers only see new staff messages when they next sign in (portal bell + Messages badge). Nothing is emailed. Say if you want a "you have a new message" email; it would be a new Customer Emails type that ships off, like the others.
5. Re-record training chapter 25 (Email, Messaging & Notifications) with F90. Its script now walks through Messages.
**Done when:** deployed, one team and one customer message sent on prod, and the email decision made.

---

### F99 — Deploy + migrate the AI summary money fix 🟡 OPEN (next deploy)

**Surfaced by:** S-AI-SUMMARY-MONEY-KEY (2026-09-24).
**Why:** until this ships, an AI summary generated by an admin or accountant (balances, amounts, revenue) can be shown from the cache to a dispatcher for up to 24 hours. The fix needs its migration — the new code reads a `with_money` column that only exists after `migrate --apply`.
**Operator action:**
1. ✅ **Done 2026-09-24** — prod verified read-only at 18:25 UTC: HEAD `ef6c6f7` (S-CHAT-SEEN), both chat migrations applied (17:53 / 17:58 UTC), 0 pending, php-fpm reloaded after the pull, old chat/messenger tables gone, the one DM carried over, live `chat.js`/`chat.css` current, `/messenger` 404. Original step: deploy `main` and apply the migration in the same step (`sudo /var/www/fleetforge/bin/deploy.sh`, which runs `php bin/migrate.php --apply`; confirm `202609250300_S-AI-SUMMARY-MONEY-KEY_with_money_cache_key.sql` shows as applied). Deploying the code WITHOUT the migration makes every AI summary card fail to read or write its cache.
2. Nothing else: existing cached summaries stay and are shown to financial roles only; a dispatcher's first summary on each record is generated fresh (one AI call) and cached for dispatchers from then on.
**Done when:** the migration is applied on prod.

---

### F98 — Deploy the updated AI assistant, try it, and decide whether AI may propose changes 🟡 OPEN (next deploy)
...
### F95 — Deploy Monthly Billing, add its scheduled job, then run October as the first cycle 🟡 OPEN (next deploy)

**Surfaced by:** S-BILLING-MODULE (2026-09-24), from the operator's "a billing module which handles the monthly billing cycle A to Z".
**Why:** the month's billing now runs in **Billing → Monthly Billing** as a cycle (readiness checks → readings → workbench → review → approve → send → close). Batch Invoicing moved into it (old links redirect). It needs its migration and one crontab line on prod.
**Operator action:**
1. Deploy `main` (runs migration `202609250100_S-BILLING-MODULE_cycles.sql`): `sudo /var/www/fleetforge/bin/deploy.sh`, then `php bin/migrate.php --status` → pending 0.
2. Add the crontab line (predeploy item **E-BILLING-MODULE**): `15 15 * * * /usr/bin/php /var/www/fleetforge/cron/billing_cycle_open.php >> /var/www/fleetforge/logs/cron.log 2>&1` under `sudo -u www-data crontab -e`. It opens each month's cycle on the 1st and reminds the owner; it creates no invoices.
3. **Billing → Settings:** choose the default owner (gets the reminders); enter today's USD→CAD rate if any lease bills in US dollars.
4. On the 1st, open the cycle it created (or **Start the … cycle**) and work the Readiness tab top-down. Prod's ~1,550 unsent drafts show under **Cycles → Months with unsent drafts** — open the oldest and work forward, in stages.
5. **Round 2 (S-BILLING-MODULE-2) — also runs migration `202609250200_S-BILLING-MODULE-2_charges_signoffs.sql`** (charges, step sign-offs, the one-email-per-customer template). **Behaviour change on deploy:** Billing → Settings → **Payment terms run from** defaults to *the day the invoice is sent* — sending an arrears or backlog invoice now moves its due date to send date + the customer's terms (the invoice date is untouched). This resolves KNOWN ISSUE #113; switch it to *the invoice date* only if you want the old rule.
6. Prod's two F71 double-mileage drafts can now be fixed from the cycle's **Review** tab → **Fix: remove the overage line** (same as F71 step 2).
**Note:** no re-login is needed (Billing uses the existing invoice permissions). The monthly invoice job is now safe to turn on (KNOWN ISSUE #112 fixed: it follows the cycle mode, never re-bills a month already covered, skips closed cycles) — but it stays OFF until you decide; try it on a prod copy first. Training chapter 10 ("Batch Invoicing") now shows the old screen and menu path — re-record it with F90.
**Done when:** October's cycle is closed with its billing register downloaded.

---

### F94 — Deploy S-PDF-LETTERHEAD, then open one of each PDF on the live site 🔴 BLOCKING (every Download PDF on prod is broken until deployed)

**Surfaced by:** S-PDF-LETTERHEAD (2026-09-24), from the operator's "none of the pdf feature is working".
**Why:** on prod, credit application #2's PDF was never copied to S3 when storage moved there (the link was an S3 404). No invoice PDF had ever been stored, and the portal's Download PDF pointed at a file that didn't exist. The invoice button used a pop-up that browsers block. The fix is code only.
**Operator action:**
1. Deploy `main` (PHP only: no migration, no `FF_ASSET_VERSION` bump): `sudo /var/www/fleetforge/bin/deploy.sh`.
2. Open each once and check the logo band and layout:
   - an invoice → **View PDF**;
   - Credit Applications → application #2 → **Download PDF** (rebuilt from its saved copy the first time, then stored);
   - Receivables → Statements → **Generate PDF**;
   - Accounting → Reports → P&L → **Export PDF**;
   - Collections → a customer → Dunning Letters → **View PDF** (only letters generated after the deploy have a file).
3. Optional: **Settings → Design → PDF documents** turns the logo off, sets the accent colour, and edits the invoice footer line ("Thank you for your business.").
**Note:** all PDFs are now US Letter (they were a mix of A4 and Letter). Invoice PDFs are rebuilt in the new look on first open; the numbers on them don't change.
**Done when:** each PDF above opens with the logo band on prod.

---

### F93 — Deploy S-CCA-PREFILL, then send the next credit application through FleetForge 🟡 OPEN (next deploy)

**Surfaced by:** S-CCA-PREFILL (2026-09-24).
**Why:** the online credit application (link → form → PDF → review) has been live since June but has been used once on prod (a test). The form now opens pre-filled, and a "Needs Info" re-send carries the customer's previous answers.
**Operator action:**
1. Deploy `main` (PHP only: no migration, no `FF_ASSET_VERSION` bump).
2. For the next new customer, fill in email, phone, address and GST/PST on the customer first. Then **Customer → Credit Application → + Send Application** instead of emailing the PDF.
3. If something's missing, use **Needs Info** → **Re-send Application Link**. The customer gets the form back with their answers.
**Done when:** one real application has gone through send → submit → review on prod.
...
### F89 — Deploy S-SOP-KNOWN-ISSUES, then clean up what the old behaviour left in the books 🟡 OPEN (before the next month-end close)

**Surfaced by:** S-SOP-KNOWN-ISSUES (2026-09-24) — the SOP's chapter 12 backlog (I1–I27) fixed; only I24 (pay link proven live, F80) remains.
**Operator action:**
1. Deploy `main`, then `php bin/migrate.php --apply` (one migration: `202609241800_S-SOP-KNOWN-ISSUES_ledger_fixes.sql` — new account **3050 Opening Balance Equity**, payment/deposit bank columns, GST remittance columns, reconciliation balances, bill-line unit, `credit_note_refunds`, revenue-map mileage keys).
2. Run the report (read-only): `sudo -u www-data php /var/www/fleetforge/scripts/sop_known_issues_backfill.php`. With the accountant, for each section:
   - **A. Bank opening balances** with no opening entry: if the accountant did NOT post them by hand (the old SOP workaround), post them with `… --apply --banks=<ids>`.
   - **B. GST/HST periods remitted before today**: their input tax credits are still in 1050 (and 2030 still credits the same amount). Unless already cleared by hand, one manual journal entry DR 2030 / CR 1050 for the total shown.
   - **C. Vendor credits** that posted AP against AP: one correcting entry each (the report shows them).
   - **D. Revenue by line type**: from the next invoice sent, rental books per equipment category (4010–4050) and mileage to 4060 instead of 4110. Past entries stay; a reclass is the accountant's call (F85).
3. Accounting → Settings: check **GL Account Mapping → Opening Balance Equity** = 3050; review **Revenue Mapping** (engine hours, cartage, sweep, wash, fuel still go to 4110 until mapped); **FX & Other** if FX revaluation will be used.
4. **Behaviour changes to confirm:**
   - The nightly automatic dunning letters used to email every 30+-day overdue customer regardless of settings. They are now a **Dunning letters** reminder in Settings → Customer Emails, OFF by default (production has customer emails off anyway — see F88).
   - Credits are applied in FleetForge (QuickBooks-side applications raise drift) — F84 rule 2 corrected below.
   - Creating a fixed asset now asks how it was paid for and posts the purchase; a CapEx completion or work-order capitalization posts too.
   - A bank transfer between CAD and USD now needs both amounts.
   - December may be closed before the year-end close; Super Admins can **Reopen** a closed month (reason required).
5. QuickBooks → Settings → **Sync & monitoring** (new card): confirm bank sync / drift check on, and set the refund deposit account + payment method if precharge cash refunds will be pushed.
**Done when:** deployed, the report's sections A–C are resolved with the accountant, and the September close uses the new screens.
...
2. Ask the accountant to read chapter 5 (Accounting) and chapter 10 (Month-end close), and to confirm or correct the automatic entries that replaced the old workarounds (S-SOP-KNOWN-ISSUES): the GST/HST remittance clearing 2030 and 1050 (I4), the bank opening-balance entry to 3050 (I8), and the asset-purchase entry (I2). Tell the developer what changes; the chapter is updated in code.
...
5. Chapter 12 (Known issues) — I1–I27 fixed by S-SOP-KNOWN-ISSUES except I24 (see F89); the chapter now lists what is left.
...
2. **Customer payments are recorded in QuickBooks only** (pay links / QuickBooks Payments / deposits entered by the accountant). FF mirrors them. Don't also record them in FF. Apply unapplied PAYMENT money in QuickBooks — FF follows. **Credit notes are applied in FleetForge** (S-SOP-KNOWN-ISSUES / SOP I23: FF pushes each application; an application made in QuickBooks is not copied back and raises drift). A credit refunded in cash is recorded in FF (Refund as Cash) AND against the credit memo in QuickBooks.
