...
- ✅ **S-QBO-21** SHIPPED 2026-05-29 (Phase QBO-10 / 1 of 1 — journal entry push FF → QBO; **COMPLETES PHASE QBO-10 (catch-all JE Pusher)**; JournalEntryPusher 7 inline preflight gates + bridge-derived filter per D-QBO-21-1 spec §8.10 verbatim 5-item FF-ENUM list + JournalEntryEnqueuer with defense-in-depth bridge-derived rejection at gate-0 + JournalEntryService::create/post/reverse hooks; admin UI `/quickbooks/journal_entries` in same commit per D-UI-COMPLETENESS-1; nav 14→15; CLASS 12 mapping updated → **7/7 Pushers mapped**; migrate 73→74/0/0 for acc_qbo_journal_entry_map; new smoke 31 sub-checks incl. integration tests for Enqueuer→Pusher dispatch + bridge-derived end-to-end auto-skip; **K-22 caught via AskUserQuestion**: prompt's 7-item QBO-style filter list rejected in favor of spec §8.10 canonical 5-item FF-ENUM list; D-QBO-21-1/2/3/4/5/6/7; **NEW operator follow-ups F12 (AccountValidator tax_receivable + tax_payable categories must be mapped BEFORE first JE push) + F13 (S-QBO-21-UPDATE-FOLLOWUP queued)**)
- ✅ **S-OPERATOR-FOLLOWUPS-TRACKING** SHIPPED 2026-05-29 (debt-paydown — NEW canonical doc + memory file + CLASS 13 advisory closing the recurring failure mode that operator follow-ups in SESSION LOG prose get lost across context wipes; NEW docs/FLEETFORGE_OPERATOR_FOLLOWUPS.md with 4-status legend + 11 initial items + archive section; NEW memory/feedback_operator_followups_tracking.md; NEW CLASS 13 in _smoke_doc_freshness.php; D-OPERATOR-FOLLOWUPS-TRACKING-1/-2)
- ✅ **S-QBO-19** SHIPPED 2026-05-29 (Phase QBO-8 / 2 of 2 — bill payment push FF → QBO; **COMPLETES PHASE QBO-8 (2/2)**; BillPaymentPusher 8 inline preflight gates incl. bank account chain lookup per D-QBO-19-3 + BillPaymentEnqueuer; admin UI `/quickbooks/bill_payments` in same commit per D-UI-COMPLETENESS-1; nav 13→14; CLASS 12 mapping updated; migrate 72→73/0/0; **PER OPERATOR ASK shipped 2 ADDITIONAL coverage-gap smokes — admin_retry_endpoints + payments_embed extension**; D-QBO-19-1/2/3/4/5/6/7)
- ✅ **S-QBO-15** SHIPPED 2026-05-29 (Phase QBO-6 / 3 of 3 — QBO Payments embed in customer portal; **COMPLETES PHASE QBO-6 (3/3)**; PaymentInitiator + QboClient::generatePaymentsHostedUrl + PaymentWebhookHandler initiation handshake + 2 portal pages + 3 portal/admin endpoints + acc_qbo_payment_initiations table 17 cols + UNIQUE token + 5 indexes + 2 FKs CASCADE/SET NULL; migrate 71→72/0/0; admin /quickbooks/payments extended with Initiations sub-view per D-UI-COMPLETENESS-1 + operator AskUserQuestion answer; D-QBO-15-1/2/3/4/5)
- ✅ **S-QBO-PAYMENT-SYNC-UI** SHIPPED 2026-05-29 (debt-paydown — admin UI mirror of `/quickbooks/bills` for payment sync; closes structural gap that S-QBO-18 → S-QBO-BILL-SYNC-UI exhibited; `/quickbooks/payments` admin page + list/retry endpoints + nav 12→13 children; **NEW CLASS 12 in doc_freshness strict-fails Pusher↔UI gaps + NEW memory file `feedback_ui_completeness_with_backend.md`** codifying the discipline; D-UI-COMPLETENESS-1 + D-QBO-PAYMENT-SYNC-UI-1)
...
- Phase QBO numbered total: 11/30 → 14/30 → 15/30 → 16/30 → 17/30 → **18/30** (S-QBO-12 + S-QBO-18 + S-QBO-13 + S-QBO-14 + S-QBO-15 + S-QBO-19 + **S-QBO-21** advance the counter; SYNC-UI + GOTCHAS-PAYDOWN + PAYMENT-SYNC-UI + OPERATOR-FOLLOWUPS-TRACKING are debt-paydowns)
- Debt-paydown/infra total: 27 → 30 → 31 → **32** (S-VENDOR-CURRENCY-COLUMN + S-QBO-BILL-SYNC-UI + S-QBO-BILL-GOTCHAS-PAYDOWN + S-QBO-PAYMENT-SYNC-UI + **S-OPERATOR-FOLLOWUPS-TRACKING** registered as QBO paydowns; S-DOC-FRESHNESS-EXPAND/EXPAND-2 not counted — project infrastructure)
- D131 smoke count: 22 → 24 → 25 → 26 → 28 → **30** (S-QBO-21 added qbo_journal_entry_push; S-QBO-19 added qbo_bill_payment_push + qbo_admin_retry_endpoints coverage-gap smoke per operator ask; S-QBO-15 added qbo_payments_embed (extended 28→31 same session); SYNC-UI + GOTCHAS-PAYDOWN + DOC-FRESHNESS-EXPAND-2 + PAYMENT-SYNC-UI + OPERATOR-FOLLOWUPS-TRACKING extend existing smokes in-place)
- Migrate count: 65 → 71 → 72 → 73 → **74** (S-QBO-21 acc_qbo_journal_entry_map migration; S-QBO-19 acc_qbo_bill_payment_map earlier; S-QBO-15 acc_qbo_payment_initiations earlier; S-QBO-14 + PAYMENT-SYNC-UI + OPERATOR-FOLLOWUPS-TRACKING ship NO migration)
- **Phase QBO-6 progress**: ✅ **COMPLETE (3/3)** — S-QBO-13 + S-QBO-14 + S-QBO-15 all shipped 2026-05-27/28/29
- **Phase QBO-8 progress**: ✅ **COMPLETE (2/2)** — S-QBO-18 BillPusher + S-QBO-19 BillPaymentPusher
- **Phase QBO-10 progress**: ✅ **COMPLETE (1/1)** — S-QBO-21 JournalEntryPusher catch-all surface
...
| **S-QBO-22** | Fixed asset JE sync | QBO-11 | M | Sonnet | S-QBO-21 | ✅ SHIPPED 2026-05-29 | Depreciation + asset_disposal + impairment JEs route through existing JournalEntryPusher (no new Pusher class per spec §8.13). Migration ENUM +impairment (D-QBO-22-2) + PrivateNote FA-enrichment (D-QBO-22-1) + filter chip + 3-tile FA KPI strip on /admin/quickbooks/journal_entries (D-QBO-22-3) + bridge-derived regression guard (D-QBO-22-4). 24/24 smoke PASS w/ per-function coverage. Asset records stay FF-only per spec §8.13 CCA Schedule 8 continuity. Commit b94da9d. |
| **S-QBO-23** | Tax remittance JE sync | QBO-11 | S | Sonnet | S-QBO-21 | ✅ SHIPPED 2026-05-29 | CLOSES Phase QBO-11. No new Pusher class — tax_remittance JEs route through existing JournalEntryPusher (sibling of S-QBO-22). NO ENUM ALTER (tax_remittance already present + not bridge-derived). D-QBO-23-1 dispatcher refactor (buildSourceNoteSection routes FA + tax-remittance) + D-QBO-23-3 source-type chip group UI. Migration seeds sync_mode.tax_remittance='inherit_je' marker only. 19/19 smoke PASS incl. dispatcher-refactor regression guards. Supports accountant filing GST34 through QBO NETFILE. |
| **S-QBO-24** | Drift detection cron `qbo_drift_check.php` | QBO-12 | L | Sonnet | All Pushers shipped through S-QBO-21 | ✅ SHIPPED 2026-05-30 | DriftChecker engine + cron (03:30, lock ff_qbo_drift_check) + Run-now button on /quickbooks/drift + drift_run.php. Hybrid: snapshot layer (push_failed + FF-side amount_drift, no API, pre-cutover-valuable) + live layer (missing_in_ff, post-cutover). 8 entity maps; per-entity tolerance (§15.5); notify threshold. Reuses acc_qbo_drift_events (S-QBO-3); spec §15.3 sketch corrected to as-built (D-QBO-24-2). GL-balance → F23; resolution = S-QBO-25. Settings-only migration 80→81. 15/15 smoke. 4 D-QBO-24-* locked. |
| **S-QBO-25** | Drift resolution workflows | QBO-12 | L | Opus | S-QBO-24 | ✅ SHIPPED 2026-05-30 | Per-event resolve/accept/suppress/reopen/resync + bulk-by-category accept/suppress + auto-resolve-on-parity in DriftChecker. Migration 81→82/0/0 ALTER acc_qbo_drift_events ADD resolution_type ENUM + idx_resolution + backfill. recordDrift terminal-state guard so accept/suppress survive re-runs (CRITICAL invariant). 5 action verbs on drift_resolve.php + NEW drift_bulk_resolve.php. Admin UI per-row Accept/Suppress/Reopen/Re-sync + bulk-by-category bar + status filter +accepted/+suppressed (suppressed hidden by default per K-22 #4). 28/28 smoke (15 S-QBO-24 + 13 new). 4 D-QBO-25-* locked. Bulk force-resync/force-pull = S-QBO-26 (next). |
| **S-QBO-26** | Manual Sync page | QBO-12 | L | Sonnet | S-QBO-25 | ✅ SHIPPED 2026-05-30 | CLOSES Phase QBO-12. NEW manual_sync.php endpoint + admin page (force_full_resync-gated). 3 actions: force_resync (bulk re-enqueue all mapped rows via Enqueuer, 8 push types, gates honor D-QBO-14-1/D-QBO-21-1/sync_enabled/sync_mode — D-QBO-26-1/4) + force_pull (Puller::pullAll + SyncToken refresh, 5 pull-capable types; push-only → 422 → S-QBO-27 per D-QBO-26-2) + reset_synctoken (realm-migration recovery, D-QBO-26-3). Reuses force_full_resync permission + DriftChecker::ENTITY_CHECKS. NO migration. Nav 16→17 (Manual Sync after Drift). 10/10 smoke + 6 nav-assert smokes bumped. 4 D-QBO-26-* locked. |
