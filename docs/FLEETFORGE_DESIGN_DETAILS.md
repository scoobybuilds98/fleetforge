...
| S-SOP-MODULE | SOP module (own file `public/assets/css/sop.css`, loaded by /sop pages only) | whole file | `.sop-hero` (blueprint-grid hero), `.sop-chapter-card` (per-chapter `--acc` accent via `.sop-acc--*`), `.sop-content` typography + numbered step rail (`counter(sopstep)`), `.sop-path` screen chips, `.sop-callout--*`, `.sop-cards`, `.sop-entries` (DR/CR cards), `.sop-cl*` month-end checklist, `.sop-signal--*` live-check pills, the SVG diagrams in `lib/Sop/diagrams/` are coloured by the shared `dg-*` classes (moved to `module-chrome.css` by S-MODULE-CHROME; no fills in the SVGs). Motion: stroke-dashoffset + animateMotion, all off under reduced motion |
| S-MODULE-CHROME | Module heroes + KPI tile glow (own file `public/assets/css/module-chrome.css`, linked by `header.php` and `header_embed.php`) | whole file | `.ff-hero` blueprint-grid hero (`::before` grid masked toward the art, radial `--acc` glows) with `.ff-hero-crumbs/-eyebrow/-title/-sub/-facts/-actions/-art`; `.ff-hero--entity` (flex-wrap: `.ff-hero-id` avatar + main, actions wrap to their own row when they don't fit, `.ff-hero-mark` 120px faint watermark); `.ff-hero-own` (a page's own title block inside the hero); accents `.ff-acc--{primary,info,success,warning,danger,purple}` (default via zero-specificity `:where(.ff-hero)` — S-MODULE-CHROME-2); `.ff-stats > .stat-card` corner glow from `--stat-accent` + `ff-rise` stagger (no top bar); **key-number tiles** (S-LIST-COMPACT) — any `.stat-grid.ff-stats` after a hero (`:is(.ff-hero ~ .stat-grid.ff-stats, .ff-hero ~ * > .stat-grid.ff-stats)`): separate compact cards (gap 12px, ~64px, icon | label over value + sub-line, clickable ones lift, `.ring-active` rings; 2 columns ≤1000px) — the hero itself keeps its full size; `.ff-contents`; the shared `dg-*` SVG paint classes (SOP diagrams + `lib/Ui/art/`). Rendered by `FleetForge\Ui\ModuleHero`. ≤1000px: art hidden, one column; ≤560px: watermark hidden; reduced motion: all loops off |
| S-DASHBOARD-REDESIGN / S-DASHBOARD-VIZ | Dashboard (own file `public/assets/css/dashboard.css`, loaded by app/admin/dashboard only) | whole file | `.dash-hero` (+ `.dash-pulse` ring: `.dash-ring--lease/avail/other` arcs driven by inline stroke-dasharray), `.dash-attn*` Needs-attention cards (`.dash-tone--ok/info/warn/danger` set `--tone`/`--tone-text`), `.dash-kpis` / `.dash-kpi-group` grouped tiles, `.dash-nav` sticky section bar (`.is-on` via scroll-spy), `.dash-sec` numbered sections, `.dash-panel` + `.dash-tabs` + `.dash-list`/`.dash-item` work panels, `.dash-card` chart cards, and the `dv-*` visuals: `.dv-kpis`, `.dv-segbar`/`.dv-seg` (aging, fleet status), `.dv-legend`, `.dv-rank` ranked lists with `.dv-bar`, `.dv-months` heat strip (`--i`), `.dv-types`/`.dv-split`, `.dv-shares`, `.dv-c--*` tone fills, `.dv-delta.is-good/.is-bad` |
| S-SHELL-REDESIGN | App shell (own file `public/assets/css/shell.css`, linked by header.php after module-chrome.css) | whole file | Visual-only over app.css: sidebar surface (brand glows + blueprint grid), `.sidebar-brand` (+ `.has-logo` / `.is-logo-broken` fallback), `.nav-section-label` (becomes a divider in collapsed rails), `.nav-item` accent tile via `--acc` (`.is-active`, `.has-active-child`), `.nav-children` tree line, `.sidebar-user` card; topbar `.topbar-context` (+ `.topbar-context-ic` accent tile), centred `.search-wrapper`, `.topbar-tray` utility group, avatar-only account chip below 1440px; phones: the right-hand group keeps its natural width and the page title ellipsises (the account chip never leaves the screen). The "Find a page" filter was removed (S-SHELL-FIND-REMOVE) |
| S-SETTINGS-REDESIGN | Settings (own file `public/assets/css/settings.css`, loaded by app/admin/settings only) | whole file | `.set-shell` (272px nav + main; 208px ≤1180 with subtitles hidden; chips row ≤860), sticky `.set-nav` with its own scroll, `.set-nav-label` group headings, `.set-nav-item` (`.set-nav-ic` accent tile via `.ff-acc--*`, `.set-nav-name/-sub/-badge`, `.is-active` gradient + glowing rail, `.is-locked`), `.set-panel-head` for the open section; inside `.set-main`: cards at padding 0 (header 16/22, body 20/22), switch-style `.form-check > input[type=checkbox]`, flush tables; phones collapse the tabs' inline grids to one column |
| S-TABLES-REDESIGN | Tables (own file `public/assets/css/tables.css`, linked by header.php + header_embed.php after shell.css) | whole file | `--tbl-*` locals per table (`--tbl-acc` = module colour via `--acc`); joined `.table-toolbar` + list card panel; header capsule (`thead th` surface-2, rounded ends, tinted in list cards); desktop rows as rounded strips (td background-image hairline, hover/selected tint, selected rail); `.ff-title-cN` name column; `.btn-ghost` row actions as outlined pills; tfoot capsule; `.ff-fit` / `.ff-nw-cN` / `.ff-wrap-cN` / `.ff-act` / `.ff-overflow` (sticky actions) from `FF_TableFit`; `.ff-table-card` (padding 10px, scrolls sideways); toolbar `.tab-bar` segmented control, `.ff-focus-chip`, `.ff-pager` compact pager; `table.spec-table` + `.card.spec-card` key/value cards; phones keep app.css `.table-stack` |
| S-BACKGROUNDS | Background palettes (own file `public/assets/css/backgrounds.css`, loaded right after app.css by every shell) | whole file | Per palette in `config/backgrounds.php` (midnight, graphite, navy, obsidian, pine; espresso = app.css): `html[data-bg=x]` → `--sidebar-bg` only; `html[data-bg=x]:not([data-theme=light])` → the dark neutral tokens (bg-*, text greys, borders, topbar, table, input, segment); `html[data-bg=x][data-theme=light]` → the light ones. Design tab tiles (`#ff-design-tab .ff-bg-*`) preview dark + light halves |
| S-RECORD-REDESIGN | Record pages (own file `public/assets/css/records.css`, linked by header.php + header_embed.php after tables.css) | whole file | App-wide `.tab-bar` restyle (underline tabs in a slim frame, accent underline + badge; toolbar tab bars stay segmented) and `.tab-bar--sticky` (full width, every tab, wraps, pinned under the topbar; `.tab-anchor` before it); `.rec-layout` (main + 320px rail; ≤1180 rail becomes a card grid above main) with `.rec-main` section cards (padding 0, sentence-case titles with an accent dot) and `.rec-dl` label/value grids; rail `.rec-card` (`--accent/--danger/--warning`), `.rec-card-head/-ic/-title/-link/-foot`, `.rec-big`, `.rec-kv`, `.rec-meter` (ok/warn/danger/info), `.rec-alerts` (tone dots), `.rec-links` (+ `--grid`), `.rec-entity`; `.rec-more` / `.rec-more-menu` header More menu (rows normalised, hidden rows stay hidden, hero unclips while open); `.ff-live-badge`; unit Payoff tab `.po-*`. Server pieces: `FleetForge\Ui\RecordUi` |
| S-SIDEBAR-LOGO | Sidebar logo (`shell.css` brand block + `FleetForge\Ui\BrandLogo` + `api/v1/storage/logo.php`) | `.sidebar-brand.has-logo` … | The logo is a derived copy (margins trimmed, re-padded, ≤720px) left-aligned on the nav edge, `max-height: 56px`, 12px gap to the chevron; `mix-blend-mode: lighten` on every sidebar logo (the sidebar is dark in every theme/palette, so only pixels darker than it change); canvas class `.logo-bg-dark/-light/-clear/-mixed` — light + mixed get a rounded badge with a hairline ring; `.sidebar-logo-mark` (the logo's own icon) replaces the logo in collapsed/tablet rails (`.has-logo-mark`), a wide logo without one shows the truck tile (`.is-logo-wide`) |
| S-TOPBAR-CHROME | Topbar surface (`shell.css`) | `.topbar`, `[data-theme="light"] .topbar`, `[data-theme="dark"] .topbar` | Light theme: the sidebar's surface (`--sidebar-bg` + a primary radial glow top-left), solid (no glass blur), `--sidebar-border` edge, `--shadow-ambient`. Dark theme: a lifted band (`--sidebar-text-hover` 8%→5% over `--sidebar-bg`, inset top highlight, 11% edge). Controls (`.topbar-left`, search box, tray buttons except the display menu's wrapper, bell, account chip, phone search button) re-token `--text-*`, `--bg-surface*`, `--border-color*`, `--topbar-text` to the sidebar set; the tray frame is tinted directly; menus keep page tokens. Unread badge ring = `--sidebar-bg` |
