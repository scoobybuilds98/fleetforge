...
*Last updated: 2026-04-07 — PERM-1 Per-User Permission Overrides + Per-User Display Settings complete (new `user_permission_overrides` table with layered `can()` resolution: super_admin short-circuits true, then per-user override beats role default, falls through when no row exists; 3 super_admin-only API endpoints — index/update/reset — with full field-level validation, super_admin target protection via 409 SUPER_ADMIN_PROTECTED, transactional + audit_log; admin matrix page with 24 modules × 5 actions, none→allow→deny→none cycle, reason modal, override summary, reset-all confirmation, FF_Toast two-arg signature; Manage Permissions card on user show page hidden when target is super_admin. New `display_font_size` (70–130 step 5) + `display_density` (compact/comfortable/spacious) columns on users; `api/v1/users/display_settings/update.php` — any authenticated user, self-only, partial update, validates step set + enum, syncs $_SESSION on save so next page render uses new values; `header.php` server-clamps both values + injects `<style id="ff-display-font-size">.page-content { font-size: X% }</style>` SCOPED STRICTLY to `.page-content` so sidebar/topbar/footer/modals are never rescaled, sets `<body data-density>`, exposes `window.FF_DISPLAY` to JS; topbar quick-controls popover with A−/A+ + density chips + reset before notifications bell, same dropdown-enter transition as other topbar dropdowns; `FF_Display` JS singleton (`STEPS`, `DENSITY`, `_applyDom`, `_persist`, `setFontSize/setDensity/setBoth/reset`); CSS density variants `body[data-density="compact"|"spacious"] .page-content .card-body / .card-header / table cells / .form-group / .page-header / .form-control` (comfortable empty = defaults); profile page new "Display" tab between Profile and Login History — slider 70–130 + 3 density radio cards + live preview card + reset + error banner, Alpine `displaySettingsTab()` delegates all persistence to FF_Display. **All stop conditions passed (16 tests)**: permissions API — GET index for dispatcher (24 modules, 0 overrides), POST update grant `payments.view` (override_id=1, DB row exists, GET reflects {role:false, override:1, effective:1}), POST update on super_admin target → 409 SUPER_ADMIN_PROTECTED, POST update granted='maybe' → 422 with field map, POST update module='widgets' → 422 "Unknown module", POST update granted=null clears row, POST reset cleared 3 overrides; display_settings API — POST font_size=120 → 200 DB updated, POST density=spacious → 200, POST both → 200, POST font_size=99 → 422 with step list, POST density=huge → 422 with enum list, POST {} → 422 "Nothing to update", POST font_size=200 → 422; **end-to-end revoke** — dispatcher gets /customers → 200, super_admin overrides customers.view = denied, dispatcher re-logs in, /customers → 403, override cleared, dispatcher re-logs in, /customers → 200; profile + permissions pages → 200; PHP syntax check ✅ all 11 PHP files clean. PERM-1 complete. — VALID-2 Form Validation Messages complete (every form in the admin app now accumulates field-level errors into the standard `{success:false, error:{code:"VALIDATION_ERROR", fields:{...}}}` envelope and paints them into inline `.field-error` slots + `.form-error-banner` banners. 15 accounting pages verified/wired — statements / bank-accounts / journal-entries / bank-reconciliation / depreciation / capex / fixed-assets / tax / settings / bills / vendor-credits / deposits / collections / categorization-rules / chart-of-accounts — ~400 error markers total. 2 server API bugs fixed: api/v1/vendors/create.php was fail-fast MISSING_REQUIRED → rewritten to accumulate into $fields[] and call json_validation_error() once (all guards: name/vendor_type/email/rating/hourly_rate + duplicate-name through fields envelope); api/v1/yards/create.php used bespoke `['errors' => [...]]` extra key + fail-fast → rewritten to use standard fields envelope + accumulated capacity/manager_id guards + duplicate-name through fields envelope. New smoke test tests/_smoke_valid2.php (~290 lines): manually writes a real PHP session file to /var/tmp/sess_<id> (the path Herd PHP-FPM uses, NOT CLI PHP's /var/folders/x6/.../T/), injects ff_user + csrf_token, then POSTs 30 bad payloads via cURL against the live Herd endpoints with Cookie + X-CSRF-Token. **Final run: 30/30 PASS** — Journal Entry (empty/1-line/unbalanced), Bank Account, Bank Transfer, Bill, Tax Period, CapEx, Vendor Credit, AR Deposit, Customer (empty/bad email/discount >100/negative credit), Equipment Unit, Lease, Invoice (empty/bad period), Payment (empty/negative amount), Vendor, Reservation, Damage Claim, Maintenance WO, Inspection, Mileage Log, Yard. Every case asserts HTTP 422 + success=false + error.code='VALIDATION_ERROR' + error.fields containing expected keys. Defensive cleanup deletes stray `Smoke Acme%` customers at end of run. VALID-2 complete. — PAYOFF-1 Unit Payoff Calculator complete (11 new columns on `acc_fixed_assets` for acquisition detail/financing/monthly-fixed costs via ALTER TABLE; new `api/v1/accounting/fixed_assets/payoff.php` endpoint — bcmath-only calculator returning total invested, net revenue to date, still to recover, progress %, 3 rolling-average scenarios (3/6/12-month), 14-month dense chart window, optional custom monthly override + one-time extra-cost adjustment; new `api/v1/accounting/fixed_assets/payoff_report.php` fleet-wide aggregate endpoint (4 grouped queries across all linked assets, sortable on 6 computed columns); Payoff Analysis tab added to asset detail modal with 4 KPI summary cards, colour-coded progress bar, 3 clickable scenario cards that re-run the API with period=3/6/12, theme-aware ApexCharts area chart with target-line annotation, revenue/expense breakdown, and Manual Override panel with Custom badge; new Edit modal (asset register had none) supporting the 11 new payoff fields + equipment unit link, with optimistic-lock updated_at; new fleet-wide Payoff Report page (`/accounting/fixed-assets/payoff-report`) with summary tiles, filter toolbar, sortable table, colour-coded progress bars, CSV export; nav link added under Fixed Assets group; `FixedAssetService::create/update` extended to persist all 11 new fields; `create.php`/`update.php` sanitisation updated with bool coercion for `is_financed`; verified with real data on FA-2025-00003 (unit 6 RFR-001, $45k purchase + $6,150 detail = $52,050 total invested, $27,931.53 net revenue = 53.66% progress, 9 months to payoff at 6-month rolling $2,976/mo average). NAV-1 Accounting Nav Fixes complete (topnav dropdown clipping root-cause fix + sidebar accounting section restructured to mirror topnav groups + `match_prefix` field added to sidebar). Sessions completed to date: S001 (Foundation), S002 (Schema + Seeds), S003 (Dashboard Stub + Remaining Seeds), S004 (Dashboard KPIs + Charts), S005 (Customers), S006 (Equipment), S007 (Leases), S008 (Invoices + Billing Engine), AUDIT-1 (69-issue QC audit), S008.5 (Critical Fix), S009 (Payments), S009-EXT (Topbar), S010 (Billing Cron), S011 (Credit Notes), S012 (Damage Claims), S013 (Mileage Logs), S014 (Vendors), S015 (Maintenance Work Orders), S016 (Inspections), SEARCH-FIX, S017 (Users + Admin + Audit + Settings), S017-B (My Profile + change_password + save_preference + login history), S017-C (super_admin set_password + delete bypass), S017-X (sidebar scroll + copyright footer + stress test + customers/show bug fix), S017-UX (scrollable tab tables + filter/sort bars on all show pages + global search endpoint + leases/invoices list redesign), S018 (Reservations Module), S018-EXT (Yards Management + Pickup Yard Dropdown + Date Picker + Conflict Override), AUDIT-2 (43-bug adversarial audit + full fix pass + 98/98 stress test — all API endpoints hardened), S019 (Rates Module — rate_cards + customer_equipment_rates + lookup_rates + admin UI + lease pre-fill), S019-EXT (Rates polish + Invoice show comprehensive rebuild), S020 (Compliance Module — color-coded grid + CSV export + inline update + cron), S020-EXT (MVI removal + from/to dates + document links + storage serve endpoint), S020-EXT2 (Compliance from/to direct columns + super_admin invoice access + KPI tile auto-refresh all modules), S021 (Reports Module — 4 tabs / 20 views / 15 charts / CSV / 15-min cache), S021-UX (scroll-to-top bug + tab flash + smooth transitions), S022 (Documents Module), S023 (Analytics Module), S026 (GPS Tracking — Samsara), S027 (Claude AI Integration), VALID-1 (Form Validation Hardening), DATE-1 (Date Field Hardening), S028 (Accounting Module Foundation), S028-b (Settings audit + manual cron overrides + prefix wiring), S028-c (Settings root cause fix + accounting top nav), S030 (Auto-JE Bridge + GL Ledger + Trial Balance + AR Aging + AR Reconciliation), S031 (GL & AR Layer — Customer Statements + Collections + Bad Debt + Deposits + Crons), S032 (Accounts Payable — Bill CRUD, AP payments, vendor credits, AP aging, cash requirements), S033 (Bank Management — bank accounts CRUD, manual transactions, CSV import with auto-detect/auto-match, reconciliation workflow, NSF processing, inter-account transfers + 9 bugs found and fixed in deep audit including critical Alpine `this`-context crash), SETTINGS-AUDIT (72 settings audited — 33 OK, 10 configured, 21 orphaned, 3 bugs fixed), NAV-1 (Accounting Nav Fixes — topnav overflow clipping + sidebar restructure to mirror topnav groups + `match_prefix` field added to sidebar). 34 decisions locked. SOFT_DELETE_TABLES: 15. lib/Billing/ 5 files. lib/GPS/ 1 file. lib/Reports/ 1 file. lib/AI/ 4 files. lib/Accounting/ 4 files (AccountingService, JournalEntryService, AutoEntryBridge, BankService). Next: S034 — Fixed Assets & Depreciation (Spec Phase 22). **Tables**: `acc_fixed_assets`, `acc_depreciation_runs`, `acc_depreciation_run_lines`, `acc_asset_disposals`, `acc_asset_impairments`. **Build scope**: (1) **Asset register** — full CRUD on `acc_fixed_assets` with class/category, acquisition date+cost, salvage value, useful life, depreciation method, accumulated depreciation, NBV, status; auto-prompt to create asset record when a new equipment unit is created (link via `equipment_unit_id`); also supports non-fleet assets (office equipment, leasehold improvements). (2) **Three depreciation methods**: **straight-line** (default — `(cost − salvage) / (useful_life_years × 12)` per month), **declining balance / CRA CCA** (half-year rule year 1 → `cost × (rate / 2)`, then `opening UCC × rate` thereafter), **units-of-production** (`(cost − salvage) × (units_this_period / total_expected_units)` — for fleet assets the units come from `mileage_logs`). (3) **Monthly depreciation run** — accountant clicks "Run Depreciation" for a period; system calculates depreciation for every active asset; preview screen shows opening NBV / depreciation / closing NBV per asset; on confirm, posts ONE consolidated JE (`DR 5010 Depreciation — Fleet` + `DR 6190 Depreciation — Non-Fleet` / `CR 1220 Accum Depr Fleet` + `CR 1260 Accum Depr Other`); updates `accumulated_depreciation` + `net_book_value` on every asset; marks run as posted (cannot re-run for same period without reverse). (4) **Asset disposal** — triggered when an equipment unit is set to `decommissioned`; calculates gain/loss = proceeds − NBV; posts JE: `DR Cash + DR 1220 Accum Depr / CR 1210 Equipment Cost + CR/DR 7010/7020 Gain/Loss`. (5) **Asset impairment** — records pre-impairment NBV, impairment loss, post-impairment NBV; posts JE: `DR 7020 Impairment Loss / CR 1220 Accum Depr`. (6) **CapEx review workflow** — work orders with total cost > `accounting.capex_threshold_cad` (default $2,500) raise a capitalization flag; accountant reviews → capitalize (add to asset cost + recalculate depreciation schedule from that month forward) or expense. **Wiring**: 6 admin pages (`/accounting/fixed-assets` register + show + create + edit, `/accounting/depreciation` run preview/post page, `/accounting/capex` review queue), 12+ API endpoints (assets CRUD, depreciation preview/post/reverse, disposal, impairment, capex flag review), `lib/Accounting/DepreciationService.php` (3 method calculators + monthly run orchestrator + JE posting), AutoEntryBridge hooks (`onAssetDisposed`, `onAssetImpaired`, `onDepreciationPosted`), period-validation (closed period redirect per §16), audit_log on every event. **Settings already wired** (S028): `accounting.capex_threshold_cad`, `accounting.default_depreciation_method`, `accounting.default_useful_life_years`, `accounting.default_salvage_pct`. **Stop conditions to verify**: assets CRUD ✅; all 3 depreciation methods produce correct values against worked examples ✅; monthly run preview matches post ✅; consolidated JE balances ✅; cannot re-run same period ✅; disposal posts gain JE ✅; disposal posts loss JE ✅; impairment posts JE + reduces NBV ✅; capex flag appears for WO > threshold ✅; capex capitalize updates asset cost + future depreciation ✅; UCC half-year rule correct in year 1 ✅; units-of-production pulls miles from `mileage_logs` ✅; closed period auto-redirects to earliest open period ✅.*
