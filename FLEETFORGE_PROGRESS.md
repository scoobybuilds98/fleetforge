...
S027 — Claude AI Integration — complete.
  ...
  Language Reports, Anomaly Alerts (SQL-based detection, 5 detectors).
  20 new files, 7 modified. 17 database query tools registered.
  Full-page AI Assistant at /ai with 5 tabs. Floating chat widget on
  all pages. Summary cards on customer/lease/equipment show pages.
  AI connection test in Settings → Integrations.

S026 — Samsara GPS Tracking — complete.
  Full fleet GPS tracking: Leaflet.js map at /tracking with sidebar,
  auto-refresh, color-coded markers. GPS tab on equipment/show (10th tab).
  SamsaraClient extended with 4 new methods. Server-side proxy API at
  /api/v1/gps/locations. Connection test in Settings → Integrations.
  3 test units configured with Samsara device IDs.

S025 — Settings Module Overhaul — complete.
  5 files in app/admin/settings/: 6-tab layout (General, Users, Portal Users,
  Audit Log, System, Integrations). Super_admin can manage all portal_users,
  invite admin users, view audit trail, monitor system health.

S024 — Customer Portal — complete.
  20 files in app/portal/: complete customer-facing portal with separate auth.
  Portal login: john@apexfreight.com / Test1234 (dev only)
...
READ IN THIS ORDER:
  ...
  3. FLEETFORGE_SPEC_FINAL.md             ← grep section for Accounting
...
S024 — Customer Portal (Phase 15) ✅ COMPLETE
S025 — Settings Module Overhaul ✅ COMPLETE
S026 — Samsara GPS Tracking ✅ COMPLETE
S027 — Claude AI Integration ✅ COMPLETE
VALID-1 — Form Validation Hardening ✅ COMPLETE
...
PORTAL AUTH: Portal uses separate session key ff_portal_user (not ff_user).
  ...
  Portal CSS classes: portal-* prefix (portal-section, portal-table, portal-form-*).
