...
### Bug investigation (BLOCKED on operator input)

**INV-2026-00090 mileage charge investigation** — BLOCKED
Scope: lease + invoice generated by Avi during real-use testing showed "X km driven" but no mileage charge on the invoice. Most likely Model C behavior (period_distance < period_allowance → no excess → no charge), not a bug. Confirm by collecting actual values.
Effort: TBD (10 min if Model C confirmation; full session if real bug).
Blocked on: operator to provide INV-2026-00090's lease estimated_mileage_km, mileage_rate_km, period_distance_km, lease start/end dates, invoice period_start/period_end.
Discussed: 2026-05-07 in planning chat.
