...
// ── C11: nav has 6 QuickBooks children incl. Customers ─────
$c11Errors = [];
$nav = require __DIR__ . '/../config/navigation.php';
// Two entries carry label='QuickBooks': the separator (line ~223)
// and the actual group with `children` (line ~228). Skip the
// separator by requiring the entry to have a non-empty children list.
$qbo = null;
...
if ($qbo === null) {
    ...
    if (count($children) !== 6) {
        $c11Errors[] = 'expected 6 QuickBooks children, got ' . count($children) . ' (' . implode(', ', $labels) . ')';
    }
    if (!in_array('Customers', $labels, true)) {
        $c11Errors[] = "no 'Customers' child in QuickBooks nav";
    }
    // Confirm position — Customers between Drift and Settings.
    $expectedOrder = ['Dashboard', 'Sync Queue', 'Sync Log', 'Drift', 'Customers', 'Settings'];
    if ($labels !== $expectedOrder) {
        $c11Errors[] = 'nav order mismatch — got [' . implode(', ', $labels) . '], expected [' . implode(', ', $expectedOrder) . ']';
    }
}
if (empty($c11Errors)) {
    echo "PASS C11 nav has 6 QuickBooks children with Customers in expected position\n";
    $pass++;
} else {
