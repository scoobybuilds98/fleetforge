...
if ($qbo === null) {
    ...
    if (count($children) !== 16) {
        $c11Errors[] = 'expected 16 QuickBooks children, got ' . count($children) . ' (' . implode(', ', $labels) . ')';
    }
    if (!in_array('Customers', $labels, true)) {
        $c11Errors[] = "no 'Customers' child in QuickBooks nav";
    }
    $expectedOrder = ['Dashboard', 'Sync Queue', 'Sync Log', 'Drift', 'Customers', 'Vendors', 'Accounts', 'Tax Codes', 'Items', 'Invoices', 'Credit Memos', 'Bills', 'Bill Payments', 'Payments', 'Journal Entries', 'Settings'];
    if ($labels !== $expectedOrder) {
        $c11Errors[] = 'nav order mismatch — got [' . implode(', ', $labels) . '], expected [' . implode(', ', $expectedOrder) . ']';
    }
}
if (empty($c11Errors)) {
    echo "PASS C11 nav has 16 QuickBooks children with Customers in expected position\n";
    $pass++;
} else {
