...
if ($qbo === null) {
    ...
    if (count($children) !== 13) {
        $c11Errors[] = 'expected 13 QuickBooks children, got ' . count($children) . ' (' . implode(', ', $labels) . ')';
    }
...
if (empty($c11Errors)) {
    echo "PASS C11 nav has 13 QuickBooks children with Customers in expected position\n";
    $pass++;
} else {
