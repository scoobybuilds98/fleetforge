...
$c17Errors = [];
try {
    db_execute("DELETE FROM acc_qbo_vendor_map WHERE ff_vendor_id = 999996");
    db_execute("DELETE FROM acc_qbo_sync_log WHERE entity_type='vendor' AND entity_id = 999996");
    db_execute("DELETE FROM vendors WHERE id = 999996");
    db_execute(
        "INSERT INTO vendors (id, name, vendor_type) VALUES (999996, ?, 'other')",
        ['SMOKE C17 Vendor']
    );
    $sentinelVendorIds[] = 999996;
    ff_smoke_v_set_setting('quickbooks.sync_mode.vendor', 'qbo_to_ff');

    $r = VendorPusher::pushCreate(999996);
    ...
    $mapRow = db_row("SELECT push_status FROM acc_qbo_vendor_map WHERE ff_vendor_id = ?", [999996]);
    if ($mapRow === null) {
        $c17Errors[] = "expected acc_qbo_vendor_map row for ff_vendor_id=999996 (none found)";
    ...
    $logRow = db_row(
        "SELECT error_code, response_status, http_method FROM acc_qbo_sync_log
          WHERE entity_type='vendor' AND entity_id=? AND error_code='skipped_by_mode'
          ORDER BY id DESC LIMIT 1",
        [999996]
    );
    if ($logRow === null) {
...
if (empty($c17Errors)) {
    echo "PASS C17 VendorPusher skipped_by_mode persists map row + sync_log (S-QBO-CUSTOMER-VENDOR-PUSH-STATE-INFRA)\n";
    $pass++;
} else {
...
$c18Errors = [];
try {
    db_execute("DELETE FROM acc_qbo_vendor_map WHERE ff_vendor_id = 999997");
    db_execute("DELETE FROM acc_qbo_sync_log WHERE entity_type='vendor' AND entity_id = 999997");
    db_execute("DELETE FROM vendors WHERE id = 999997");
    db_execute(
        "INSERT INTO vendors (id, name, vendor_type, deleted_at) VALUES (999997, ?, 'other', NOW())",
        ['SMOKE C18 Vendor (soft-deleted)']
    );
    $sentinelVendorIds[] = 999997;

    $r = VendorPusher::pushCreate(999997);
    ...
    $mapRow = db_row("SELECT push_status FROM acc_qbo_vendor_map WHERE ff_vendor_id = ?", [999997]);
    if ($mapRow === null) {
        $c18Errors[] = "expected acc_qbo_vendor_map row for ff_vendor_id=999997 (none found)";
    ...
    $logRow = db_row(
        "SELECT error_code, response_status, http_method FROM acc_qbo_sync_log
          WHERE entity_type='vendor' AND entity_id=? AND error_code='skipped_soft_deleted'
          ORDER BY id DESC LIMIT 1",
        [999997]
    );
    if ($logRow === null) {
...
if (empty($c18Errors)) {
    echo "PASS C18 VendorPusher skipped_soft_deleted persists map row + sync_log (S-QBO-CUSTOMER-VENDOR-PUSH-STATE-INFRA)\n";
    $pass++;
} else {
    ...
    try {
        db_execute(
            "DELETE FROM acc_qbo_sync_log WHERE entity_type='vendor' AND entity_id BETWEEN 999990 AND 999999"
        );
    } catch (Throwable $e) {
        echo "WARN  defensive sync_log cleanup failed: " . $e->getMessage() . "\n";
    }
