...
if ($qbo === null) {
    ...
    if (count($children) !== 13) {
        $c13Errors[] = 'expected 13 QuickBooks children, got ' . count($children) . ' (' . implode(', ', $labels) . ')';
    }
...
if (empty($c13Errors)) {
    echo "PASS C13 nav has 13 QuickBooks children with Tax Codes in expected position\n";
    $pass++;
} else {
