...
if ($qbo === null) {
    ...
    if (count($children) !== 15) {
        $c13Errors[] = 'expected 15 QuickBooks children, got ' . count($children) . ' (' . implode(', ', $labels) . ')';
    }
...
if (empty($c13Errors)) {
    echo "PASS C13 nav has 15 QuickBooks children with Tax Codes in expected position\n";
    $pass++;
} else {
