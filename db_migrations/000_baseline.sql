-- db_migrations/000_baseline.sql
--
-- S-SCHEMA-BASELINE — Full-schema baseline migration.
--
-- PURPOSE: makes "apply all migrations from zero == master" TRUE. Before this
-- file, 113 of master's 156 tables had no corresponding migration (they lived
-- only in FLEETFORGE_DATABASE_MASTER.sql). A fresh DB built by migrations alone
-- reproduced only 43 tables. This baseline closes that gap.
--
-- WHAT THIS FILE DOES:
--   1. Creates all 156 schema tables using CREATE TABLE IF NOT EXISTS (pure additive;
--      guaranteed no-error on existing DBs — no DROP, no ALTER, no destructive
--      statement of any kind: D-BASELINE-1 / Guard 1).
--   2. Seeds schema_migrations with rows for all 96 pre-baseline migration files
--      (checksum='' → Runner treats as pre-checksum archived baseline, not drift).
--      INSERT IGNORE ensures no collision on existing rows (Guard 1 idempotency).
--
-- WHAT THIS FILE DOES NOT DO:
--   - Does not drop or alter any existing table or column.
--   - Does not affect data (no INSERT into application tables).
--   - Does not run against existing prod automatically: prod must pre-mark this
--     file applied (INSERT into schema_migrations) BEFORE running migrate --apply.
--     See docs/runbooks/baseline_reconcile.md for the exact procedure.
--
-- GO-FORWARD RULE (D-BASELINE-6): new schema changes go as NEW db_migrations/
-- delta files (YYYYMMDDHHII_label.sql) AND keep FLEETFORGE_DATABASE_MASTER.sql
-- in sync. The upgraded keystone smoke enforces "from-zero == master." This
-- file is NEVER updated for new schema — it is a fixed snapshot of 2026-06-06.
--
-- DEFERRED FKs NOTE: the 3 circular FKs (billing_period_id, last_billed_invoice_id,
-- acc_bill_lines.asset_id) are already inline in their CREATE TABLE blocks in this
-- file (as they are in master). No separate ALTER TABLE is needed.
--
-- @session  S-SCHEMA-BASELINE
-- @decision D-BASELINE-1, D-BASELINE-2, D-BASELINE-3, D-BASELINE-4,
--           D-BASELINE-5, D-BASELINE-6
...
CREATE TABLE IF NOT EXISTS `acc_ai_suggestions` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_ap_payment_allocations` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_asset_impairments` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `asset_id` int unsigned NOT NULL,
...
CREATE TABLE IF NOT EXISTS `acc_auto_categorization_rules` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_bad_debt_writeoffs` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_bank_transactions` (
  ...
  `source` enum('manual','import','system','qbo_cdc') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'manual' COMMENT 'Per D-QBO-20-3: qbo_cdc tag added for read-only mirror rows pulled via cron/qbo_bank_cdc.php; existing rows preserve manual/import/system values (ENUM extension, not replacement).',
  `is_readonly` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'Per D-QBO-20-4: rows with source=qbo_cdc MUST set is_readonly=1; FF reconciliation UI rejects is_readonly=1 rows (double-count guard). BankTransactionPuller enforces on insert; admin bank-rec match endpoints enforce on user-initiated match attempts.',
  `qbo_bank_txn_id` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Composite {entity}:{id} cross-reference back to acc_qbo_bank_transaction_map.qbo_bank_txn_id for forensic drill-down (FK relationship lives on the map side for cleaner CASCADE semantics).',
  ...
  KEY `journal_entry_id` (`journal_entry_id`),
  KEY `created_by` (`created_by`),
  KEY `idx_qbo_bank_txn` (`qbo_bank_txn_id`),
...
CREATE TABLE IF NOT EXISTS `acc_cca_classes` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_cca_continuity` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_categorization_rules` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `name` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
  `vendor_id` int unsigned DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `acc_collection_notes` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `customer_id` int unsigned NOT NULL,
...
CREATE TABLE IF NOT EXISTS `acc_customer_deposits` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_disclosure_notes` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `fiscal_year` int NOT NULL,
...
CREATE TABLE IF NOT EXISTS `acc_dunning_letters` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `customer_id` int unsigned NOT NULL,
...
CREATE TABLE IF NOT EXISTS `acc_fx_revaluations` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_impairment_tests` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `asset_id` int unsigned NOT NULL,
  `fiscal_year` int NOT NULL,
...
CREATE TABLE IF NOT EXISTS `acc_lease_amortization_schedules` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_lease_classifications` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `lease_id` int unsigned NOT NULL,
...
CREATE TABLE IF NOT EXISTS `acc_lease_residual_reviews` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `lease_id` int unsigned NOT NULL,
  `fiscal_year` int NOT NULL,
...
CREATE TABLE IF NOT EXISTS `acc_oauth_states` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_promise_to_pay` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `customer_id` int unsigned NOT NULL,
  `invoice_id` int unsigned DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `acc_place_of_supply_rules` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_qbo_account_map` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_qbo_bank_account_map` (
  ...
  `qbo_bank_account_id` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Intuit Account.Id where AccountType IN ("Bank","CreditCard"); one-to-one per FF (uq_qbo)',
  `qbo_account_name_snapshot` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'QBO Account.Name at mapping time; verifyMappingStillValid compares + flags name_drift',
  `qbo_currency_snapshot` enum('CAD','USD') COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'QBO CurrencyRef.value at mapping time; verifyMappingStillValid compares + flags currency_drift (D-QBO-20-1)',
  ...
  `qbo_account_type_snapshot` varchar(20) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'QBO AccountType snapshot ("Bank" / "CreditCard") for drift detection',
  `mapping_status` enum('mapped','unmapped','conflict') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'mapped' COMMENT 'mapped = live link; unmapped = operator unlinked (row preserved for forensic trail); conflict = drift detected by verifyMappingStillValid',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Phase QBO-9 S-QBO-20: FF↔QBO bank account mapping (Exception #2 / D-QBO-CORE-2). One-time-import pattern mirroring AccountMatcher (S-QBO-8). Direct mapping path; alternate to gl_account_id pivot used by S-QBO-19 BillPaymentPusher. Snapshot columns drive verifyMappingStillValid drift detection.';
CREATE TABLE IF NOT EXISTS `acc_qbo_bank_transaction_map` (
  ...
  `qbo_bank_txn_id` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Composite key {entity_type}:{entity_id} per D-QBO-20-2 — QBO IDs are entity-scoped (Purchase #5 != Deposit #5)',
  `qbo_entity_type` enum('Purchase','Deposit','Transfer','JournalEntry') COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'QBO entity type per spec §8.12 CDC entities list',
  `qbo_entity_id` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Raw Intuit entity id (without prefix) for direct QBO API lookup via getEntity',
  `qbo_account_id` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'BankAccountRef.value snapshot — QBO bank account that this transaction belongs to from the pulling perspective (Transfer captured from one side only per D-QBO-20-5)',
  ...
  `qbo_currency_snapshot` varchar(3) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'QBO CurrencyRef.value snapshot; multi-currency surface — FX revaluation deferred to S-QBO-FX-RECON-FOLLOWUP',
  ...
  `qbo_description_snapshot` varchar(500) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'QBO description snapshot at last pull (Memo OR PrivateNote OR Line[0].Description fallback chain)',
  `pull_status` enum('pending','pulled','superseded','failed','skipped_unmapped_account','skipped_zero_amount') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'pending' COMMENT 'pulled = current; superseded = QBO-side deletion noticed via markStale (FF row preserved with note suffix); failed = pull error; skipped_unmapped_account = transaction for an unmapped QBO bank account per cron_skipped_unmapped=1 default; skipped_zero_amount = $0 transaction (no economic significance; filter out mirror clutter)',
  `pull_error` text COLLATE utf8mb4_unicode_ci COMMENT 'Last error for failed state',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Phase QBO-9 S-QBO-20: per-transaction mapping FF↔QBO. Composite {entity}:{id} key handles QBO ID entity-scoping. pull_status taxonomy includes skipped_unmapped_account (cron_skipped_unmapped=1 path) + skipped_zero_amount (mirror noise filter) + superseded (markStale path).';
CREATE TABLE IF NOT EXISTS `acc_qbo_bill_map` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_qbo_payment_map` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_qbo_webhook_events` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_qbo_customer_map` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_qbo_invoice_map` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_qbo_vendor_map` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_qbo_drift_events` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_qbo_sync_log` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_qbo_sync_queue` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_qbo_item_map` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_qbo_tax_code_map` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_recurring_entry_lines` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_report_configurations` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_tax_filing_periods` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_tax_remittances` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_vendor_credit_applications` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_workpaper_annotations` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acc_year_end_checklist` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `year` smallint unsigned NOT NULL,
...
CREATE TABLE IF NOT EXISTS `credit_notes` (
  ...
  `province_snapshot` varchar(10) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `customer_credit_applications` (
  ...
  `token_hash` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'SHA-256 hex of the public link token (D-CCA-1). The RAW token is emailed once and never stored; lookup is by hash. UNIQUE.',
  ...
  `status` enum('sent','opened','submitted','reviewed') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'sent' COMMENT 'Lifecycle (D-CCA-2). not_sent is DERIVED (no active row), not an enum value.',
  `review_outcome` enum('approved','declined','needs_info') COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Admin review decision (S-CCA-4). NULL until reviewed.',
  `form_data` json DEFAULT NULL COMMENT 'Full customer submission snapshot (D-CCA-4). NEVER written back to the customers row.',
  `rendered_html` mediumtext COLLATE utf8mb4_unicode_ci COMMENT 'Frozen HTML render of the submitted application (D-CCA-6). Populated in S-CCA-3.',
  `print_name_first` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Typed legal first name at signing (D-CCA-5).',
  `print_name_last` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Typed legal last name at signing (D-CCA-5).',
  ...
  `terms_version` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Terms version label in force at submit (D-CCA-5).',
  `terms_url` varchar(500) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Snapshot of the carrier-agreement URL shown at submit (D-CCA-5).',
  `signature_path` varchar(500) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'StorageClient path of the drawn-signature PNG (D-CCA-5). NEVER emitted in API output (Trap 7).',
  `submitted_ip` varchar(45) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Applicant IP at submit (legal metadata, D-CCA-5).',
  `submitted_user_agent` varchar(500) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Applicant User-Agent at submit (legal metadata, D-CCA-5).',
  ...
  `review_notes` text COLLATE utf8mb4_unicode_ci COMMENT 'Internal admin notes on the review (S-CCA-4).',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Phase CCA S-CCA-1: customer credit-application lifecycle. One row per send (re-send = new row, D-CCA-3); public tokenized invite (hash-stored, D-CCA-1); customer submission stored as a JSON snapshot, never written back to customers (D-CCA-4).';
...
CREATE TABLE IF NOT EXISTS `customer_equipment_rates` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `customer_id` int unsigned NOT NULL,
...
CREATE TABLE IF NOT EXISTS `customer_rate_history` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `customer_id` int unsigned NOT NULL,
  `equipment_type` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
  `lease_id` int unsigned DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `damage_claim_photos` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `invoices` (
  ...
  `late_fee_rule_id` int unsigned DEFAULT NULL,
  `late_fee_rule_snapshot` json DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `lease_billing_periods` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `lease_id` int unsigned NOT NULL,
  `invoice_id` int unsigned DEFAULT NULL,
  `period_start` date NOT NULL,
...
CREATE TABLE IF NOT EXISTS `lease_status_log` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `lease_id` int unsigned NOT NULL,
  `old_status` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL,
  `new_status` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL,
...
CREATE TABLE IF NOT EXISTS `notification_log` (
  ...
  `channel` enum('email','sms','in_app','webhook','slack') COLLATE utf8mb4_unicode_ci NOT NULL,
  ...
  `status` enum('queued','sent','delivered','failed','bounced','skipped') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'queued',
  `error_message` text COLLATE utf8mb4_unicode_ci,
  `sent_at` datetime DEFAULT NULL,
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `notification_log_archive` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `rule_id` int unsigned DEFAULT NULL,
  `channel` enum('email','sms','in_app','webhook','slack') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
  ...
  `status` enum('queued','sent','delivered','failed','bounced','skipped') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'queued',
...
-- ── schema_migrations seed: mark all 96 pre-baseline migrations as applied ──
-- These rows prevent Runner from trying to re-apply pre-baseline deltas on a
-- fresh DB. checksum='' is treated by Runner as pre-checksum baseline (not drift).

INSERT IGNORE INTO `schema_migrations` (`version`, `filename`, `checksum`, `applied_by`, `execution_ms`, `applied_at`)
VALUES
  ('SAMSARA-1_schema', 'SAMSARA-1_schema.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('S-FIX-2_credit_notes_source_overpayment', 'S-FIX-2_credit_notes_source_overpayment.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('S-PROD-1A_security_hardening', 'S-PROD-1A_security_hardening.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('S-PROD-2_ses_bounce_handler', 'S-PROD-2_ses_bounce_handler.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('S-LEASE-MILEAGE_schema', 'S-LEASE-MILEAGE_schema.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605032052_S-CLEANUP-BATCH-1_settings_seeds', '202605032052_S-CLEANUP-BATCH-1_settings_seeds.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605040316_S-MILEAGE-1_precharge_schema', '202605040316_S-MILEAGE-1_precharge_schema.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605040430_S-MILEAGE-1B_samsara_fixture_setting', '202605040430_S-MILEAGE-1B_samsara_fixture_setting.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605040530_S-CLEANUP-LBP-100_drop_lbp_model_a_scaffolding', '202605040530_S-CLEANUP-LBP-100_drop_lbp_model_a_scaffolding.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605040717_S-CLEANUP-LBP-COLS-2_drop_lbp_reconciliation_scaffolding', '202605040717_S-CLEANUP-LBP-COLS-2_drop_lbp_reconciliation_scaffolding.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605062139_S-LEASE-GPS-COST_lease_gps_cost', '202605062139_S-LEASE-GPS-COST_lease_gps_cost.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605120906_S-MILEAGE-2B_drawdown_enum', '202605120906_S-MILEAGE-2B_drawdown_enum.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605120907_S-MILEAGE-2B_model_c_retirement', '202605120907_S-MILEAGE-2B_model_c_retirement.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605121902_S-MILEAGE-3_credit_notes_source_ext', '202605121902_S-MILEAGE-3_credit_notes_source_ext.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605121925_S-MILEAGE-3_close_adjustments_drop', '202605121925_S-MILEAGE-3_close_adjustments_drop.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605122229_S-TEMPLATE-MILEAGE-DEFAULTS_not_null_default', '202605122229_S-TEMPLATE-MILEAGE-DEFAULTS_not_null_default.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605130934_S-SEED-RATE-CARDS-LOAD_standard_2025_default', '202605130934_S-SEED-RATE-CARDS-LOAD_standard_2025_default.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605132211_S-AUTH-FIX_mfa_verified_until', '202605132211_S-AUTH-FIX_mfa_verified_until.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605140907_S-SETTINGS-CLEANUP_security_mfa_labels', '202605140907_S-SETTINGS-CLEANUP_security_mfa_labels.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605170100_S-BILLING-HOLISTIC-ENGINE_invoice_audit_columns', '202605170100_S-BILLING-HOLISTIC-ENGINE_invoice_audit_columns.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605170101_S-BILLING-HOLISTIC-ENGINE_line_item_type_enum', '202605170101_S-BILLING-HOLISTIC-ENGINE_line_item_type_enum.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605170102_S-BILLING-HOLISTIC-ENGINE_credit_notes_source_enum', '202605170102_S-BILLING-HOLISTIC-ENGINE_credit_notes_source_enum.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605170103_S-BILLING-HOLISTIC-ENGINE_leases_engine_version', '202605170103_S-BILLING-HOLISTIC-ENGINE_leases_engine_version.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605170104_S-BILLING-HOLISTIC-ENGINE_lock_existing_to_old', '202605170104_S-BILLING-HOLISTIC-ENGINE_lock_existing_to_old.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605170105_S-BILLING-HOLISTIC-ENGINE_settings_engine_flag', '202605170105_S-BILLING-HOLISTIC-ENGINE_settings_engine_flag.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605170200_S-DESIGN-SETTINGS-FOOTER-LOGIN_brand_settings_seed', '202605170200_S-DESIGN-SETTINGS-FOOTER-LOGIN_brand_settings_seed.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605190030_S036_ai_summaries_accounting_types', '202605190030_S036_ai_summaries_accounting_types.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605190100_S037FX_schema', '202605190100_S037FX_schema.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605190200_S037YE_schema', '202605190200_S037YE_schema.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605190300_S-ACCT-AJE_workflow', '202605190300_S-ACCT-AJE_workflow.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605190400_S-ACCT-WTB', '202605190400_S-ACCT-WTB.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605190500_S-ACCT-CCA-1', '202605190500_S-ACCT-CCA-1.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605190600_S-ACCT-COMP', '202605190600_S-ACCT-COMP.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605190700_S-ACCT-POS', '202605190700_S-ACCT-POS.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605190800_S-ACCT-GPS', '202605190800_S-ACCT-GPS.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605190900_S-ACCT-DISC', '202605190900_S-ACCT-DISC.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605191000_S-ACCT-DMG', '202605191000_S-ACCT-DMG.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605191100_S-ACCT-LESSOR-1', '202605191100_S-ACCT-LESSOR-1.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605191200_S-ACCT-LESSOR-2', '202605191200_S-ACCT-LESSOR-2.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605191300_S-ACCT-LESSOR-3', '202605191300_S-ACCT-LESSOR-3.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605191400_S-ACCT-LESSOR-4', '202605191400_S-ACCT-LESSOR-4.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605191500_S-ACCT-LESSOR-5', '202605191500_S-ACCT-LESSOR-5.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605191600_S-ACCT-LESSOR-6', '202605191600_S-ACCT-LESSOR-6.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605200400_S-PERM-SESSION-REFRESH', '202605200400_S-PERM-SESSION-REFRESH.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605200500_S-QBO-1', '202605200500_S-QBO-1.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605201900_S-QBO-2', '202605201900_S-QBO-2.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605202100_S-QBO-3', '202605202100_S-QBO-3.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605202200_S-QBO-OAUTH-FIX', '202605202200_S-QBO-OAUTH-FIX.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605202300_S-QBO-5', '202605202300_S-QBO-5.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605210233_S-QBO-7', '202605210233_S-QBO-7.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605210757_S-QBO-8', '202605210757_S-QBO-8.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605211726_S-QBO-9', '202605211726_S-QBO-9.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605212000_S-QBO-10', '202605212000_S-QBO-10.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605212200_S-INTEL-TAB', '202605212200_S-INTEL-TAB.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605222000_S-INTEL-V2-A', '202605222000_S-INTEL-V2-A.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605222100_S-INTEL-V2-B', '202605222100_S-INTEL-V2-B.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605222200_S-INTEL-V2-C', '202605222200_S-INTEL-V2-C.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605222300_S-INTEL-V2-D', '202605222300_S-INTEL-V2-D.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605222400_S-INTEL-V2-E', '202605222400_S-INTEL-V2-E.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605240000_S-QBO-VALIDATOR-SCOPE-SPLIT', '202605240000_S-QBO-VALIDATOR-SCOPE-SPLIT.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605240100_S-QBO-11', '202605240100_S-QBO-11.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605240200_S-QBO-11-FIXPACK-1', '202605240200_S-QBO-11-FIXPACK-1.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605250000_S-QBO-FIXPACK-3', '202605250000_S-QBO-FIXPACK-3.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605260000_S-SETTINGS-AUDIT-3', '202605260000_S-SETTINGS-AUDIT-3.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605270000_S-QBO-PUSHER-SKIP-RECORD-FIX-INVOICE', '202605270000_S-QBO-PUSHER-SKIP-RECORD-FIX-INVOICE.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605270100_S-QBO-CUSTOMER-VENDOR-PUSH-STATE-INFRA', '202605270100_S-QBO-CUSTOMER-VENDOR-PUSH-STATE-INFRA.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605270200_S-QBO-12', '202605270200_S-QBO-12.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605270300_S-VENDOR-CURRENCY-COLUMN', '202605270300_S-VENDOR-CURRENCY-COLUMN.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605270400_S-QBO-18', '202605270400_S-QBO-18.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605271000_S-QBO-BILL-GOTCHAS-PAYDOWN', '202605271000_S-QBO-BILL-GOTCHAS-PAYDOWN.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605271500_S-QBO-13', '202605271500_S-QBO-13.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605290800_S-QBO-15', '202605290800_S-QBO-15.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605291000_S-QBO-19', '202605291000_S-QBO-19.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605292000_S-QBO-21', '202605292000_S-QBO-21.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605300000_S-PORTAL-REQUEST-ROUTING', '202605300000_S-PORTAL-REQUEST-ROUTING.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605301000_S-PORTAL-REQUEST-THREAD', '202605301000_S-PORTAL-REQUEST-THREAD.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605302000_S-QBO-20', '202605302000_S-QBO-20.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605302200_S-QBO-22', '202605302200_S-QBO-22.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605302300_S-QBO-23', '202605302300_S-QBO-23.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605302400_S-QBO-16', '202605302400_S-QBO-16.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605302500_S-QBO-24', '202605302500_S-QBO-24.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202605302600_S-QBO-25', '202605302600_S-QBO-25.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202606010000_S-QBO-CREDIT-MEMO-APPLY', '202606010000_S-QBO-CREDIT-MEMO-APPLY.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202606010100_S-QBO-17', '202606010100_S-QBO-17.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202606010200_S-QBO-27', '202606010200_S-QBO-27.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202606010300_S-QBO-CREDIT-APP-UNAPPLY', '202606010300_S-QBO-CREDIT-APP-UNAPPLY.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202606010400_S-QBO-BILL-ITC-TAX-RATE', '202606010400_S-QBO-BILL-ITC-TAX-RATE.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202606010500_S-QBO-FA-MAP', '202606010500_S-QBO-FA-MAP.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202606020000_S-CRON-FIX-NOTIFICATION-HARDENING', '202606020000_S-CRON-FIX-NOTIFICATION-HARDENING.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202606030000_S-CRON-FIX-REMAINING', '202606030000_S-CRON-FIX-REMAINING.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202606040000_notification-preferences', '202606040000_notification-preferences.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202606050000_equipment-template-combo-category', '202606050000_equipment-template-combo-category.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202606060001_yards-soft-delete', '202606060001_yards-soft-delete.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202606060002_S-CCA-1_credit_applications', '202606060002_S-CCA-1_credit_applications.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202606060003_S-PERM-DRIFT-FIX_role_permission_overrides', '202606060003_S-PERM-DRIFT-FIX_role_permission_overrides.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00'),
  ('202606060004_S-CCA-2_settings_disclaimer', '202606060004_S-CCA-2_settings_disclaimer.sql', '', 'baseline:000_baseline', 0, '2026-06-06 00:00:00')
;

-- end of 000_baseline.sql
